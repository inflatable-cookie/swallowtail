...
if [ "$(wc -l < "$route_matrix_actual" | tr -d ' ')" -ne 35 ]; then
  printf 'provider route matrix must contain exactly 35 route rows\n' >&2
  exit 1
fi
...
if [ "$(wc -l < "$route_lifecycle_actual" | tr -d ' ')" -ne 35 ]; then
  printf 'provider session lifecycle matrix must contain exactly 35 route rows\n' >&2
  exit 1
fi
...
printf 'provider route, lifecycle, 28-solution/35-route feature, and activity matrices passed\n'

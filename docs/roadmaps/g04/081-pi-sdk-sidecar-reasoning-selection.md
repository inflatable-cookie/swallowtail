...
Its feature-matrix `reasoning_selection` cell was `No` until this lane.
...
bootstrap and state snapshots. Pi clamps unsupported values to model capability,
...
Qualify and bind immutable portable `ReasoningSelection` on `pi.sdk-sidecar`
for one closed provider/model family across new, load, resume, and fresh
...
- [x] promote Research 228 with a non-empty exact deliver-now table
- [x] bind only exact model/value/lifecycle rows whose selected value is
      confirmed by `session.thinkingLevel` before readiness
- [x] preserve selection in immutable request, plan, prepared evidence,
...
source-tagged sidecar, and private wire `swallowtail-pi-sdk-jsonl-v1`.

Research 228 admits one bounded row: `anthropic` / `claude-opus-4-5` with
`off|minimal|low|medium|high`. Static pi-ai metadata gates preparation; auth
filters runtime availability only.
...
- [x] promote Research 228 with an exact bounded table

### Batch 81.2 — Conditional Selection Binding

- [x] Execute card 226.
- [x] bind admitted portable reasoning rows through preparation, plan,
      sidecar bootstrap, attachment, and effective-state confirmation

### Batch 81.3 — Route-Local Acceptance

- [x] Execute card 227.
- [x] prove exact selection, omission, rejection, lifecycle, replay, and
      cleanup truth

## Acceptance Criteria

- [x] only exact Research 228 provider/model/value/lifecycle rows prepare
- [x] request, plan, prepared evidence, sidecar command, and reported state
      agree before readiness
- [x] unsupported values, model drift, clamp, substitution, stored-state
...
- [x] omission retains exact prior behavior and claims no selected/default
      mode
- [x] load, resume, and fresh restoration preserve Contract 012/017 meaning
...
- this milestone: exact Pi SDK-sidecar reasoning selection — complete through
  cards 225-227
...
- Stop after card 225 if exact model/value membership cannot be made static
  enough for preparation-time rejection. **Did not fire on final promotion.**

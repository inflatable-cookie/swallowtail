...
/// Returns the census semantic id of one published `qoder.headless` row.
///
/// An identity outside the tranche panics rather than being folded into a
/// neighbouring row, so a borrowed Deep Agents, Kiro, or ZCode identity
/// cannot pass.
pub(super) fn semantic_id(identity: &ConsumerRouteRowIdentity) -> String {
    assert!(
        identity.namespaced_extension().is_none(),
        "qoder.headless publishes no bounded route-local descriptor: {identity:?}"
    );
    match identity {
        ConsumerRouteRowIdentity::Feature(feature) => match feature {
            ...
            other => panic!("unexpected qoder.headless projection feature {other:?}"),
        },
        ConsumerRouteRowIdentity::Control(control) => {
            panic!("qoder.headless publishes no selectable control: {control:?}")
        }
    }
    .to_owned()
}

use crate::headless_events::CursorHeadlessEventParser;
use crate::headless_handle::CursorHeadlessCancellation;
...
async fn next_output(
    ...
    cancellation: &CursorHeadlessCancellation,
...
fn event_delivery_failed(cleanup: CleanupOutcome) -> TerminalOutcome {
    TerminalOutcome::new(
        TerminalStatus::RuntimeFailed(SafeDiagnostic::new(
            "swallowtail.cursor.headless.event_delivery_failed",
            "Cursor headless event delivery failed",
        )),
        cleanup,
    )
}
...
fn process_cleanup_failed() -> CleanupOutcome {
    CleanupOutcome::Failed(SafeDiagnostic::new(
        "swallowtail.cursor.headless.process_cleanup_failed",
        "Cursor headless process cleanup failed",
    ))
}

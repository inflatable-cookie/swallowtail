...
| `gemini.live` | `swallowtail-adapter-gemini`; `swallowtail.gemini.live` | realtime media session with exact `minimal\|low\|medium\|high` reasoning selection on `gemini-3.1-flash-live-preview`; Gemini Live raw WebSocket | approved Gemini Live endpoint; project authorization API key | `gemini.live-facade`; exact opaque `...BidiGenerateContent.thinking-2026-08-23` revision | `prepare_gemini_live` → `prepare_live_session` → `open_session` or fresh realtime restoration; the selected level remains fixed across initial setup, one planned rollover, and fresh restoration, while omission keeps `MINIMAL` without claiming caller selection | `GeminiLiveDriver`; `RealtimeMediaSessionDriver` |

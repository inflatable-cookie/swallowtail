...
impl AccessLeases {
    pub(super) async fn acquire(
        ...
        {
            return Err(failure(
                "swallowtail.deepseek.network_grant_mismatch",
                "DeepSeek network grant did not match preflight",
            ));
        }
            ...
            || !matches!(credential, CredentialLease::Secret(_))
        {
            ...
            return Err(failure(
                "swallowtail.deepseek.credential_lease_rejected",
                "DeepSeek credential lease did not match preflight",
            ));
        }
        Ok(Self {
        ...
        match self.credential.take() {
            Some(lease) => match services.credential() {
                ...
                None => CleanupOutcome::Failed(swallowtail_core::SafeDiagnostic::new(
                    "swallowtail.deepseek.credential_release_failed",
                    "DeepSeek credential service disappeared during cleanup",
                )),
            },
...
fn missing(service: &str) -> RuntimeFailure {
    failure(
        "swallowtail.deepseek.host_service_missing",
        format!("DeepSeek direct inference requires the {service} service"),
    )
}

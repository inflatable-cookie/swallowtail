...
The candidate preparation stopped at a second real release-gate defect after
the coordinator corrected the first one. The worker branch was rebased onto
the coordinator's `origin/main` at `99825ba32ea6e87ca107c4d4c1b964b5b87598c8`,
which includes the Card 103 closeout index entry. The preserved release note,
release index entry, and Card 101 Result remain scoped to this card.

The fresh read-only status report is retained at
`.effigy/reports/release/status-0.4.2-attempt-2.json` (SHA-256
...
and `Cargo.lock`) on both attempts; `.release-prepared.json` was not written.
...
pending correction of the Card 103 roadmap status and a new coordinator
dispatch.

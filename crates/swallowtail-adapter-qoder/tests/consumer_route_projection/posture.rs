...
use super::fixtures::{contribution, ready_status, run};
...
fn every_published_row_keeps_its_exact_view_lifecycle_and_actor_posture() {
    let published = contribution(&run(), "qoder.headless.posture");
...
fn every_published_row_publishes_supported_and_available_separately() {
    ...
    let published = contribution(&run(), "qoder.headless.support");

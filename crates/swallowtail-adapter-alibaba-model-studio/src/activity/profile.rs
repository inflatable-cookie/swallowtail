...
pub(crate) fn activity_profile() -> ObservableActivityProfile {
    let binding = crate::alibaba_model_studio_facade_binding();
    let assessment = crate::alibaba_model_studio_facade_claim().assess(binding.version());
    let behavior = assessment
        .behavior_revision()
        .expect("static Alibaba facade is qualified")
        .clone();
    ObservableActivityProfile::available(
        ...
        .expect("static Alibaba activity kind is valid")],
        ActivityUnknownEventPosture::FailClosed,
    )
    .expect("static Alibaba activity profile is valid")
}
...
pub(crate) fn with_activity(
    ...
    requirements.push(
        profile
            ...
            .expect("Alibaba activity is available"),
    );

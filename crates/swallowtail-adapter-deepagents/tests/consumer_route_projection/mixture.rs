use swallowtail_adapter_deepagents::{DeepAgentsPreparedIntegration, deepagents_acp_descriptor};
...
///
/// `kiro.acp` is a separate release-leaf package, so this proof rebuilds its
...
pub(super) const NEIGHBOUR_DRIVER_ID: &str = "swallowtail.kiro.acp";
...
pub(super) fn ready(integration: &DeepAgentsPreparedIntegration) -> &AccessStatus {
...
pub(super) fn neighbour_descriptor() -> DriverDescriptor {
    DriverDescriptor::new(
        AdapterIdentity::new(
            neighbour_driver_id(),
            deepagents_acp_descriptor().identity().version().clone(),
        ),
        IntegrationFamilyId::new("kiro-acp").expect("family id is valid"),

...
pub(super) struct ImageInput {
    ...
    pub(super) materialization: SharedAttachment,
}
...
pub(super) async fn prepare_image(
    ...
    scope: ScopeId,
) -> Result<Option<ImageInput>, RuntimeFailure> {
    ...
    Ok(Some(ImageInput {
        ...
        materialization: SharedAttachment::new(service, lease),
    }))
}

fn failure(reason: &'static str) -> RuntimeFailure {
    crate::failure::failure(
        "swallowtail.anthropic.attachment_materialization_failed",
        format!("Anthropic Messages {reason}"),
    )
}

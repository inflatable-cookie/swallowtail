...
impl AcpActivityProjection {
    ...
    fn unknown(&mut self, namespace: &str) -> Result<Vec<ActivityObservation>, RuntimeFailure> {
        ...
        let activity = OpenActivity {
            id: ActivityId::new(format!("goose-acp:unknown:{}", self.next_id))
                .map_err(|_| malformed())?,
            provider_ref: None,
            kind: ActivityKind::Unknown(
                ActivityNamespace::new(format!("goose.acp.{namespace}"))
                    .map_err(|_| malformed())?,
            ),
    ...
    fn open_or_insert(
        ...
        let id = ActivityId::new(format!("goose-acp:{key}"))
            .or_else(|_| {
                self.next_id = self.next_id.saturating_add(1);
                ActivityId::new(format!("goose-acp:minted:{}", self.next_id))
            })
            .map_err(|_| malformed())?;

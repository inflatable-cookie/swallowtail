...
/// ordinary feature or either control into another view or lifecycle fails
/// here even when the emitted identity set is unchanged.
fn expected() -> BTreeSet<Posture> {
    BTreeSet::from([
        ...
        (
            "control.model-selection".to_owned(),
            "selection-summary",
            ConsumerRouteLifecycle::SelectionSummary,
            ConsumerRouteActorPosture::ConsumerSelectable,
        ),
        (
            "control.app-server-mode".to_owned(),
            "session-start",
...
fn every_published_row_keeps_its_exact_view_lifecycle_and_actor_posture() {
    for name in [PLAN, BUILD] {
        let published = contribution(&profile(name), "zcode.app-server.posture");
        assert_eq!(observed(&published), expected(), "{name} drifted");
    }
}
...
fn every_published_row_publishes_supported_and_available_separately() {
    let ready = projection_fixture::ready_status();
    ...
    let published = contribution(&profile(PLAN), "zcode.app-server.support");
    assert_eq!(all_rows(&published).count(), 10);

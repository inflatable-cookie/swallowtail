...
impl RunHandle for AnthropicRunHandle {
        ...
        Box::pin(async move {
            ...
            match self.task.join().await {
                ...
                Err(_) => CleanupOutcome::Failed(SafeDiagnostic::new(
                    "swallowtail.anthropic.task_join_failed",
                    "Anthropic operation task could not be joined",
                )),
            }
        })
    }

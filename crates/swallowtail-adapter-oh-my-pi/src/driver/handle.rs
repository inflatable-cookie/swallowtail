...
impl TurnCancellation {
    pub(super) fn new(connection: Arc<OhMyPiConnection>, turn: Arc<ActiveTurn>) -> Self {
        Self {
            connection,
            turn,
...
impl SessionCancellation {
    pub(super) fn new(connection: Arc<OhMyPiConnection>, active: ActiveSlot) -> Self {
        Self {
            connection,
            active,
            requested: AtomicBool::new(false),
...
pub(super) struct OhMyPiTurnHandle {
...
pub(super) struct OhMyPiTurnBinding {
...
impl TurnHandle for OhMyPiTurnHandle {
            ...
            let task_cleanup =
                if let Some(task) = active.and_then(|mut active| active.deadline_task.take()) {
                    if task.join().await.is_err() {
                        cleanup_failure(
                            "turn_join_failed",
                            "OhMyPi RPC turn deadline task did not join cleanly",
                        )
                    } else {

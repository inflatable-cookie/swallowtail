...
#[path = "activity/content.rs"]
mod content;
#[path = "activity/tool.rs"]
mod tool;
...
impl AcpActivityProjection {
    ...
    fn unknown(&mut self, namespace: &str) -> Result<Vec<ActivityObservation>, RuntimeFailure> {
        ...
        let activity = OpenActivity {
            id: ActivityId::new(format!("opencode-acp:unknown:{}", self.next_id))
                .map_err(|_| malformed())?,
            provider_ref: None,
            kind: ActivityKind::Unknown(
                ActivityNamespace::new(format!("opencode.acp.{namespace}"))
                    .map_err(|_| malformed())?,
            ),
    ...
    fn open_or_insert(
        ...
        let id = ActivityId::new(format!("opencode-acp:{key}"))
            .or_else(|_| {
                self.next_id = self.next_id.saturating_add(1);
                ActivityId::new(format!("opencode-acp:minted:{}", self.next_id))
            })
            .map_err(|_| malformed())?;

...
/// Working resource for one bounded Copilot CLI ACP session.
...
#[derive(Clone, Debug, Eq, PartialEq)]
/// Prepared Copilot CLI ACP session ready for initialize plus one bounded prompt.
pub struct CopilotCliPreparedSession {
...
fn access_requirement(integration: &CopilotCliPreparedIntegration) -> AccessRequirement {

...
async fn probe_joined(
    ...
    if request.target().version_axis() != claim.axis() {
        return Err(failure(
            ...
            "Claude Agent discovery target uses a different version axis",
        ));
    }
...
async fn probe_process(
    ...
    .map_err(|_| {
        failure(
            "swallowtail.claude_agent.discovery_classification_failed",
            "Claude Agent version observation could not be classified",
        )
    })?;
...
fn outcome(status: DiscoveryStatus) -> DiscoveryOutcome {
    DiscoveryOutcome::new(
        ...
        Some(SafeDiagnostic::new(
            status_code(status),
            "Claude Agent installed discovery did not produce a compatible observation",
        )),
    )
}
...
const fn status_code(status: DiscoveryStatus) -> &'static str {
    match status {
        DiscoveryStatus::Absent => "swallowtail.claude_agent.discovery_absent",
        DiscoveryStatus::Discovered => "swallowtail.claude_agent.discovery_discovered",
        DiscoveryStatus::Incompatible => "swallowtail.claude_agent.discovery_incompatible",
        DiscoveryStatus::Malformed => "swallowtail.claude_agent.discovery_malformed",
        DiscoveryStatus::TimedOut => "swallowtail.claude_agent.discovery_timed_out",
        DiscoveryStatus::Cancelled => "swallowtail.claude_agent.discovery_cancelled",
        DiscoveryStatus::Failed => "swallowtail.claude_agent.discovery_failed",
        DiscoveryStatus::CleanupFailed => "swallowtail.claude_agent.discovery_cleanup_failed",
    }
}

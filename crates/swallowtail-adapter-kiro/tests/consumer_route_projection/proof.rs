...
#[test]
fn the_coverage_ledger_dispositions_exactly_the_nine_kiro_rows() {
    let mut tuples = BTreeSet::new();
    let mut semantics = BTreeSet::new();
    for entry in &KIRO_ACP_TRANCHE {
        assert_eq!(
            entry.route_id, KIRO_ROUTE,
            "{} does not belong to the exact census route",
            entry.semantic_id
        );
        assert!(
            KIRO_OPERATION_SHAPES.contains(&entry.operation_shape),
            "{} names an operation shape outside the kiro.acp census",
            entry.semantic_id
        );
        assert!(
        ...
        for profile in entry.emitted_by {
            assert!(
                KIRO_PROFILES.contains(profile),
                "{} names an unknown prepared Kiro profile",
                entry.semantic_id
            );
        }
    }
    assert_eq!(KIRO_ACP_TRANCHE.len(), 9);
    assert_eq!(tuples.len(), 9);
    assert_eq!(semantics.len(), 9);
}

#[test]
fn every_prepared_kiro_profile_emits_exactly_its_ledger_identities() {
    let observed = observed_dispositions();
    assert_eq!(observed.len(), KIRO_PROFILES.len());
    for profile in KIRO_PROFILES {
...
#[test]
fn withheld_kiro_rows_are_emitted_by_no_prepared_profile() {
    let observed = observed_dispositions();
    ...
    for off_route in WITHHELD_OFF_ROUTE {
        assert!(
            ...
            "{off_route} has no kiro.acp census row and must never be constructed"
        );
        assert!(
            !KIRO_ACP_TRANCHE
                .iter()

...
Wire the consumer-supplied streamable-HTTP MCP entry into `claude-agent.acp`
production `session/new` at the route's current point (`0.79.0` in Research
351). Provider-free tests only.

## Why It Matters

Research 351 classified `claude-agent.acp` as a `direct-http-candidate` from its shipped
...
- Emit `{ type: "http", name, url, headers: [{name, value}] }`. The provider also maps `stdio` and `sse`; emit `http` only.
...
- Only `claude-agent.acp` paths inside the crate: Claude Code, the watcher, and the SDK route stay untouched.
...
- **Owned mutable paths:** `crates/swallowtail-adapter-claude-agent/src/` modules for `claude-agent.acp` only, with their tests and fixtures; `docs/guides/claude-agent-prepared-integration.md`; the `claude-agent.acp` route
  and feature matrix rows; the working public-API baseline for `swallowtail-adapter-claude-agent` if
...
Research 351. Focused and affected-package validation for `swallowtail-adapter-claude-agent`, route and
docs QA.

## Next Task

Chatterbox asks the operator whether to run a live gate for `claude-agent.acp`.

---
title: g04.011 hosted API-key Anthropic Messages worker handoff
...
created: 2026-08-20
updated: 2026-08-20
handoff_path: /Users/tom/Dev/projects/swallowtail/docs/handoffs/20260820-142930-g04-011-hosted-api-key-anthropic-messages.md
...
The connection-lifecycle facade through overlay is on `main`. Research 169
mapped Anthropic Messages, Codex app-server, and Ollama attach onto
Contract 057. Anthropic Messages is the first implementation tranche: a
hosted API-key addable route that reuses `prepare_anthropic_direct`.
...
Without an adapter-local descriptor, a consumer still cannot list Anthropic
Messages in the addable catalog or collect an API key through 057. The
prepared facade already exists. First-proof is the seam between them, not a
new Messages driver.
...
- **Planning base commit:** `98fc43de535a36febb2e95e604be081a0cbfd0c9`
...
- **Planning artifacts included at the base:** Contract 057; realized
  lifecycle facade; Research 169; completed g04.010; milestone g04.011;
  ready cards 030-032
- **Worker branch:** `g04-011-hosted-api-key-anthropic-messages`
- **Worker worktree:** launcher-provided dedicated worktree first. Manual
  fallback path is
  `$AGENTS_WORKTREE_CONTAINER_DIR/swallowtail-g04-011-hosted-api-key-anthropic-messages`
- **Worktree creation command:** only if the current context is unusable and
  ...
  `git worktree add -b g04-011-hosted-api-key-anthropic-messages "$AGENTS_WORKTREE_CONTAINER_DIR/swallowtail-g04-011-hosted-api-key-anthropic-messages" origin/main`
...
- **Roadmap milestone:** `docs/roadmaps/g04/011-hosted-api-key-anthropic-messages.md`
- **Ready cards, in order:**
  `docs/roadmaps/g04/batch-cards/030-anthropic-messages-addable-descriptor.md`,
  then `docs/roadmaps/g04/batch-cards/031-anthropic-messages-admission-and-api-key.md`,
  then `docs/roadmaps/g04/batch-cards/032-anthropic-messages-refresh-subject-and-047.md`
- **Allowed runway:** g04.011 cards 030 → 031 → 032. Stop after the 047 and
  overlay path. Do not start Codex, Ollama, or hosted OAuth.
...
- **Parallel safety check:** admission and refresh write the same admitted
  Anthropic record. No parallel lane.
- **Canonical refs:** Contract 057; Contracts 011, 014, 037, 047; Research
  169; `anthropic_direct_descriptor`; `prepare_anthropic_direct`;
  `AddableRouteDescriptor`; `admit_instance`; `refresh_readiness`;
  `observe_authenticated_subject`; `apply_model_presentation_overlay`
...
- **PR base/head:** `main` / selected worker branch
  (`g04-011-hosted-api-key-anthropic-messages` unless the launcher supplied a
...
- **In scope:** adapter-local hosted addable descriptor for
  `anthropic.messages`; secret API-key field with optional env name
  `ANTHROPIC_API_KEY`; opaque endpoint config field; 057 admission and
  API-key collection as `CredentialRef`; reuse of `prepare_anthropic_direct`;
  host-supplied refresh; subject Absent; 047 snapshot plus overlay keys.
- **Out of scope:** hosted interactive OAuth; Claude subscription; Codex;
  Ollama; OpenHands production wiring; live Messages calls; reading secret
  bytes into portable records; adding overlay metadata to 047; rewriting
  `public-api-0.3.3`; GitHub Release; crates.io; tag mutation.
- Do not invent architecture or change Contract 057.
- Topology is hosted. Do not fold it into `ExecutionLayer`.
- API-key collection does not register URL-open, loopback, or device-code
  ports. Missing those ports must not fail this loop.
- Overlay already keys Anthropic catalogue rows because they carry
  `provider_id` `anthropic`. Do not invent a model or change
  `Ready` / `NotReady`.
...
- **Planning lineage:** g04 tagged `v0.3.3`, merged kernel through overlay
  (PRs 4-8), then Research 169. First-proof starts with hosted API-key
  Anthropic Messages.
- **Why these cards are ready:** Research 169 mapped the prepared facade
  onto 057. The descriptor, API-key collection, and 047 path are bounded.
  Hosted OAuth stays a remaining gate.
- **Decisions and preferences:** do not extract secrets. Do not treat Claude
  Agent or Codex ChatGPT as this hosted proof.
- **Open tensions:** `start_sign_in` still requires immediately-ready host
  futures; do not change that here. Codex and Ollama catalogue rows still
  omit `provider_id`; do not fix those here. There is no
...
- **Report after:** card 030 descriptor; card 031 admission and API-key
  collection; card 032 refresh/subject/047 and the PR
...
Once that checks out, take card 030 first. Ship the hosted Anthropic
Messages addable descriptor. When 030 is green, continue into 031, then 032.
When refresh, subject, and the 047 path are green, open the PR and stop.
...
4. From the selected worktree, confirm `git rev-parse HEAD` equals
   ...
   `git merge-base --is-ancestor 98fc43de535a36febb2e95e604be081a0cbfd0c9 HEAD`
...
- Do not start Codex, Ollama, or hosted OAuth work.

### When the assigned runway is complete

1. Run the required final validation for card 032, plus `effigy package:api`
...
4. Open a reviewable PR against the current pushed `main` tip. The handoff's
   planning base `98fc43de535a36febb2e95e604be081a0cbfd0c9` is the planning
   commit before the handoff was created, not a self-referential hash for the
   commit that contains this file.
5. In the PR body, link the milestone, cards 030-032, Contract 057, Research
...
- **Closeout refs:** cards 030-032, g04.011, `docs/roadmaps/README.md`,
  `docs/logs/README.md`

...
fn the_no_route_specific_control_audit_produces_no_public_control_descriptor() {
    let published = contribution(&session(), "deepagents.acp.audit");
    assert_eq!(
        published.session_start_rows().len(),
        0,
        "deepagents.acp admits no session-start control"
    );
...
#[test]
fn prepared_deepagents_rows_claim_no_acknowledgement_or_per_turn_authority() {
    let published = contribution(&session(), "deepagents.acp.authority");
    let named = published
        .sources()
    ...
    assert_eq!(
        named,
        vec![(
            "deepagents.acp.authority".to_owned(),
    ...
    for forbidden in [
        "/fixture/bin/",
        "deepagents.projection.isolated",
        "deepagents.projection.workspace",
    ] {
        assert!(
...
fn a_changed_source_id_replaces_the_snapshot_rather_than_merging_into_it() {
    let session = session();
    let first = contribution(&session, "deepagents.acp.first");
    let second = contribution(&session, "deepagents.acp.second");
    assert_eq!(
        rows(&first),
        rows(&second),
    ...
    for row in all_rows(&second) {
        assert_eq!(row.source().id().as_str(), "deepagents.acp.second");
    }
}
...
fn a_row_from_another_prepared_revision_is_rejected_at_admission() {
    let alternate = contribution(&alternate_revision(), "deepagents.acp.alternate");
    let borrowed = alternate
        .selection_rows()
        .next()
        .expect("the alternate session publishes rows")
        .clone();
    let mine = contribution(&session(), "deepagents.acp.local");
    assert_ne!(
...
/// Proves a drifted access dimension yields no `deepagents.acp` row at all.
#[test]
fn a_drifted_access_dimension_publishes_no_deepagents_row() {
    for observed in drifted_observations() {
        let failure = prepared_with("1", observed)
            .and_then(|integration| integration.prepare_session(session_input("drift")))
...
fn the_admitted_snapshot_keeps_each_access_dimension_observable() {
    let admitted = contribution(&session(), "deepagents.acp.dimensions");
    ...
    assert!(
        ...
        "the prepared ACP session binds no model route, so no model row may be published"
    );
}

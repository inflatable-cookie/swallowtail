use super::QoderHeadlessPreparedIntegration;
...
/// Prompt, read-only resource, and host deadline for one Qoder stream-json print run.
pub struct QoderHeadlessRunProfileInput {
...
impl QoderHeadlessRunProfileInput {
    #[must_use]
    /// Creates an explicit print-run profile with no hidden model or yolo default.
    pub const fn new(
        request_id: RequestId,
        content: OperationContent,
        working_resource: WorkingResourceRef,
...
#[derive(Clone, Debug, Eq, PartialEq)]
/// Prepared Qoder headless run with immutable plan, request, and evidence agreement.
pub struct QoderHeadlessPreparedRun {
...
impl QoderHeadlessPreparedIntegration {
    /// Validates and prepares one bounded stream-json print run without starting provider work.
    pub fn prepare_run(
        &self,
        ...
        let descriptor = crate::qoder_headless_descriptor();
...
impl QoderHeadlessPreparedRun {
    ...
    pub fn low_level_driver(&self) -> crate::QoderHeadlessDriver {
        crate::QoderHeadlessDriver::new(self.environment.clone())
    }
...
fn access_requirement(integration: &QoderHeadlessPreparedIntegration) -> AccessRequirement {

...
| `grok-build.catalogue` | `not-applicable` | `no` | `not-applicable` | `not-applicable` | `not-applicable` | `not-applicable` | exact `1.0.25` under `QualifiedOnly`; every older or newer point fails closed; catalogue has no provider session | process and time cleanup only |

...
| `1.1.19→1.1.20` | review mode auto-approves reads inside the workspace; benign tool errors and permission denials no longer become cascade failure | skill icons, customization listing, settings preservation | compatible repairs within selected Read/ReadWrite authority |
| `1.1.20→1.1.21` | invalid-UTF-8 tool results no longer stall the stream; read/write repairs stay under selected resource authority; embedded ripgrep removes an ambient child dependency | voice and `mic-serve`; always-proceed MCP/page autoapproval; cost line | compatible repair; always-proceed is unselected |

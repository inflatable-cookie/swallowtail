use crate::ZcodePreparedIntegration;
...
use swallowtail_core::{
    ...
    HostServiceKind, IntegrationFamilyId, ModelId, ModelRoute, ModelRouteId, ModelRouteRevision,
    OperationRequirements, OperationShape, PreflightContext, PreflightFailure, PreflightPlan,
    ProviderId, TransportFamilyId, preflight,
};
...
///
/// `qoder.headless` is a separate release-leaf package, so this proof rebuilds
/// its exact route identity from public core types instead of adding a
/// cross-adapter dependency the release staging does not admit. Every other
/// applicability dimension is copied from this prepared run, so the composer
/// rejection isolates route identity alone.
pub(super) const NEIGHBOUR_DRIVER_ID: &str = "swallowtail.qoder.headless";
...
/// Exact host services the prepared app-server run requires.
...
pub(super) fn ready(integration: &ZcodePreparedIntegration) -> &AccessStatus {
...
pub(super) fn neighbour_descriptor() -> DriverDescriptor {
    DriverDescriptor::new(
        AdapterIdentity::new(
            neighbour_driver_id(),
            crate::zcode_app_server_descriptor()
                .identity()
                .version()
                .clone(),
        ),
        IntegrationFamilyId::new("qoder-headless").expect("family id is valid"),
        TransportFamilyId::new("qoder-stream-json").expect("transport id is valid"),
    )
...
/// Rebuilds the exact prepared model route the run binds.
pub(super) fn model_route(instance: &ConfiguredInstance) -> ModelRoute {
    ModelRoute::new(
        ModelRouteId::new(projection_fixture::MODEL_ROUTE_ID).expect("route id is valid"),
        ModelRouteRevision::new("projection-1").expect("route revision is valid"),
        instance.id().clone(),
        ModelId::new(projection_fixture::MODEL_ID).expect("model id is valid"),
        instance.capabilities().clone(),
    )
    .with_provider_id(ProviderId::new(projection_fixture::PROVIDER_ID).expect("provider is valid"))
}
...
pub(super) fn plan_with(
    ...
    let route = model_route(instance);

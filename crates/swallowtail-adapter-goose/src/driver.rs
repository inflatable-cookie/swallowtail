use crate::{
    ...
    mcp::{GooseAcpRemoteMcpPlacement, production_mcp_servers},
    turn::ActiveTurn,
};
...
const DRIVER_ID: &str = "swallowtail.goose.acp";

/// Low-level interactive driver for the installed Goose ACP agent.
pub struct GooseAcpDriver {
    isolated_environment: EnvironmentRef,
    http_mcp: Option<GooseAcpRemoteMcpPlacement>,
}
...
impl GooseAcpDriver {
    ...
    pub fn with_http_mcp_placement(
        self,
        server: GooseAcpRemoteMcpPlacement,
    ...
    pub(crate) fn with_prepared_http_mcp(
        mut self,
        server: Option<GooseAcpRemoteMcpPlacement>,
    ) -> Self {
    ...
    fn validate_plan(
        &self,
        plan: &PreflightPlan,
    ) -> Result<crate::selection::GoosePlanSelection, RuntimeFailure> {
        if plan.driver_identity().id().as_str() != DRIVER_ID {
            return Err(failure(
                "swallowtail.goose.acp.plan_driver_mismatch",
        ...
        if plan.credential_mechanism() != &CredentialMechanism::LocalUnauthenticated
            ...
            || plan.endpoint_audience().as_str() != crate::GOOSE_LOCAL_ACCOUNT_AUDIENCE
        {
            return Err(failure(
                "swallowtail.goose.acp.access_profile_rejected",
                "Goose ACP requires its host-owned local-config profile",
            ));
        }
        if plan.harness_configuration_posture() != Some(HarnessConfigurationPosture::Ambient) {
            return Err(failure(
                "swallowtail.goose.acp.configuration_posture_rejected",
                "Goose ACP requires explicit ambient configuration inside its selected environment",
            ));
        }
        if plan.requirements().harness_isolation() != Some(HarnessIsolation::AmbientHost) {
            return Err(failure(
                "swallowtail.goose.acp.isolation_rejected",
                "Goose ACP requires explicit ambient-host isolation posture",
            ));
        }
        crate::selection::select_goose_acp_plan(plan)
    }
}

/// Describes the installed Goose ACP discovery and session roles.
#[must_use]
pub fn goose_acp_descriptor() -> DriverDescriptor {
    DriverDescriptor::new(
        ...
        IntegrationFamilyId::new("goose").expect("static family id is valid"),
    ...
    .with_interface_compatibility(crate::goose_acp_claim())
}

impl InteractiveSessionDriver for GooseAcpDriver {
    fn open_session(
        &self,
        plan: PreflightPlan,
...
impl GooseAcpDriver {
    async fn start_session(
        ...
        selected: crate::selection::GoosePlanSelection,
    ) -> Result<GooseSessionHandle, Box<(RuntimeFailure, ResourceLease)>> {
        let cwd = resource
            .filesystem()
        ...
        let runtime_id =
            RuntimeSessionId::new(format!("goose-acp:{}", request.request_id().as_str()))
                .map_err(|_| Box::new((malformed(), resource.clone())))?;
        let active = Arc::new(Mutex::new(None));
        Ok(GooseSessionHandle {

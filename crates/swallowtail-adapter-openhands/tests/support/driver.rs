use crate::{
    OPENHANDS_PACKAGE_VERSION, openhands_agent_server_descriptor,
    openhands_local_config_access_profile, openhands_package_binding,
};
...
pub fn selection(host: ExecutionHostId) -> FixtureSelection {
    ...
    let access_id =
        AccessProfileId::new("openhands.fixture.local-loopback").expect("valid access id");
    let instance_id =
        ConfiguredInstanceId::new("openhands.fixture.instance").expect("valid instance");
    ...
    let version_binding =
        openhands_package_binding(OPENHANDS_PACKAGE_VERSION).expect("fixture version");
    let instance = ConfiguredInstance::new(
        instance_id,
        InstanceRevision::new("fixture-revision").expect("valid revision"),
        AdapterId::new("swallowtail.openhands.agent-server").expect("valid adapter"),
        host.clone(),
        InstanceTargetRef::new("openhands.fixture.interpreter").expect("valid target"),
        InstanceOwnership::HostOwnedEphemeral,
        access_id.clone(),
        SupportAuthority::ProviderSupported,
        ProtocolFacadeId::new("openhands-agent-server-http-ws-v1").expect("valid facade"),
        InstancePolicyId::new("openhands.fixture.isolated").expect("valid policy"),
        capabilities.clone(),
    )
    .with_interface_versions([version_binding.clone()])
    .with_harness_configuration_posture(HarnessConfigurationPosture::Ambient);
    let access = openhands_local_config_access_profile(access_id.clone());
    ...
    let plan = preflight(&context, &requirements)
        .expect("OpenHands Agent Server fixture preflight succeeds");
    FixtureSelection {
        plan,
        resource: WorkingResourceRef::new("openhands.fixture.workspace").expect("valid resource"),
    }
}

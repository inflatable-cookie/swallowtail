...
pub fn selection_with_access(
    ...
    let descriptor = deepagents_acp_descriptor();
    let access_id = AccessProfileId::new("deepagents.fixture.provider-api-key").expect("valid access id");
    let instance_id = ConfiguredInstanceId::new("deepagents.fixture.instance").expect("valid instance");
    ...
    let instance = ConfiguredInstance::new(
        ...
        AdapterId::new("swallowtail.deepagents.acp").expect("valid adapter"),
        host.clone(),
        InstanceTargetRef::new("deepagents.fixture.executable").expect("valid target"),
        InstanceOwnership::HostOwnedEphemeral,
        access_id.clone(),
        SupportAuthority::ProviderSupported,
        ProtocolFacadeId::new("acp-v1").expect("valid facade"),
        InstancePolicyId::new("deepagents.fixture.isolated").expect("valid policy"),
        capabilities.clone(),
    )
    .with_interface_versions([version_binding.clone()])
    .with_harness_configuration_posture(HarnessConfigurationPosture::Ambient);
    let access = deepagents_provider_api_key_access_profile(access_id.clone());
    ...
    let plan = preflight(&context, &requirements).expect("Deep Agents fixture preflight succeeds");
    FixtureSelection {
        plan,
        resource: WorkingResourceRef::new("deepagents.fixture.workspace").expect("valid resource"),
    }
}

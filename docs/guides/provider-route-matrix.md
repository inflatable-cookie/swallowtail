...
| `claude-agent.sdk` | `swallowtail-adapter-claude-agent`; `swallowtail.claude-agent.sdk` | Unix-only fresh read-only interactive session with confirmed effective model, caller-deadline-bounded open, turn, interrupt, and close, streamed output and identity-and-lifecycle activity, consumer-mediated tool admission with intact tool input, capability-gated interrupt, and a root-only degraded close that reports Clean only on attested owned-tree emptiness; strict LF JSONL sidecar wire over stdio to the source-tagged Node sidecar driving the official Claude Agent SDK | approved host launch recipe (interpreted Node plus the sidecar entry point) and environment; maintainer-supported `claude/delegated-subscription-auth` profile with the credential held only by the official store | `claude-agent.sdk.package`; qualified exact `0.3.259` one-point claim plus exact native `2.1.259`, Node `22.23.2`, `swallowtail-claude-agent-sdk-jsonl-v1` wire, and sidecar source-tag axes; no unverified-newer posture | `prepare_claude_agent_sdk_session` → `open_session` → `start_turn`; open verifies wire, behavior, SDK, native binary, Node, leased cwd, read-only tool set, and first-party subscription readiness before any provider work | `ClaudeAgentSdkDriver`; `InteractiveSessionDriver` |

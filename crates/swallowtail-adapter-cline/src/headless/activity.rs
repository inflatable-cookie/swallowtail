...
pub(super) struct ClineHeadlessActivityProjection {
...
impl ClineHeadlessActivityProjection {
    ...
    pub(super) fn tool_start(
        ...
        self.tools.insert(tool_name.to_owned(), activity.clone());
    ...
    pub(super) fn tool_end(
        ...
        let activity = self.tools.remove(tool_name).ok_or_else(activity_drift)?;
        Ok(vec![self.observation(
            &activity,
    ...
    fn open(
        ...
        Ok(OpenActivity {
            id: ActivityId::new(format!("cline-headless:{}", self.next_id))
...
fn activity_drift() -> RuntimeFailure {
    failure(
        "swallowtail.cline.headless.activity_invalid",
        "Cline headless activity did not match the qualified stream",
    )
}

...
pub(in crate::driver) use deadline::complete_before_deadline;
...
pub(super) struct TurnCancellation {
    ...
    remote_uncertain: Arc<AtomicBool>,
    reason: AtomicU8,
}
...
impl TurnCancellation {
    ...
    pub(super) fn timeout(&self) {
            ...
            .is_ok()
        {
            ...
            self.remote_uncertain.store(true, Ordering::SeqCst);
            self.cancelled.store(true, Ordering::SeqCst);
        }
    }

    pub(super) fn fail_remote_uncertain(&self) {
        self.session_usable.store(false, Ordering::SeqCst);
        self.remote_uncertain.store(true, Ordering::SeqCst);
    }

    pub(super) fn reason(&self) -> u8 {
...
impl SessionCancellation {
    pub(super) fn new(active: ActiveSlot, usable: Arc<AtomicBool>) -> Self {
        Self {
            active,
            usable,
...
pub(super) fn cleanup_result(result: Result<(), RuntimeFailure>) -> CleanupOutcome {
    result.map_or_else(

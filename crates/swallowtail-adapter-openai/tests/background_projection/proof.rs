...
/// Returns the exact ledger tuples one prepared background profile claims.
...
fn the_coverage_ledger_dispositions_exactly_the_twenty_four_background_rows() {
    ...
    for entry in &OPENAI_BACKGROUND_TRANCHE {
        assert_eq!(
            entry.route_id, BACKGROUND_ROUTE,
            "{} does not belong to the exact census route",
            entry.semantic_id
        );
        assert!(
            BACKGROUND_OPERATION_SHAPES.contains(&entry.operation_shape),
            "{} names an operation shape outside the openai.background census",
            entry.semantic_id
        );
        assert!(
    ...
    assert_eq!(semantics.len(), 24);
}

#[test]
fn every_prepared_background_profile_emits_exactly_its_ledger_identities() {
...
fn every_emitted_row_carries_the_prepared_structured_run_operation_shape() {
    for (profile, shape) in prepared_operation_shapes() {
        assert_eq!(
            ...
            "{profile} publishes rows under an operation shape the background route never prepares"
        );
    }
}

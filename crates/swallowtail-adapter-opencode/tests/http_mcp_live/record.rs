...
pub enum HttpMcpLiveStop {
    /// Host executable was not exact `1.18.18`.
    HostVersion,
    /// No already-configured model was usable without a login.
    NoUsableModel,
    /// OpenCode required host-owned login during the attempt.
    HostAuthRequired,
...
impl HttpMcpLiveRecord {
    ...
    pub fn from_attempt(
        ...
        let stop = if !connected {
            ...
            failure.diagnostic().code() == "swallowtail.opencode.acp.host_auth_required"
        }) {

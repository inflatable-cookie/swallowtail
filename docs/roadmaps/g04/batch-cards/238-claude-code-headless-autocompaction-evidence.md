...
Freeze exact Claude Code headless autocompaction version, value, precedence,
application, confirmation, and lifecycle truth, then promote Research 237 with
a closed deliver-now table or an honest empty set.
...
- the native option's meaning or version membership is not exact
- ambient configuration can override the caller selection
...
Claude response-only or ACP, Fast, Ultracode, spend caps, advisor, permission
widening, teams, context-size claims, production binding, live provider work,

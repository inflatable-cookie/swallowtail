...
Swallowtail releases are source-only annotated Git tags on canonical `main`.
There is no crates.io publication, GitHub Release object, binary, sidecar,
installer, or model artifact. Versions are coordinated across every package
...
| 6. Merge on both green, with operator approval | Queue | — |
| 7. If the merge SHA is not the green run's SHA and the trees differ, dispatch `CI` with `workflow_dispatch` at the merge SHA | planner | before the tag request |
| 8. Tag request to the operator | planner | immediate on a qualifying green run; do not wait for consumer smoke |
| 9. Local annotated tag and push | planner or a briefed worker, after operator authority naming the exact SHA | — |
| 10. Source-consumer and working-application smoke | consumer-proof task | after the tag |
...
after the last docs-only commit and recorded in the PR closeout comment,
never by another tree mutation. If no qualifying hosted run
...
Do not create, move, or push a tag from a green gate, changelog, or merge.
...
- `git ls-remote --tags origin vX.Y.Z` shows the tag object, and
  `git rev-parse vX.Y.Z^{commit}` equals the authorized SHA.
- `effigy package:source-consumer` resolves the new tag from an isolated
  external Cargo consumer.
- The release note in `docs/releases/` names the tag, tag object and qualifying
  run, and `docs/releases/README.md` lists it as current.

## Roll back

Never move or recreate a published tag. A defective release is superseded by
a new patch version through the same lane; its release note records what it
corrects.

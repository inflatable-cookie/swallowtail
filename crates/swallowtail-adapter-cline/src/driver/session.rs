...
impl InteractiveSessionHandle for ClineSessionHandle {
    ...
    fn start_turn<'a>(
        ...
        Box::pin(async move {
            ...
            let scope = ScopeId::new(format!("cline-acp:turn:{}", request.turn_id().as_str()))
                .map_err(|_| malformed())?;
            ...
            let task_cleanup = match self.pump_task.take() {
                Some(task) => match task.join().await {
                    ...
                    Err(_) => CleanupOutcome::Failed(swallowtail_core::SafeDiagnostic::new(
                        "swallowtail.cline.acp.task_join_failed",
                        "Cline ACP protocol task did not join cleanly",
                    )),
                },
            ...
            let resource_cleanup = match self.resource.take() {
                Some(resource) => match self.services.working_resource() {
                    ...
                    None => CleanupOutcome::Failed(swallowtail_core::SafeDiagnostic::new(
                        "swallowtail.cline.acp.resource_release_failed",
                        "Cline ACP working-resource service disappeared during cleanup",
                    )),
                },
                None => CleanupOutcome::NotApplicable,
            };

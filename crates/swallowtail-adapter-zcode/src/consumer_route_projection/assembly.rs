...
fn a_portable_row_carrying_another_route_applicability_is_rejected() {
    let integration = projection_fixture::prepared("prepared-1");
    let mine = contribution(&profile(PLAN), SHARED_SOURCE);
...
///
/// Each drifted snapshot agrees on route, instance, revision, model, and
/// operation evidence, so the rejection comes from the exact access dimension

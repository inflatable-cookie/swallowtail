...
pub(super) fn profile(
    ...
    let behavior = match observation.compatibility() {
        ...
        InstalledExecutableCompatibility::Incompatible => {
            return Err(failure(
                "swallowtail.cline.headless.preparation.activity_version_incompatible",
                "Cline headless activity requires the exact permitted package",
            ));
        }
    };
    ...
    .map_err(|_| {
        failure(
            "swallowtail.cline.headless.preparation.activity_profile_invalid",
            "Cline headless activity profile could not be derived",
        )
    })
}
...
pub(super) fn with_activity(
    ...
    requirements.push(
        activity
            ...
            .expect("prepared Cline headless activity is available"),
    );

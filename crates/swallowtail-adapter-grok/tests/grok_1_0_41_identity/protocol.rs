//! Frozen mapped-surface ledger for the Grok Build ACP 1.0.41 run.

use super::support::{PROTOCOL, assert_exact_string_set, digest_lines, json, strings};
use std::collections::BTreeMap;

/// Every selected-surface literal the probe searched for, in sorted order so
/// the frozen presence digest is reproducible.
...
const SUBAGENT_HANDOFF_MODULE: &str =
    "crates/codegen/xai-grok-shell/src/session/acp_session_impl/subagent_handoff.rs";

fn per_hop(protocol: &serde_json::Value) -> BTreeMap<String, Vec<String>> {
    protocol["per_hop"]
        .as_object()
        .expect("per hop map")
        .iter()
...
#[test]
fn selected_surface_literals_are_frozen_and_identical_to_the_1_0_40_ceiling() {
    ...
    assert_eq!(
        protocol["baseline_selected_literal_presence_digest"],
        digest.as_str(),
        "the 1.0.41 presence map is byte-identical to the 1.0.40 ceiling"
    );
    assert_eq!(
...
#[test]
fn model_document_stays_byte_identical_into_1_0_41() {
    let protocol = json(PROTOCOL);
    ...
    let hops: Vec<&str> = digests[MODEL_DOCUMENT]
        ...
        .collect();
    assert_eq!(hops, vec!["1.0.40", "1.0.41"]);
...
#[test]
fn acp_module_inventory_keeps_the_mapped_core_and_adds_one_unmapped_module() {
    let protocol = json(PROTOCOL);
    let inventory = &protocol["acp_module_inventory"];
    assert_exact_string_set(&inventory["stable_core"], STABLE_CORE);
    let paths = per_hop(&protocol);
    let baseline = &paths["1.0.40"];
    let hop = &paths["1.0.41"];
    let mut added: Vec<String> = hop
        .iter()
    ...
    let mut removed: Vec<String> = baseline
        .iter()
        .filter(|path| !hop.contains(path))
        .cloned()
        .collect();
    added.sort();
    removed.sort();
    assert_eq!(added, vec![SUBAGENT_HANDOFF_MODULE.to_owned()]);
    assert!(
        removed.is_empty(),
        "no mapped module is removed: {removed:?}"
    );
    assert_exact_string_set(
        &protocol["hop_delta_from_1_0_40"]["added"],
        &[SUBAGENT_HANDOFF_MODULE],
    );
    assert_exact_string_set(&protocol["hop_delta_from_1_0_40"]["removed"], &[]);
    assert_exact_string_set(
        &inventory["per_hop_added"]["1.0.41"],
        &[SUBAGENT_HANDOFF_MODULE],
    );
    assert_exact_string_set(&inventory["per_hop_removed"]["1.0.41"], &[]);
    for (version, entry) in protocol["per_hop"].as_object().unwrap() {
        let hop_paths = &paths[version];
        assert_eq!(
            hop_paths.len(),
            entry["acp_module_path_count"].as_u64().unwrap() as usize
        );
        assert_eq!(
            digest_lines(hop_paths),
            entry["acp_module_path_set_digest"].as_str().unwrap()
        );
        for module in STABLE_CORE {
            assert!(
                hop_paths.iter().any(|path| path == module),
                "{version} lost {module}"
            );
        }
    }
...
fn unmapped_boundaries_and_run_flags_are_frozen() {
    ...
    assert_exact_string_set(&protocol["compared"], &["1.0.40", "1.0.41"]);
    assert_exact_string_set(

...
pub(crate) struct DeepSeekHarnessCancellation {
    process: Arc<dyn ProcessHandle>,
    requested: AtomicBool,
}

impl DeepSeekHarnessCancellation {
...
impl CancellationControl for DeepSeekHarnessCancellation {
...
pub(crate) struct DeepSeekHarnessRunHandle {
...
impl RunHandle for DeepSeekHarnessRunHandle {
        ...
        Box::pin(async move {
            match self.task.join().await {
                ...
                Err(_) => CleanupOutcome::Failed(SafeDiagnostic::new(
                    "swallowtail.deepseek_harness.task_join_failed",
                    "DeepSeek Harness operation task could not be joined",
                )),
            }
        })
    }

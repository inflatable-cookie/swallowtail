---
title: g05.009 Card 067 Candidate J breadth audit worker handoff
...
created: 2026-09-04
updated: 2026-09-04
handoff_path: /Users/tom/Dev/projects/swallowtail/docs/handoffs/20260904-133530-g05-card067-candidate-j-breadth-audit.md
...
The coordinator is dispatching the single approved g05.009 Candidate J breadth-audit lane after operator-confirmed Chatterbox promotion. This is planning-only evidence work; this handoff activates implementation-worker mode.
...
- **Worker branch:** `worker/g05-card067-candidate-j-breadth-audit`
- **Worker worktree:** Paseo-managed worktree with slug `g05-card067-candidate-j-breadth-audit`; use the launcher-provided actual root
...
- **Active spec lane:** Contract 061; Batch 9.4 Candidate J audit
...
- **Ready card:** `/Users/tom/Dev/projects/swallowtail/docs/roadmaps/g05/batch-cards/067-contract-061-candidate-j-breadth-audit.md`
- **Allowed runway:** audit llama.cpp, Ollama (35 census rows) against current `main`; write exactly one new triage note, fill this card's Result, and make zero Rust changes
- **Dispatch topology:** one approved concurrent group: Cards 062, 064, 065, 066, and 067; no shared mutable scope
- **Surfaces this lane owns:** this card; exactly one `docs/triage/YYYYMMDD-HHMMSS-contract-061-candidate-j-audit.md`; append-only `PAPERCUTS.md`
...
- **Review oracle:** the note is evidence, never authority; every row must reconcile exactly to 35, every facade/source identity needs a code reference or absence proof, and rubric closures cannot be by omission
- **Model capability profile:** Deepseek v4 Flash Worker; cheapest adequate strong code-reading audit route
...
- **In scope:** Card 067 only, as defined by the canonical g05.009 Dispatch Manifest.
...
- **Planning lineage:** v0.4.0 is closed; the feature/currentness freeze is lifted. g05.009 Candidate J breadth audits are approved alongside Card 062.
- **Why this card is ready:** operator-confirmed direction from Chatterbox on 2026-09-04 promoted commit `ba0a12b8b9b7dec8862221b2d1b10f533291db00`; the manifest marks Card 067 ready with exact row count 35.
...
Run the worker Completion Protocol preflight before broad reads. Then read `AGENTS.md`, the active milestone, Card 067, Batch 9.4 refs, and Contract 061 from the selected worktree. Reconcile all 35 rows before writing the one triage note.

## Completion Protocol

Use the committed handoff as the sole dispatch artifact. Confirm the launcher-provided worktree is registered, clean, and non-`main`; fetch origin and verify `HEAD == origin/main`, the planning base is an ancestor, and this handoff exists in tracked `HEAD`. Read only the named canonical refs after that preflight. Work only inside the owned paths, preserve zero Rust changes, run the required validation, falsify the note against the review oracle, fill Card 067's Result, push the worker branch, and open a reviewable PR against current `main`. Do not merge. Stop and report if the row count fails to reconcile, current `main` moves under the audit in an owned surface, or a new public/contract decision is required.

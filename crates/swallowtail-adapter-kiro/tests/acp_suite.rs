...
fn success_turn_uses_acp_only_and_joins_cleanup() {
    ...
    let mut session = block_on(driver.open_session(
        ...
        OpenSessionRequest::new(
            RequestId::new("kiro-open").expect("valid request"),
    ...
    let mut turn = block_on(session.start_turn(
        TurnRequest::new(
            RuntimeTurnId::new("kiro-turn").expect("valid turn"),
    ...
    assert_eq!(observed.arguments, ["acp"]);
    for forbidden in [
        "--cloud",
        "--agent",
        "chat",
...
#[test]
fn read_write_plan_still_launches_acp_without_cloud_or_agent() {
    ...
    let mut session = block_on(driver.open_session(
        selected.plan,
        OpenSessionRequest::new(
            RequestId::new("kiro-write-open").expect("valid request"),
            selected.resource,
            None,
            SessionPlanAgreement::explicit(
    ...
    assert_eq!(outcome.status(), &TerminalStatus::Completed);
    assert_eq!(host.observed_process().arguments, ["acp"]);
    assert!(host.writes().iter().any(|message| {
...
fn missing_host_auth_fails_closed_without_login() {
    ...
    let error = match block_on(driver.open_session(
        selected.plan,
        OpenSessionRequest::new(
            RequestId::new("kiro-auth").expect("valid request"),
            selected.resource,
            None,
            SessionPlanAgreement::explicit(
                swallowtail_core::SessionAccessPolicy::ambient_harness(ResourceAccess::Read),
    ...
    assert_eq!(
        ...
        "swallowtail.kiro.acp.host_auth_required"
    );
    assert_eq!(host.releases(), 1);
}
...
fn malformed_and_version_mismatch_fail_before_prompt() {
    for (scenario, suffix) in [
        ...
        let error = match block_on(driver.open_session(
            selected.plan,
            OpenSessionRequest::new(
                RequestId::new(format!("kiro-{suffix}")).expect("valid request"),
                selected.resource,
                None,
                SessionPlanAgreement::explicit(
                    swallowtail_core::SessionAccessPolicy::ambient_harness(ResourceAccess::Read),
...
fn drifted_agent_info_version_fails_closed() {
    ...
    let error = match block_on(driver.open_session(
        selected.plan,
        OpenSessionRequest::new(
            RequestId::new("kiro-agent-version").expect("valid request"),
            selected.resource,
            None,
            SessionPlanAgreement::explicit(
                swallowtail_core::SessionAccessPolicy::ambient_harness(ResourceAccess::Read),
        ...
        Err(error) => error,
        Ok(_) => panic!("agentInfo.version drift must fail closed"),
    };
    assert_eq!(
        error.diagnostic().code(),
...
fn session_deadline_is_rejected_before_spawn() {
    ...
    let error = match block_on(driver.open_session(
        selected.plan,
        OpenSessionRequest::new(
            RequestId::new("kiro-deadline").expect("valid request"),
            selected.resource,
    ...
    let outcome = block_on(
        driver.discover_installed_executable(
            InstalledExecutableDiscoveryRequest::new(
                RequestId::new("kiro-probe").expect("request"),
                ScopeId::new("kiro-probe").expect("scope"),
                host_id.clone(),

...
fn lifecycle_corpus_freezes_every_method_and_behavior_boundary() {
    ...
    assert_eq!(corpus["qualified_range"], "0.80.0..=0.152.1");
...
fn release_evidence_covers_existing_and_lifecycle_milestones() {
    ...
    let expected = [
        ...
        "0.150.0", "0.150.1", "0.151.0", "0.152.0", "0.152.1",
    ];
    assert_eq!(
        releases
            .iter()
...
fn lifecycle_exclusions_preserve_existing_app_server_window() {
    ...
    assert_eq!(exclusions["0.149.2"], "unpublished-stable");
    assert_eq!(exclusions["0.150.2"], "unpublished-stable");
    assert_eq!(exclusions["0.151.1"], "unpublished-stable");
    assert_eq!(corpus["unverified_newer"]["example"], "0.152.2");
    assert_eq!(

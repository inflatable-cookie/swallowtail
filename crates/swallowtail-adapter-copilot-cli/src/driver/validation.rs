fn validate_open(
    ...
    for (present, code, message) in [
        (
            ...
            "swallowtail.copilot-cli.acp.task_service_missing",
            "Copilot CLI ACP requires a scoped task service",
        ),
        (
            services.process().is_some(),
            "swallowtail.copilot-cli.acp.process_service_missing",
            "Copilot CLI ACP requires a process service",
        ),
        (
            services.working_resource().is_some(),
            "swallowtail.copilot-cli.acp.resource_service_missing",
            "Copilot CLI ACP requires a working-resource service",
        ),
    ] {
        if !present {
    ...
    if request.access_policy() != &SessionAccessPolicy::ambient_harness(resource_access) {
        return Err(failure(
            "swallowtail.copilot-cli.acp.access_policy_rejected",
            "Copilot CLI ACP requires exact ambient working-resource access",
        ));
    }
...
fn validate_initialize(
    ...
    // Initialize result fields are unrecovered from public Copilot source.
    // Do not invent agentInfo. Fail closed if a present version drifts.
    if let Some(info) = response.get("agentInfo") {
        ...
        {
            return Err(failure(
                "swallowtail.copilot-cli.acp.agent_version_rejected",
                "Copilot CLI ACP identity does not match the preflight version",
            ));
        }
    }
    Ok(())
...
fn session_resource_access(plan: &PreflightPlan) -> Result<ResourceAccess, RuntimeFailure> {
    plan.requirements()
        ...
        .ok_or_else(|| {
            failure(
                "swallowtail.copilot-cli.acp.resource_access_missing",
                "Copilot CLI ACP requires explicit working-resource access",
            )
        })
}

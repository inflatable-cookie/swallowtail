...
pub(crate) struct ZcodeActivityProjection {
...
impl ZcodeActivityProjection {
    ...
    pub(crate) fn tool_started(
        ...
        let activity_id = ActivityId::new(format!("zcode:tool:{}", self.next_id))
    ...
    pub(crate) fn unknown(
        ...
        self.next_id = self.next_id.checked_add(1).ok_or_else(activity_drift)?;
        let namespaced = event_type.replace('/', ".");
        let namespace = ActivityNamespace::new(format!("zcode.event.{namespaced}"))
            .or_else(|_| ActivityNamespace::new("zcode.event.unknown"))
            .map_err(|_| activity_drift())?;
        observation(
            ActivityId::new(format!("zcode:unknown:{}", self.next_id))
                .map_err(|_| activity_drift())?,
            self.operation_id.clone(),
            ProviderActivityRef::new(namespaced).map_err(|_| activity_drift())?,
    ...
    fn assistant_observation(
        ...
        observation(
            ActivityId::new("zcode:assistant").map_err(|_| activity_drift())?,
            self.operation_id.clone(),
...
fn activity_drift() -> RuntimeFailure {
    failure(
        "swallowtail.zcode.app_server.activity_invalid",
        "ZCode activity did not match the qualified app-server stream",
    )
}

...
impl CurlTransport {
    pub(crate) async fn request(
        ...
        let blocking = services.blocking_work().cloned().ok_or_else(|| {
            failure(
                "swallowtail.kimi_platform.blocking_service_missing",
                "Kimi Platform HTTP requires a blocking-work service",
            )
        })?;
        ...
        receiver.await.map_err(|_| {
            failure(
                "swallowtail.kimi_platform.blocking_result_missing",
                "Kimi Platform blocking HTTP work returned no result",
            )
        })?
    }
    ...
    pub(crate) fn subscribe(
        ...
        let blocking = services.blocking_work().cloned().ok_or_else(|| {
            failure(
                "swallowtail.kimi_platform.blocking_service_missing",
                "Kimi Platform SSE requires a blocking-work service",
            )
        })?;
        let url = request_url(&endpoint, &request)?;
        ...
        let work = blocking.run(
            scope,
            Box::new(move || perform_sse(url, credential, request, sender, job_cancelled)),
        );
        Ok(Subscription {
            receiver,
            cancelled,

...
use super::ledger::DEEPAGENTS_ROUTE;
...
/// Returns the census semantic id of one published `deepagents.acp` row.
///
/// An identity outside the tranche panics rather than being folded into a
/// neighbouring row, so a borrowed Kiro, Qoder, or ZCode identity cannot pass.
pub(super) fn semantic_id(identity: &ConsumerRouteRowIdentity) -> String {
    assert!(
        identity.namespaced_extension().is_none(),
        "deepagents.acp publishes no bounded route-local descriptor: {identity:?}"
    );
    match identity {
        ConsumerRouteRowIdentity::Feature(feature) => match feature {
            ...
            other => panic!("unexpected deepagents.acp projection feature {other:?}"),
        },
        ConsumerRouteRowIdentity::Control(control) => {
            panic!("deepagents.acp publishes no selectable control: {control:?}")
        }
    }
    .to_owned()
}

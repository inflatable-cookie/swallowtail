...
#[test]
fn exact_copilot_thought_classification_and_tools_remain_distinct() {
    let mut projection =
        AcpActivityProjection::new(RuntimeTurnId::new("copilot-cli-activity-turn").unwrap());
    let updates = [
        json!({"sessionUpdate":"agent_thought_chunk","content":{"type":"text","text":"Inspecting"}}),
        json!({"sessionUpdate":"agent_message_chunk","content":{"type":"text","text":"Copilot display text."}}),
        ...
        json!({"sessionUpdate":"copilot_future_activity","private":"excluded"}),
...
fn noncanonical_tool_lifecycles_degrade_without_failure() {
    let mut projection =
        AcpActivityProjection::new(RuntimeTurnId::new("copilot-cli-tool-lifecycle-turn").unwrap());
    let updates = [

use super::QwenPreparationInput;
use super::failure::preparation_failure;
use crate::qwen_headless_descriptor;
    ...
    let target =
        InstanceTargetRef::new(input.target.executable().as_host_value()).map_err(|_| {
            preparation_failure(
                ...
                "swallowtail.qwen.preparation.target_invalid",
                "Qwen approved target could not be bound to the configured instance",
            )
        })?;
    Ok(ConfiguredInstance::new(
        input.instance_id.clone(),
        input.instance_revision.clone(),
        qwen_headless_descriptor().identity().id().clone(),
        ...
        ProtocolFacadeId::new("qwen-headless-stream-json-v1").expect("static Qwen facade is valid"),
        InstancePolicyId::new("qwen-prepared-ambient-read").expect("static Qwen policy is valid"),
        run_capabilities(),
    )

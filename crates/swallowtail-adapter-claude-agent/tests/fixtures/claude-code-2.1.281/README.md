# Claude Code 2.1.281 qualification stop

Official npm wrapper and `darwin-arm64` / `linux-x64` platform packages for
`2.1.278`, `2.1.280`, and `2.1.281` were downloaded under `/tmp`, hashed,
and read. The binaries were not executed. `identity.json` records channel
identity. `dist-inventory.json` records every shipped file and each hop's
exact file delta. `builtin-hook-ledger.json` records the built-in plugin
registration set, hook events, effects, and embedded Linux module hashes;
both platform binaries corroborate registration. `protocol.json` freezes the
response-only decision inputs, including the `instructionFiles` and telemetry
opt-out parser results and the v2 launch-directory rule.

Research 341 froze the stop. Research 342 records the claim under the
narrowed built-in-hook guarantee.

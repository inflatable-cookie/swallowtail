...
- [2026-09-03 Claude Agent SDK 0.3.259 Identity](./2026-09-03-claude-agent-sdk-0-3-259-identity.md) — card 055 refresh exit; official `0.3.259` and coupled native `2.1.259` frozen from official artifacts without executing them; 15-file tree inventory with 7 identical and 8 changed files; `permissionPrompts`, `user_message_uuids`, task summary, managed MCP, and bridge/browser deltas classified unmapped; credential non-custody and `harnessSchema` unchanged; exact package and native axes rebound with no behavior revision
...
- [2026-09-02 Claude Agent SDK Foundation](./2026-09-02-claude-agent-sdk-foundation.md) — card 055; provider-free `claude-agent.sdk` route on exact SDK `0.3.259` and native `2.1.259`; policy and npm stable rechecked unchanged; credential non-custody mechanically checked; every public operation is caller-bounded through its return, and cleanup reports `Clean` only on attested `OwnedTreeEmpty`, `Degraded` on confirmed root-only completion, and `Failed` on an observed survivor or unconfirmed root exit

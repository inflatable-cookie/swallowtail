...
pub(super) struct OpenHandsActivityProjection {
...
impl OpenHandsActivityProjection {
    ...
    fn open(&mut self) -> Result<OpenActivity, RuntimeFailure> {
        ...
        Ok(OpenActivity {
            id: ActivityId::new(format!("openhands-agent-server:{}", self.next_id))
...
fn activity_drift() -> RuntimeFailure {
    failure(
        "swallowtail.openhands.agent_server.activity_invalid",
        "OpenHands Agent Server activity did not match the qualified event stream",
    )
}

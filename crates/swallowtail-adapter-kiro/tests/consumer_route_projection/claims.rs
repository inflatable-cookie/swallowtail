...
fn the_no_route_specific_control_audit_produces_no_public_control_descriptor() {
    let published = contribution(&session(), "kiro.acp.audit");
    assert_eq!(
        published.session_start_rows().len(),
        0,
        "kiro.acp admits no session-start control"
    );
...
#[test]
fn prepared_kiro_rows_claim_no_acknowledgement_or_per_turn_authority() {
    let published = contribution(&session(), "kiro.acp.authority");
    let named = published
        .sources()
    ...
    for forbidden in [
        "/fixture/bin/",
        "kiro.projection.isolated",
        "kiro.projection.workspace",
    ] {
        assert!(
...
fn a_changed_source_id_replaces_the_snapshot_rather_than_merging_into_it() {
    let session = session();
    let first = contribution(&session, "kiro.acp.first");
    let second = contribution(&session, "kiro.acp.second");
    assert_eq!(
        rows(&first),
        rows(&second),
    ...
    for row in all_rows(&second) {
        assert_eq!(row.source().id().as_str(), "kiro.acp.second");
    }
}
...
fn a_row_from_another_prepared_revision_is_rejected_at_admission() {
    let alternate = contribution(&alternate_revision(), "kiro.acp.alternate");
    let borrowed = alternate
        .selection_rows()
        .next()
        .expect("the alternate session publishes rows")
        .clone();
    let mine = contribution(&session(), "kiro.acp.local");
    assert_ne!(
...
/// Proves a drifted access dimension yields no `kiro.acp` row at all.
#[test]
fn a_drifted_access_dimension_publishes_no_kiro_row() {
...
fn the_admitted_snapshot_keeps_each_access_dimension_observable() {
    let admitted = contribution(&session(), "kiro.acp.dimensions");

...
pub const ANTIGRAVITY_LATEST_QUALIFIED_VERSION: &str = "1.1.24";
...
mod tests {
    ...
    fn exact_installed_release_is_qualified_and_newer_is_visible() {
        ...
        assert!(claim.supports(&version("1.1.18")));
        assert!(claim.supports(&version("1.1.24")));
        assert!(!claim.permits(&version("1.1.8")));
        let InterfaceCompatibilityAssessment::UnverifiedNewer(newer) =
            claim.assess(&version("1.1.25"))
        else {
    ...
    fn binding_accepts_only_bare_stable_semver() {
        ...
        assert!(antigravity_release_binding("1.1.24").is_some());
        for rejected in [
            "",
            " 1.1.9",
            "agy 1.1.9",

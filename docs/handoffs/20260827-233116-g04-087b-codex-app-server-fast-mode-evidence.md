---
title: g04.087b Codex app-server Fast-mode evidence worker handoff
...
created: 2026-08-27
updated: 2026-08-27
handoff_path: /Users/tom/Dev/projects/swallowtail/docs/handoffs/20260827-233116-g04-087b-codex-app-server-fast-mode-evidence.md
...
This lane owns Codex app-server Fast-mode evidence: card 245, Research 242, its
...
Codex core has Fast/service-tier machinery, but exec evidence stopped because
live catalogue substitution and silent downgrade prevented binding. App-server
has different configuration, returned-state, persistence, and billing seams
that require their own exact qualification.
...
- **Planning artifacts:** g04.087, card 245, Research 242 reservation, lane log,
  indexes, and sole Next Task
- **Worker branch:** `worker/g04-087b-codex-app-server-fast-mode-evidence`
- **Worker worktree:** `/Users/tom/Dev/worktrees/swallowtail-g04-087b-codex-app-server-fast-mode-evidence`
- **Worktree creation command:** `git worktree add -b worker/g04-087b-codex-app-server-fast-mode-evidence /Users/tom/Dev/worktrees/swallowtail-g04-087b-codex-app-server-fast-mode-evidence origin/main`
...
- **Active lane:** per-route feature completion, g04.087 lane B
- **Roadmap:** `docs/roadmaps/g04/087-fourth-parallel-per-route-feature-qualification.md`
- **Ready card:** `docs/roadmaps/g04/batch-cards/245-codex-app-server-fast-mode-evidence.md`
- **Research:** `docs/research/242-codex-app-server-fast-mode-evidence.md`
- **Lane log:** `docs/logs/2026-08-27-g04-087b-codex-app-server-fast-mode-evidence.md`
...
- **Canonical refs:** `docs/architecture/system-architecture.md`; Contracts
  006, 014, 020, 029, 034, 037, 040, 047, 052; Research 201, 229, 234, and
  238; `docs/guides/codex-prepared-integration.md`
...
- **Tool/runtime restrictions:** official docs and exact tagged source plus
  ...
  account/live-catalogue inspection, provider prompt, paid work, or host mutation
...
- **In scope:** card 245 exactly; app-server versions, model membership, access,
  feature gate, tier id, configuration/thread/turn scope, request,
  confirmation, billing, persistence, restoration, omission, Research 242, and
  the assigned log.
- **Allowed changed files:** assigned card, Research 242, assigned log, and new
  Codex-local frozen evidence under a unique fixture/evidence path.
- **Out of scope:** production code/API, Codex exec, personality, verbosity,
  Plan effort, multi-agent, search, shared milestone/inventory/programme/triage/
  matrices/indexes/Next Task, currentness, live provider work, release, merge,
  rollover, or g04 closure.
- Do not promote exec argv or its empty-set conclusion. Keep ChatGPT credit and
  API Priority billing separate.
...
- Research 234 freezes exact `0.149.1` core Fast/tier machinery and the exec
  stop. It is a source map and sibling-route contrast only.
- App-server support is maintained from `0.110.0..=0.149.1`; feature evidence
  may be narrower and must name exact tags/models/access profiles.
- A non-empty row needs closed model membership, no silent downgrade,
  pre-effect selection, returned/effective confirmation, billing semantics,
  load/resume/replacement behavior, and unchanged omission.
- Report after Research 242 and card 245 are complete, or earlier on a stop.
- Report to the operator, who relays the PR to the orchestrator.

## Suggested Next Move

Run the startup preflight. Trace Fast from exact tagged config/protocol schemas
through app-server thread/turn request construction, returned state, and
persistence before constructing the closed table.
...
5. Read `AGENTS.md`, g04.087, card 245, Research 242, the lane log, and named
   canonical refs. Then run the cheap repo orientation checks.

### While you work

- Execute only card 245 and edit only allowed lane files.
...
2. Complete card 245, Research 242, and the assigned log honestly.
3. Push the worker branch and open a PR against current pushed `main`.
4. Link g04.087, card 245, Research 242, changed evidence, validation, and
...
`main` after lane A lands. An evidence-backed PR comment is canonical if

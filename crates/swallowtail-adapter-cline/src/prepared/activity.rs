...
pub(super) fn profile(
    ...
    let behavior = match observation.compatibility() {
        ...
        InstalledExecutableCompatibility::Incompatible => {
            return Err(failure(
                "swallowtail.cline.acp.preparation.activity_version_incompatible",
                "Cline ACP activity requires the exact permitted package",
            ));
        }
    };
    ...
    .map_err(|_| {
        failure(
            "swallowtail.cline.acp.preparation.activity_profile_invalid",
            "Cline ACP activity profile could not be derived",
        )
    })
}
...
pub(super) fn with_activity(
    ...
    requirements.push(
        activity
            ...
            .expect("prepared Cline activity is available"),
    );

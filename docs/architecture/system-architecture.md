...
- opt-in debug observation is a separate host sink, not a second public event
  ...
  operation
- operation-scoped watchers are realized through provider-neutral core records,
  one optional runtime `WatcherHostService`, and `LocalWatcherHostService` in
...
- `swallowtail-adapter-opencode` implements version-bound OpenCode
  `1.14.48..=1.18.28` model discovery and ambient-host interactive sessions with
...
Tagged OpenAPI and artifact-tree evidence for 69 stable releases from `1.14.48`
through `1.18.28` closes six selected operations through every transitive local schema
...
release. Stable exact releases above `1.18.28` may execute as unverified through

...
use swallowtail_adapter_cline::{
    CLINE_PACKAGE_VERSION, cline_headless_descriptor, cline_local_account_access_profile,
    cline_package_binding,
};
...
pub fn selection(host: ExecutionHostId) -> FixtureSelection {
    ...
    let instance = ConfiguredInstance::new(
        ...
        AdapterId::new("swallowtail.cline.headless").expect("valid adapter"),
        host.clone(),
        InstanceTargetRef::new("cline.fixture.executable").expect("valid target"),
        InstanceOwnership::HostOwnedEphemeral,
        access_id.clone(),
        SupportAuthority::ProviderSupported,
        ProtocolFacadeId::new("cline-json-v1").expect("valid facade"),
    ...
    let plan =
        preflight(&context, &requirements).expect("Cline headless fixture preflight succeeds");
    FixtureSelection {
        plan,

...
pub(super) fn validate(
    ...
    {
        return Err(plan_mismatch("instance, local loopback, or model route"));
    }
...
fn require_service(
        ...
        .any(|required| required == service)
    {
        ...
        Err(crate::failure::failure(
            ...
            format!("OpenHands Agent Server requires the preflight-bound {name} service"),
        ))
    } else {
        Ok(())
    }
...
fn plan_mismatch(dimension: &str) -> RuntimeFailure {
    crate::failure::failure(
        "swallowtail.openhands.agent_server.request_plan_mismatch",
        format!("OpenHands Agent Server request does not match its preflight-bound {dimension}"),
    )
}

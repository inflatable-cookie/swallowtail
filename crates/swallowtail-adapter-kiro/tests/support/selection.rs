...
pub fn selection_with_access(
    ...
    let descriptor = kiro_acp_descriptor();
    let access_id = AccessProfileId::new("kiro.fixture.local-account").expect("valid access id");
    let instance_id = ConfiguredInstanceId::new("kiro.fixture.instance").expect("valid instance");
    ...
    let instance = ConfiguredInstance::new(
        ...
        AdapterId::new("swallowtail.kiro.acp").expect("valid adapter"),
        host.clone(),
        InstanceTargetRef::new("kiro.fixture.executable").expect("valid target"),
        InstanceOwnership::HostOwnedEphemeral,
        access_id.clone(),
        SupportAuthority::ProviderSupported,
        ProtocolFacadeId::new("acp-v1").expect("valid facade"),
        InstancePolicyId::new("kiro.fixture.isolated").expect("valid policy"),
        capabilities.clone(),
    )
    .with_interface_versions([version_binding.clone()])
    .with_harness_configuration_posture(HarnessConfigurationPosture::Ambient);
    let access = kiro_local_account_access_profile(access_id.clone());
    ...
    let plan = preflight(&context, &requirements).expect("Kiro fixture preflight succeeds");
    FixtureSelection {
        plan,
        resource: WorkingResourceRef::new("kiro.fixture.workspace").expect("valid resource"),
    }
}

use super::failure::{failure, malformed};
...
pub(crate) struct ActiveTurn {
    ...
    sequence: AtomicU64,
    output: Mutex<String>,
    activity: Mutex<crate::acp::activity::AcpActivityProjection>,
...
impl ActiveTurn {
    pub(crate) fn new(
        ...
        Ok((
            Arc::new(Self {
                ...
                activity: Mutex::new(crate::acp::activity::AcpActivityProjection::new(runtime_id)),
            ...
            match stop_reason {
                ...
                "max_tokens" => {
                    TerminalStatus::ProviderFailed(swallowtail_core::SafeDiagnostic::new(
                        "swallowtail.opencode.acp.prompt_stopped",
                        "OpenCode stopped before completing the turn",
                    ))
                }
                _ => TerminalStatus::RuntimeFailed(
                    failure(
                        "swallowtail.opencode.acp.stop_reason_unsupported",
                        "OpenCode returned an unsupported ACP stop reason",
                    )
                    .diagnostic()
                    .clone(),
                ),
    ...
    fn append_output(&self, text: &str) -> Result<(), RuntimeFailure> {
        ...
        if output.len().saturating_add(text.len()) > MAXIMUM_OUTPUT_BYTES {
            return Err(failure(
                "swallowtail.opencode.acp.output_limit_exceeded",
                "OpenCode output exceeded the adapter limit",
            ));
        }
    ...
    fn verify_session(&self, session_id: &str) -> Result<(), RuntimeFailure> {
        if session_id == self.session_id {
            ...
            Err(failure(
                ...
                "OpenCode update does not match the active session",
            ))
        }
    }
}

//! Frozen shipped-file inventory and per-hop deltas for Grok Build 1.0.30.
    ...
    for package in ["wrapper", "platform"] {
        ...
        for version in &compared {
            ...
            assert_eq!(
                ...
                inventory["per_hop_delta_from_1_0_5"][*version][package],
...
fn the_native_payload_is_the_only_changed_platform_file() {
    let inventory = json(DIST_INVENTORY);
    for (version, entry) in inventory["per_hop_delta_from_1_0_5"].as_object().unwrap() {
        ...
        if [
            "1.0.14", "1.0.15", "1.0.16", "1.0.17", "1.0.18", "1.0.19", "1.0.20", "1.0.21",
        ...
        .contains(&version.as_str())
        {
            assert_eq!(
                wrapper,
                vec!["bin/grok", "bin/postinstall.js", "package.json"],
                "{version} launcher refactor"
            );
            assert_eq!(added, vec!["bin/grok-bootstrap.js"], "{version}");
        } else {
            assert_eq!(
                wrapper,
                vec!["package.json"],
...
fn identical_files_are_byte_identical_across_every_hop_and_native_payloads_are_distinct() {
    ...
    assert_eq!(
        unique.len(),
        2,
        "the launcher split at 1.0.14 is the only launcher change"
    );
}

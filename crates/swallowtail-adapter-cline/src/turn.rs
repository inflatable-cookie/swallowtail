...
impl ActiveTurn {
            ...
            .clone()
        {
            ...
            match stop_reason {
                ...
                "max_tokens" | "max_turn_requests" | "refusal" => {
                    TerminalStatus::ProviderFailed(swallowtail_core::SafeDiagnostic::new(
                        "swallowtail.cline.acp.prompt_stopped",
                        "Cline stopped before completing the turn",
                    ))
                }
                _ => TerminalStatus::RuntimeFailed(
                    failure(
                        "swallowtail.cline.acp.stop_reason_unsupported",
                        "Cline returned an unsupported ACP stop reason",
                    )
                    .diagnostic()
                    .clone(),
                ),
    ...
    fn append_output(&self, text: &str) -> Result<(), RuntimeFailure> {
        ...
        if output.len().saturating_add(text.len()) > MAXIMUM_OUTPUT_BYTES {
            return Err(failure(
                "swallowtail.cline.acp.output_limit_exceeded",
                "Cline output exceeded the adapter limit",
            ));
        }
    ...
    fn verify_session(&self, session_id: &str) -> Result<(), RuntimeFailure> {
        if session_id == self.session_id {
            ...
            Err(failure(
                ...
                "Cline update does not match the active session",
            ))
        }
    }
}

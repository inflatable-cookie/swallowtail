...
fn catalogue_prepares_on_exact_1_0_25_and_lists_the_accepted_document() {
    ...
    let catalogue = prepared
        .prepare_catalogue(GrokCatalogueProfileInput::new(request_id(
            "catalogue.accept",
        )))
    ...
    let models = block_on(catalogue.list_models(services(host_id.clone(), process)))
    ...
    assert_eq!(observed.arguments, ["models"]);
...
#[test]
fn malformed_output_fails_closed_after_join() {
...
fn non_zero_exit_reports_status_without_stderr_secrets() {
    ...
    let catalogue = prepared
        .prepare_catalogue(GrokCatalogueProfileInput::new(request_id("catalogue.exit")))
        .expect("catalogue prepares");
...
fn elapsed_deadline_rejects_before_spawn() {
    ...
    let catalogue = prepared
        .prepare_catalogue(
            GrokCatalogueProfileInput::new(request_id("catalogue.elapsed"))
...
fn deadline_during_read_times_out_with_joined_cleanup() {
    ...
    let catalogue = prepared
        .prepare_catalogue(
            GrokCatalogueProfileInput::new(request_id("catalogue.timeout"))
...
fn host_mismatch_fails_before_spawn() {
    ...
    let catalogue = prepared
        .prepare_catalogue(GrokCatalogueProfileInput::new(request_id("catalogue.host")))
        .expect("catalogue prepares");

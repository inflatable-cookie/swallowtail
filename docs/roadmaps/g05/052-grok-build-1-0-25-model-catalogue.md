...
| Owned mutable paths | `crates/swallowtail-adapter-grok/src/**`; Grok tests and new exact `1.0.25` fixtures under `crates/swallowtail-adapter-grok/tests/**`; `docs/research/305-*` and its index line; `docs/research/290-feature-matrix-cross-evidence.tsv`; `docs/guides/grok-build-prepared-integration.md`; exact Grok rows/cells in `docs/guides/provider-route-matrix.md`, `provider-solution-feature-matrix.csv`, and `provider-solution-activity-matrix.csv`; `docs/guides/integration-guide-map.md`; `CHANGELOG.md` `[Unreleased]`; one Grok claim log and its index line; the current-role `release-baselines/production-routes-0.5.0.txt` and `release-baselines/public-api-0.5.0/swallowtail-adapter-grok.txt`; exact current-versus-tagged inventory corrections in `scripts/check-consumer-front-door.py`, `scripts/check-provider-route-matrix.sh`, `scripts/check-provider-activity-matrix.py`, and `scripts/provider_route_matrix/**`; exact provider-wide activity fixture/test updates; this task result/status; `PAPERCUTS.md` append only |
...
| Forbidden paths | Desktop; `grok_build_acp_claim` execution segments except tests proving immutability; other adapters; tagged `docs/releases/0.5.0.md`; historical release baselines; Research 281 and every other historical freeze ledger; tags; publications; host credentials or config; any live-provider evidence |
...
post-`v0.5.0` current-source production route. Do not reduce it into the ACP
route and do not add it to tagged `v0.5.0` release notes.

The retained worker must:

1. preserve the current-role `0.5.0` API and route baseline additions allowed
   between tags;
2. revert every change to Research 281's frozen 49-route historical ledger;
3. leave `docs/releases/0.5.0.md` byte-identical;
4. repair the two gates so current source/baseline truth is 50 routes while the
   tagged `v0.5.0` and Research 281 evidence remains 49 routes;
5. keep Card 050/051 wording and membership checks scoped to their frozen
   historical rows rather than mechanically rewriting 49 to 50; and
6. add deterministic regression coverage that fails if current and tagged
   inventories are collapsed again.

This is a Chatterbox ruling from existing release authority, not a freeze
exception or release mutation. No provider, tag, publication, consumer pin or
Desktop authority is added.

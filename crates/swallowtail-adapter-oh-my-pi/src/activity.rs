...
pub(crate) struct OhMyPiActivityProjection {
...
impl OhMyPiActivityProjection {
    ...
    pub(crate) fn project(
        ...
        match event {
            ...
            OhMyPiAgentEvent::OutputDelta(_) => {
            ...
            OhMyPiAgentEvent::MessageEnded(_) => {
            ...
            OhMyPiAgentEvent::ToolStarted { call_id, name } => {
            ...
            OhMyPiAgentEvent::ToolUpdated { call_id, name } => {
            ...
            OhMyPiAgentEvent::Unknown(namespace) => {
                let activity = self.open(
                    "unknown",
                    None,
                    ActivityKind::Unknown(
                        ActivityNamespace::new(format!("oh-my-pi.rpc.{namespace}"))
            ...
            OhMyPiAgentEvent::RetryObserved => {
                let activity = self.open(
                    ...
                    None,
                    ActivityKind::Unknown(
                        ActivityNamespace::new("oh-my-pi.rpc.retry")
                            .map_err(|_| activity_drift())?,
                    ),
                    None,
                    ActivityDisclosure::IdentityAndLifecycleOnly,
            ...
            OhMyPiAgentEvent::Started
            | OhMyPiAgentEvent::Settled
            | OhMyPiAgentEvent::Progress
            | OhMyPiAgentEvent::ProviderFailed => Ok(Vec::new()),
        }
    }
    ...
    fn open(
        ...
        Ok(OpenActivity {
            id: ActivityId::new(format!("oh-my-pi-rpc:{label}:{}", self.next_id))
                .map_err(|_| activity_drift())?,
...
fn activity_drift() -> RuntimeFailure {
    failure(
        "swallowtail.oh_my_pi.rpc.activity_invalid",
        "OhMyPi RPC observable activity did not match the qualified protocol",
    )
}

...
fn provisional_newer_binds_init_and_exposes_version_diagnostics() {
    ...
    let prepared = prepared_at(host.clone(), "2.1.258", Some(Arc::clone(&observer)));
    assert!(matches!(
        prepared.observation().compatibility(),
    ...
    let output = response_fixture("response-complete.jsonl").replacen("2.1.228", "2.1.258", 1);
    ...
    for stage in [
        ...
        assert!(observations.iter().any(|observation| {
            observation.kind() == DebugObservationKind::InterfaceVersion
                ...
                && observation.detail()
                    == "observed_version=2.1.258; compatibility=unverified-newer"
        }));
    }
}

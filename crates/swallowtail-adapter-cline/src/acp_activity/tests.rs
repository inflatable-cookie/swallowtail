...
#[test]
fn exact_cline_thought_classification_and_tools_remain_distinct() {
    let mut projection =
        AcpActivityProjection::new(RuntimeTurnId::new("cline-activity-turn").unwrap());
    let updates = [
        ...
        json!({"sessionUpdate":"future_cline_activity","private":"excluded"}),
    ];
...
fn noncanonical_tool_lifecycles_degrade_without_failure() {
    let mut projection =
        AcpActivityProjection::new(RuntimeTurnId::new("cline-tool-lifecycle-turn").unwrap());
    let updates = [

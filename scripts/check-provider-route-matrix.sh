...
if [ "$(wc -l < "$route_matrix_actual" | tr -d ' ')" -ne 47 ]; then
  printf 'provider route matrix must contain exactly 47 route rows\n' >&2
  exit 1
fi
...
if [ "$(wc -l < "$route_lifecycle_actual" | tr -d ' ')" -ne 47 ]; then
  printf 'provider session lifecycle matrix must contain exactly 47 route rows\n' >&2
  exit 1
fi
...
printf 'provider route, lifecycle, 39-solution/47-route feature, and activity matrices passed\n'

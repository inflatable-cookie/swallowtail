...
Status: ready; 425 rows proved; card 074 is complete, card 075 remains active, card 076 is complete; card 079 is the shared baseline before card 034
Owner: Tom
Created: 2026-08-31
Updated: 2026-09-04
...
Contract 061 settles the semantic projection boundary, and the
...
Realize one cohesive route-feature and lifecycle-scoped control projection
without changing Contracts 037, 047, or 057, reversing package dependencies,
enumerating adapters at runtime, or widening descriptive truth into execution
...
- changing `swallowtail-core` vocabulary or adding a projection-lifecycle
  dependency to core
- an umbrella crate, adapter registry, runtime route enumeration, adapter
  downcasts, callbacks into adapter code, or generic provider payloads
- caller-supplied projection limits or limits derived from the global 767-row
  census
- selection-summary breadth before the two-route vertical
- a one-route first proof or a general mid-turn negotiation claim
- amendments to Contracts 037, 047, 057, or 061
- consumer UI, routing, defaults, persistence, execution authority, or provider
  mutation authority
- provider contact, compatibility/currentness work, watcher or skill-visibility
...
- [ ] reconcile adapter-local coverage with all 767 census rows across 48 route
      IDs and 31 adapter packages
- [ ] require every row to have one exact implemented or explicit-absence
      disposition
- [ ] prove the 9 no-control routes, 9 per-turn rows, and 5 exact
      acknowledgement rows on 4 routes retain their distinct truth
- [ ] close only after docs, semantic API, package, consumer, and all-route
      evidence named by the accepting cards pass
...
- Stop before implementation if an exact public type, signature, source
  identity, numeric maximum, failure point, or fixture remains undecided.
- Stop if the shared composer needs adapter dependencies, a registry, route
  enumeration, generic provider data, or a new core dependency.
- Stop if a fixed bound cannot be justified from per-route evidence plus named
...
- immediate state: 425 rows proved; card 074 is complete, card 075 remains
  active, and card 076's Kimi acknowledgement gate is complete
- completed shared baseline: Contract 061 provider-operation observation plus
  card 073 runtime/testkit support
- later runway: 342 rows remain; card 075 reconciles the remaining 56 rows in
  this approved package frontier

...
pub(crate) fn activity_profile(
    ...
    let behavior_revision = match prepared.observation().compatibility() {
        ...
        InstalledExecutableCompatibility::Incompatible => {
            return Err(failure(
                "swallowtail.qwen.preparation.activity_version_incompatible",
                "Qwen headless activity requires a permitted executable version",
            ));
        }
    };
    ...
    .map_err(|_| {
        failure(
            "swallowtail.qwen.preparation.activity_profile_invalid",
            "Qwen headless activity profile could not be derived",
        )
    })
}
...
pub(crate) fn with_activity(
    ...
    requirements.push(
        activity
            ...
            .expect("prepared Qwen activity is available"),
    );

...
The spent run printed model `kimi-for-coding/k3` from a definition-exists
check. That is not verified exact-model attribution: this route does not
send a model on `session/new` or `session/prompt`, `negotiated_model_options`
is none, and a selected default agent can override the root `model`. The
authorized attempt has no verified exact model string.
...
has no verified exact model attribution. Future live records use the
agent-override-aware host default, not an ACP `current_value`. Honouring of
the HTTP MCP entry on exact `1.18.18` still stands.

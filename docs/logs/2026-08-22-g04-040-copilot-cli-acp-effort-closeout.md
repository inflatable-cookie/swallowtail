...
Worker implementation head: `b40637ec`
Reviewed head: `9c5836043ed7a8207206bcb6f9aa696678dfb6f2`
PR integration tip: `da0871d5241a85460cc0f98f89eace5244785752`
Merge: GitHub rebase/linear integration; local `main` fast-forwarded; no merge
commit, 2026-08-22
...
The orchestrator applied the deferred delta after merge without presenting this
family as a shipped reasoning control:

- `docs/architecture/system-architecture.md`: records that Copilot CLI ACP
...
- `docs/guides/provider-route-matrix.md` and
  ...
  `copilot-cli.acp` `reasoning_selection` as `No` and now state the exact
  model-entitlement/default-substitution reason.
...
- `docs/roadmaps/README.md` and `docs/roadmaps/g04/README.md`: move Next Task
  to g04.041 Qwen headless reasoning-effort evidence compilation.
...
Orchestrator review reproduced the npm and extracted `app.js` SHA-256 digests,
confirmed the exact model-entitlement/default-substitution source path, and
accepted reviewed head `9c583604` after all five CI jobs passed. Shared closeout
then passed `effigy qa:docs`, `effigy qa:routes`, `effigy qa:northstar`, and
`git diff --check`.
...
remain out of scope. The evidence stop is merged; no reasoning capability is
claimed.

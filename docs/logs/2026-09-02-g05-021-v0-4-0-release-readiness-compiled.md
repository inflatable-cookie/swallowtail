...
one operator-authorized authenticated working-application proof.

Card 050 is ready. Cards 051-052 are planned and serial. Card 051 first requires
read-only status and explicit-version prepare-plan evidence, then stops for
separate authorization before Effigy's sole mutating prepare path. Card 052 is
blocked until the operator names the application/repository, authenticated
route, command, exact candidate consumption, credentials/provider authority,
mutation allowance, success evidence/redaction, cleanup, and retry budget. No
application is inferred from prior consumer work, and provider-free evidence
cannot substitute for the Contract 036 application path.

...
| Publish cohesive route-feature and control truth without adapter downcasts or execution-authority widening. | Contract 061 active; candidates A, D, G, and H complete through cards 022-024 and 031-032; 249 rows proved; 518 remain; the Kimi gate stopped and card 034 is blocked behind two operator decisions | reviewed 767-row census; Contracts 037, 047, 057, 061; accepted realization and PR-review decisions; Batch 9.1, 9.4, and Kimi gates | `g05.008` card 021; `g05.009` cards 022-024 and 030-034 |
...
decisions is named and not compiled. The operator then answered all five:
...
active source IDs stay distinct. The gate then stopped on the fifth answer. A
completed `KimiPreparedSessionCatalogue::list_sessions` query opens no session,
while `ActiveSessionObservation`, `PostOpenObservationOnly`, and
`ConsumerRouteActiveSessionState` are each defined in `swallowtail-runtime` as
post-open session semantics, so publishing
`control.provider-session-catalogue` would widen shared Contract 061 vocabulary
from an adapter document. A second item is unresolved: the compound
acknowledgement row loses half-to-state association for a generic consumer. The
gate re-derives F's four route ledgers as 74 emitted, 14 withheld, and 1
undecided across 89 rows, leaves candidate F unpromoted, and returns two
operator decisions. Card 034 is `blocked`; coverage stays 249/518.
...
five `kimi-code.acp` answers were then compiled, and the gate stopped on the
provider-operation observation vocabulary. g05.009 is `strict-paused`; no card
is ready, and the sole Next Task is two operator decisions.
...
- [009 Contract 061 Consumer Projection Realization](./009-contract-061-consumer-projection-realization.md) — ready; strict-paused; cards 022-024 and 031-032 merged; 249 rows proved; the Kimi gate returned two operator decisions and card 034 carries no implementation authority

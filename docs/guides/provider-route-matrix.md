...
| `claude-agent.sdk` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | qualified exact `0.3.258` on `claude-agent.sdk.package` plus exact native, Node, wire, and sidecar source-tag points; session persistence is disabled, so no provider session is retained, loaded, or resumed | sidecar close joins its retained native handle to a declared 2000 ms bound, the host escalates through descendant-tree termination on expiry and re-joins, and an unconfirmed exit is reported as cleanup failure |

# 265 All-Route Version Currentness Checkpoint
...
After Codex `0.152.0` qualification, what are the current official stable
...
| 8 | Pi coding agent RPC (`pi.rpc`) | `pi` resolves; `--version` `0.83.0` | npm [`@earendil-works/pi-coding-agent` `0.84.4`](https://registry.npmjs.org/@earendil-works%2Fpi-coding-agent/latest), published 2026-08-28 | maintained exact published points through `0.84.3`; `0.83.1` gap; AllowUnverified | visible unverified-newer |
...
| 12 | Command Code headless (`command-code.headless`) | `command-code` resolves; `--version` `1.15.1` | npm [`command-code` `1.39.2`](https://registry.npmjs.org/command-code/latest), published 2026-08-31 | exact `1.15.1`; QualifiedOnly | record only; future range work deferred |
...
| 26 | Muse Code headless (`muse-code.headless`) | `muse` resolves; `--version` `Muse Code 0.2.1 (0.2.1-R1215.1)` | no public package or release channel for the signed payload located; local authority remains the exact payload record | exact opaque `0.2.1-R1215.1`; QualifiedOnly; mutable launcher is not the execution target | unchanged |
...
| 31 | Oh My Pi RPC (`oh-my-pi.rpc`) | `omp` resolves; `--version` `omp/18.0.11` | npm [`@oh-my-pi/pi-coding-agent` `18.0.11`](https://registry.npmjs.org/@oh-my-pi%2Fpi-coding-agent/latest), published 2026-08-29; major line reset from 17.x | maintained `17.2.9..=17.4.0`; later stable unverified only under ordered semantics; major reset requires identity evidence | record only; future range work deferred |
...
| 38 | Grok Build ACP (`grok-build.acp`) | `grok` resolves; `--version` `grok 1.0.13 (5e9a58528b76) [stable]` | npm [`@xai-official/grok` latest `1.0.13`](https://registry.npmjs.org/@xai-official/grok/latest), published 2026-08-28; `alpha` is `1.0.16` and ignored; ACP registry `1.0.16` is discovery metadata | deprecated `0.2.114..=0.2.117`; maintained `1.0.4..=1.0.5`; later stable unverified; do not flatten registry/alpha identity into npm `latest` | visible unverified-newer |
...
1. **Claude Code `2.1.252`** — the host is on the qualified headless and
   response-only windows at `2.1.251`, the official npm and GitHub stable
   point is `2.1.252`, and the current claim already permits an exact forward
   attempt. Published stable points after the current `2.1.251` ceiling
   contain `2.1.252` only. Existing gaps `2.1.244` and `2.1.249` remain
   incompatible. The next ordered stable `2.1.253` is not published.

No second family is selected in this record. Pi, Kimi, OpenCode, Ollama,
Antigravity, and Grok remain visible ordered newer candidates for later
...
Compile Claude Code `2.1.252` next through the Contract 029 Upgrade Workflow,
one family only. This checkpoint itself changes no claim, selection, matrix,
...
- [Research 127](./127-all-route-version-currentness-checkpoint.md),
  ...
  [Research 263](./263-all-route-version-currentness-checkpoint.md), and
  [Research 264](./264-codex-0-152-0-identity.md).
...
- frozen currentness records [127](./127-all-route-version-currentness-checkpoint.md),
  ...
  [263](./263-all-route-version-currentness-checkpoint.md), and
  [264](./264-codex-0-152-0-identity.md)

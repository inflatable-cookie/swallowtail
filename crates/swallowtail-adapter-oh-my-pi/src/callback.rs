...
use crate::protocol::OhMyPiUiDialogMethod;
...
impl CallbackHub {
    pub(crate) fn new(connection: Weak<OhMyPiConnection>) -> (Self, CallbackExchange) {
        ...
        let responder: Arc<dyn CallbackResponder> = Arc::new(OhMyPiCallbackResponder {
            state: Arc::clone(&state),
            connection,
        });
    ...
    pub(crate) fn enqueue(
        ...
        let mut state = self.state.lock().expect("OhMyPi callback lock poisoned");
        if state.closed {
            return Err(callback_closed());
        }
        if !state.provider_ids.insert(provider_id.clone()) {
            return Err(failure(
                "swallowtail.oh_my_pi.rpc.callback_id_reused",
                "OhMyPi RPC reused an extension UI request id",
            ));
        }
        if state.pending.len() >= CALLBACK_CAPACITY || state.requests.len() >= CALLBACK_CAPACITY {
            return Err(failure(
                "swallowtail.oh_my_pi.rpc.callback_capacity_exceeded",
                "OhMyPi RPC exceeded the bounded callback capacity",
            ));
        }
...
struct OhMyPiCallbackResponder {
    state: Arc<Mutex<CallbackState>>,
    connection: Weak<OhMyPiConnection>,
}

impl CallbackResponder for OhMyPiCallbackResponder {
...
fn claim_response(
    ...
    let mut state = state.lock().expect("OhMyPi callback lock poisoned");
    if state.expired.contains(response.callback_id()) {
        return Err(failure(
            "swallowtail.oh_my_pi.rpc.callback_expired",
            "OhMyPi RPC callback response arrived after its deadline",
        ));
    }
    if state.closed {
        return Err(callback_closed());
    ...
    let pending = state.pending.get(response.callback_id()).ok_or_else(|| {
        failure(
            "swallowtail.oh_my_pi.rpc.callback_unknown_or_duplicate",
            "OhMyPi RPC callback response is unknown or was already used",
        )
    })?;
    if &pending.operation_id != response.operation_id() {
        return Err(failure(
            "swallowtail.oh_my_pi.rpc.callback_turn_mismatch",
            "OhMyPi RPC callback response belongs to a different turn",
        ));
    }
    ...
    let mut pending = state
        ...
        .expect("validated OhMyPi callback remains pending");
    if let Some(waiter) = pending.waiter.take() {
        waiter.wake();
    }
    Ok(value)
    ...
    match result {
        ...
        CallbackResult::UserInput(response) => {
            if !pending.user_input.accepts(response) {
                return Err(failure(
                    "swallowtail.oh_my_pi.rpc.callback_result_invalid",
                    "OhMyPi RPC user-input response did not match the pending dialog",
                ));
            }
            let answer = response
                .answers()
            ...
            match pending.method {
                OhMyPiUiDialogMethod::Confirm => {
                    let confirmed = answer
                        ...
                        .ok_or_else(|| {
                            failure(
                                "swallowtail.oh_my_pi.rpc.callback_confirmation_invalid",
                                "OhMyPi RPC confirmation callback did not select an option",
                            )
                        })?
                        .parse::<bool>()
                        .map_err(|_| {
                            failure(
                                "swallowtail.oh_my_pi.rpc.callback_confirmation_invalid",
                                "OhMyPi RPC confirmation callback was not boolean",
                            )
                        })?;
                    Ok(json!({
                        "type": "extension_ui_response",
                ...
                OhMyPiUiDialogMethod::Select => {
                    let text = answer
                        ...
                        .ok_or_else(|| {
                            failure(
                                "swallowtail.oh_my_pi.rpc.callback_selection_invalid",
                                "OhMyPi RPC selection callback did not select an option",
                            )
                        })?;
                    Ok(json!({
                        "type": "extension_ui_response",
                ...
                OhMyPiUiDialogMethod::Input | OhMyPiUiDialogMethod::Editor => {
                    let text = answer.text().map(|text| text.as_str()).ok_or_else(|| {
                        failure(
                            "swallowtail.oh_my_pi.rpc.callback_input_invalid",
                            "OhMyPi RPC input callback did not contain text",
                        )
                    })?;
                    Ok(json!({
                        "type": "extension_ui_response",
        ...
        CallbackResult::Success(_) => Err(failure(
            "swallowtail.oh_my_pi.rpc.callback_result_kind_mismatch",
            "OhMyPi RPC UI callback requires a typed user-input response",
        )),
    }
}
...
fn callback_closed() -> RuntimeFailure {
    failure(
        "swallowtail.oh_my_pi.rpc.callback_closed",
        "OhMyPi RPC callback exchange is closed",
    )
}

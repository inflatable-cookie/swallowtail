...
pub(crate) struct MuseCancellation {
    process: Arc<dyn ProcessHandle>,
    requested: AtomicBool,
}

impl MuseCancellation {
...
impl CancellationControl for MuseCancellation {
...
pub(crate) struct MuseRunHandle {
...
impl RunHandle for MuseRunHandle {
        ...
        Box::pin(async move {
            match self.task.join().await {
                ...
                Err(_) => CleanupOutcome::Failed(SafeDiagnostic::new(
                    "swallowtail.muse_code.headless.task_join_failed",
                    "Muse Code operation task could not be joined",
                )),
            }
        })
    }

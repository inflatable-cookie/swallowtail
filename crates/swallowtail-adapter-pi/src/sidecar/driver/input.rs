...
pub(super) struct PiSdkSidecarAttachmentInput {
    encoded: String,
    materialization: SharedAttachmentMaterialization,
}

impl PiSdkSidecarAttachmentInput {
...
pub(super) async fn prepare_attachment(
    ...
    scope: ScopeId,
) -> Result<Option<PiSdkSidecarAttachmentInput>, RuntimeFailure> {
    ...
    Ok(Some(PiSdkSidecarAttachmentInput {
...
fn failure(reason: &'static str) -> RuntimeFailure {
    crate::failure::failure(
        "swallowtail.pi.sdk-sidecar.attachment_materialization_failed",
        format!("Pi SDK sidecar {reason}"),
    )
}

//! Contract 061 contribution emitted by the prepared Deep Agents ACP session.
//!
...
use crate::DeepAgentsPreparedSession;
...
/// Collects contributed rows from one exact prepared Deep Agents session plan.
...
/// without a `deepagents.acp` census row stays withheld the same way.

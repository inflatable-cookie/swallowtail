...
pub(super) fn activity_profile(
    ...
    let behavior_revision = match prepared.observation().compatibility() {
        ...
        InstalledExecutableCompatibility::Incompatible => {
            return Err(failure(
                "swallowtail.oh_my_pi.preparation.activity_version_incompatible",
                "OhMyPi RPC activity requires a permitted executable version",
            ));
        }
    };
    ...
    .map_err(|_| {
        failure(
            "swallowtail.oh_my_pi.preparation.activity_profile_invalid",
            "OhMyPi RPC activity profile could not be derived",
        )
    })
}
...
pub(super) fn with_activity(
    ...
    requirements.push(
        activity
            ...
            .expect("prepared OhMyPi activity is available"),
    );

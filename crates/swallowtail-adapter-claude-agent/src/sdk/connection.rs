...
impl SdkConnection {
    ...
    /// Returns the host-owned sidecar process this connection speaks to, so an
    /// enclosing cleanup guardian can own the same handle.
    pub(crate) fn process(&self) -> Arc<dyn ProcessHandle> {

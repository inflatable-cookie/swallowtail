# 082 Claude Agent ACP 0.69.0 Useful Newer

Status: completed
Owner: Tom
Created: 2026-08-18
Depends on: Research 127; Research 139; g03.081
Vision tags: maintained compatibility, exact interfaces, currentness
Contract refs: 001, 011, 015, 029, 032, 037-039, 041, 044
Planning state: cards 252-253 completed

## Problem

Research 127 ranked remaining AllowUnverified families after host-drifted
Ollama. Claude Agent ACP is `0.53.0..=0.64.0` excluding `0.58.0` with
AllowUnverified. This host is already qualified `0.63.0`. Official npm
`latest` is `0.69.0`. Raising the bound is useful-newer support, not a
silent `latest` bump and not a flatten onto Claude Code.

## Generation Runway Goal

Freeze Claude Agent ACP `0.63.0` / `0.69.0` identity against the `0.64.0`
...
- [x] rank remaining AllowUnverified registry-newer families and pick
      Claude Agent ACP
- [x] freeze host `0.63.0` and official `0.69.0` identity and selected
      ACP source
- [x] raise the adapter claim through exact `0.69.0`, extending v6
      through `0.65.0` and adding private v7 from `0.66.0`
- [x] leave Pi, Qwen, Antigravity, and Gemini for later one-family work

## Non-Goals

- flattening Claude Code headless or response-only onto this axis
- mapping advertised goal, Air, file-change, nested transcript, or
...
### Batch 82.1 — Identity Corpus

- [x] Execute card 252.
- [x] record host `0.63.0` and official `0.69.0` against exact `0.64.0`
- [x] name compatible-extension plus private v7 for card 253 without
      changing production claims

### Batch 82.2 — Claim And Acceptance

- [x] Execute card 253 after card 252 names compatible-extension.
- [x] extend `0.64.0..=0.65.0` on v6 (Deprecated) and add Maintained
      `0.66.0..=0.69.0` initialize-meta-extensions-v7
...
- [x] official `0.69.0` classifies as Qualified Maintained
...
g03.082 is complete. Next family is Pi RPC (registry newer; host already
on a qualified bound). Gemini stays deferred.

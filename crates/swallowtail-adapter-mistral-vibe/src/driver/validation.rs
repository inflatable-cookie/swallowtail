...
pub(super) fn validate(
    ...
    {
        return Err(plan_mismatch("instance, local config, or model route"));
    }
...
fn require_service(
        ...
        .any(|required| required == service)
    {
        ...
        Err(crate::failure::failure(
            "swallowtail.mistral-vibe.headless.host_service_missing",
            format!("Mistral Vibe headless requires the preflight-bound {name} service"),
        ))
    } else {
        Ok(())
    }
...
fn plan_mismatch(dimension: &str) -> RuntimeFailure {
    crate::failure::failure(
        "swallowtail.mistral-vibe.headless.request_plan_mismatch",
        format!("Mistral Vibe headless request does not match its preflight-bound {dimension}"),
    )
}

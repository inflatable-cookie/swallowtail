...
Status: complete; card 135 qualified compatible `surface-19` through `1.18.29`
Owner: Tom
Created: 2026-09-07
Updated: 2026-09-07
...
1. Card 134 freezes official npm/GitHub `1.18.29` against the `1.18.28`
...
3. Continue to card 135 only for an admitted compatible segment or private
   milestone named by card 134.
4. Card 135 changes only the admitted claim segment and matching exact
...
- [134 OpenCode HTTP 1.18.29 Identity](batch-cards/134-opencode-http-1-18-29-identity.md) — complete; compatible `surface-19` admitted
- [135 OpenCode HTTP 1.18.29 Claim](batch-cards/135-opencode-http-1-18-29-claim.md) — complete; qualified through `1.18.29`
...
| Owned mutable paths | `docs/research/292-*.md`; `docs/research/README.md` one index line; `crates/swallowtail-adapter-opencode/tests/**` identity fixtures and delta-ledger tests only; card 134 result/status; `PAPERCUTS.md` append only |
...
| Serial edges | card 135 follows only after card 134 records an admitted segment |
...
Card 134 recorded the admitted compatible `surface-19` segment through
Research 292, so card 135 is the serial follow-on.

| Field | Card 135 |
| --- | --- |
| Readiness | complete |
| Prerequisites | card 134 identity evidence with the admitted `surface-19` extension; Research 292 and the `opencode-1.18.29` corpus |
...
| Owned mutable paths | `crates/swallowtail-adapter-opencode/src/selection.rs` and its selection tests; `crates/swallowtail-adapter-opencode/tests/**` claim fixtures; `docs/guides/opencode-http-prepared-integration.md`; `docs/guides/provider-route-matrix.md` and `provider-solution-feature-matrix.csv` OpenCode version cell only; architecture ceiling lines naming this bound; `CHANGELOG.md` `[Unreleased]`; the standing-lane OpenCode claim paragraph; one new `docs/logs/` claim entry plus its index line; card 135 result and status; `PAPERCUTS.md` append only |
| Reserved shared closeout surfaces | `docs/roadmaps/README.md`, `docs/roadmaps/g05/README.md`, this roadmap, `docs/roadmaps/g05/batch-cards/README.md`, `docs/roadmaps/generation-index.md`, the rest of `docs/roadmaps/standing-lanes.md` |
...
| Approved concurrent siblings | g05.035 card 114 and disjoint g05.029/g05.034 owners |
| Serial edges | none after card 134 |
...
- [x] only an admitted segment reaches card 135

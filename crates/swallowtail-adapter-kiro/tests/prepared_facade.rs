...
use swallowtail_adapter_kiro::{
    KIRO_ACP_HTTP_MCP_PLACEMENT, KIRO_ACP_MCP_SERVER_NAME, KIRO_CLI_EXECUTABLE_NAME,
    KIRO_CLI_RELEASE_AXIS, KIRO_CLI_RELEASE_VERSION, KIRO_LOCAL_ACCOUNT_AUDIENCE,
    KiroAcpRemoteMcpPlacement, KiroPreparationInput, KiroPreparationProbe, KiroSessionProfileInput,
    kiro_local_account_access_profile, prepare_kiro_acp,
};
...
#[test]
fn prepared_session_names_kiro_acp_and_release_then_drains_one_prompt() {
    ...
    let prepared = block_on(prepare_kiro_acp(
        preparation_input(host_id.clone()),
        probe(),
        services,
    ))
    .expect("Kiro ACP prepares");
    assert_eq!(
        discovery
            .observed_process()
    ...
    assert_eq!(
        ...
        KIRO_CLI_RELEASE_AXIS
    );
    assert_eq!(
        prepared.observation().version().version().as_str(),
        KIRO_CLI_RELEASE_VERSION
    );
    assert_eq!(
    ...
    assert_eq!(
        versions,
        [(
            KIRO_CLI_RELEASE_AXIS.to_owned(),
            KIRO_CLI_RELEASE_VERSION.to_owned()
        )]
    );
...
fn preparation_rejects_access_axis_and_package_drift_before_acp_work() {
    ...
    let wrong_access = AccessProfile::new(
        AccessProfileId::new("kiro.fixture.local-account").expect("access"),
    ...
    let error = block_on(prepare_kiro_acp(
        KiroPreparationInput::new(
            ConfiguredInstanceId::new("kiro.fixture.instance").expect("instance"),
            InstanceRevision::new("1").expect("revision"),
            host_id.clone(),
            target(),
            EnvironmentRef::new("kiro.fixture.isolated").expect("environment"),
            wrong_access,
            evidence(),
        ),
        probe(),
    ...
    let axis_host = DiscoveryHost::new(KIRO_CLI_RELEASE_VERSION);
    let error = block_on(prepare_kiro_acp(
        KiroPreparationInput::new(
            ConfiguredInstanceId::new("kiro.fixture.instance").expect("instance"),
            InstanceRevision::new("1").expect("revision"),
            ...
            InstalledExecutableTarget::new(
                ExecutableRef::new(format!("/fixture/bin/{KIRO_CLI_EXECUTABLE_NAME}"))
                    .expect("executable"),
                InterfaceVersionAxis::new("kiro.headless").expect("axis"),
            ),
            EnvironmentRef::new("kiro.fixture.isolated").expect("environment"),
    ...
    .expect_err("kiro.headless axis is not this route");
    assert_eq!(
        error.diagnostic().safe().code(),
...
const HTTP_CANARY_URL: &str = "http://127.0.0.1:9/mcp/g06-037-redaction-canary";
const HTTP_CANARY_HEADER: &str = "Bearer g06-037-redaction-canary";

fn http_placement() -> KiroAcpRemoteMcpPlacement {
...
#[test]
fn prepared_session_binds_the_admitted_http_mcp_placement() {
    ...
    let contribution = session
        .consumer_route_projection_contribution(
            swallowtail_runtime::ConsumerRouteProjectionSourceId::new("kiro.acp.prepared-session")
                .expect("source"),
        )
        .expect("projects");
    ...
    assert!(texts.contains(&KIRO_ACP_HTTP_MCP_PLACEMENT));
    assert!(texts.contains(&KIRO_ACP_MCP_SERVER_NAME));
    assert!(
        !texts
            .iter()
            .any(|value| value.contains("g06-037-redaction-canary"))
    );
    let operation = FixtureHost::new(Scenario::Success);
    let handle =
...
fn working_state_restoration_carries_the_prepared_http_mcp_declaration() {
    ...
    let WorkingStateRestorationOutcome::SessionReplaced(replaced) =
        ...
        panic!("Kiro ACP restoration is a fresh session replacement");
    };
    assert!(operation.writes().iter().any(|message| {
        message["method"] == "session/new"
            && message["params"]["mcpServers"][0]["type"] == "http"
...
fn prepare(host_id: ExecutionHostId) -> swallowtail_adapter_kiro::KiroPreparedIntegration {
    let discovery = DiscoveryHost::new(KIRO_CLI_RELEASE_VERSION);
    let operation = FixtureHost::new(Scenario::Success);
    let mut services = discovery.services(host_id.clone());
    services = services.with_working_resource(
...
fn preparation_input(host_id: ExecutionHostId) -> KiroPreparationInput {
    KiroPreparationInput::new(
        ConfiguredInstanceId::new("kiro.fixture.instance").expect("instance"),
        InstanceRevision::new("1").expect("revision"),
        host_id,
...
fn session_input(id: &str) -> KiroSessionProfileInput {
    KiroSessionProfileInput::new(
        RequestId::new(format!("kiro.fixture.session.{id}")).expect("request"),
        WorkingResourceRef::new("kiro.fixture.workspace").expect("resource"),
    )
}

fn probe() -> KiroPreparationProbe {
    KiroPreparationProbe::new(
        RequestId::new("kiro.fixture.probe").expect("request"),
        ScopeId::new("kiro.fixture.probe").expect("scope"),
...
fn target() -> InstalledExecutableTarget {
    InstalledExecutableTarget::new(
        ExecutableRef::new(format!("/fixture/bin/{KIRO_CLI_EXECUTABLE_NAME}")).expect("executable"),
        InterfaceVersionAxis::new(KIRO_CLI_RELEASE_AXIS).expect("axis"),
    )
}

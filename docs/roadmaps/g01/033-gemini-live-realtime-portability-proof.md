...
- [ ] Add the minimum provider-neutral bounded-rollover capability and request
      policy without changing disabled existing routes.
- [ ] Freeze the current Gemini Live preview raw-WebSocket corpus for one exact
      server-to-server manual-audio subset.
- [ ] Add a separately registered Gemini Live realtime-media driver without
      changing the Gemini CLI ACP driver.
- [ ] Prove two serial turns across one planned connection replacement under
      local and remote-authoritative host identities.
- [ ] Preserve private handle, query-secret, cancellation, failure, and joined
      two-connection cleanup truth.

## Execution Plan

- [ ] Rollover records and Gemini Live corpus: card 095.
- [ ] Gemini Live WebSocket driver: card 096.
- [ ] Gemini Live portability conformance and closeout: card 097.

## Cards

- `batch-cards/095-realtime-rollover-records-and-gemini-corpus.md` — ready
- `batch-cards/096-gemini-live-websocket-driver.md` — planned
- `batch-cards/097-gemini-live-portability-conformance-and-closeout.md` — planned

...
impl AccessLeases {
    pub(super) async fn acquire(
        ...
        {
            return Err(failure(
                "swallowtail.gemini.live_network_grant_mismatch",
                "Gemini Live network grant did not match preflight",
            ));
        }
            ...
            || !matches!(credential, CredentialLease::Secret(_))
        {
            let _ = services
                ...
                .await;
            return Err(failure(
                "swallowtail.gemini.live_credential_lease_rejected",
                "Gemini Live credential lease did not match preflight",
            ));
        }
        Ok(Self {
            grant,
    ...
    ) -> Result<(WorkerHandle, BoxFuture<'static, Result<(), RuntimeFailure>>), RuntimeFailure>
    {
        ...
        services
            ...
            .await?;
        let (worker, handle) = receiver.await.map_err(|_| unavailable())??;
...
fn unavailable() -> RuntimeFailure {
    failure(
        "swallowtail.gemini.live_connection_unavailable",
        "Gemini Live connection could not be established",
    )
}

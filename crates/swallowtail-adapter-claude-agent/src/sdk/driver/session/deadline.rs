...
///
/// The reap reservation is taken first, before the turn's query reaches the
/// sidecar, so this task can always be handed back to its owning host rather
/// than joined by dropping it.
pub(super) fn spawn_turn_deadline(
    ...
    let reservation = crate::sdk::guardian::reserve_reap(services, &scope)?;
    crate::sdk::guardian::spawn_reserved(
        services,
        reservation,
        Box::pin(async move {

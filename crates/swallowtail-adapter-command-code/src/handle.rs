...
pub(crate) struct CommandCodeCancellation {
    process: Arc<dyn ProcessHandle>,
    requested: AtomicBool,
    scope: CancellationScope,
}

impl CommandCodeCancellation {
...
impl CancellationControl for CommandCodeCancellation {
...
pub(crate) struct CommandCodeRunHandle {
...
impl RunHandle for CommandCodeRunHandle {
        ...
        Box::pin(async move {
            match self.task.join().await {
                ...
                Err(_) => CleanupOutcome::Failed(SafeDiagnostic::new(
                    "swallowtail.command_code.headless.task_join_failed",
                    "Command Code operation task could not be joined",
                )),
            }
        })
    }

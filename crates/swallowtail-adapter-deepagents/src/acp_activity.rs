...
impl AcpActivityProjection {
    ...
    fn unknown(&mut self, namespace: &str) -> Result<Vec<ActivityObservation>, RuntimeFailure> {
        ...
        let activity = OpenActivity {
            id: ActivityId::new(format!("deepagents-acp:unknown:{}", self.next_id))
                .map_err(|_| malformed())?,
            provider_ref: None,
            kind: ActivityKind::Unknown(
                ActivityNamespace::new(format!("deepagents.acp.{namespace}"))
                    .map_err(|_| malformed())?,
            ),
    ...
    fn open_or_insert(
        ...
        let id = ActivityId::new(format!("deepagents-acp:{key}"))
            .or_else(|_| {
                self.next_id = self.next_id.saturating_add(1);
                ActivityId::new(format!("deepagents-acp:minted:{}", self.next_id))
            })
            .map_err(|_| malformed())?;

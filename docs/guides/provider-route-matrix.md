...
| `kimi-code.acp` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | exact qualified `0.28.1` plus `0.29.0..=0.38.0`; exact `0.39.0` and `0.39.1` are incompatible because the agent-core-v2 ACP terminal runner replaces fail-closed errors with an uncontained local host-process spawn under the `terminal: false` capabilities this route advertises; later stable points may be visible `UnverifiedNewer`; selected ACP route supports separate catalogue/import; initialize may advertise close/delete, which stay unmapped | catalogue/import joins its process and releases leases; interactive connection cleanup preserves provider state |

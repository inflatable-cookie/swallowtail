...
| `opencode.acp` | `swallowtail-adapter-opencode`; `swallowtail.opencode.acp` | one bounded interactive session; ACP v1 stdio on `opencode acp --pure`; one consumer MCP entry, stdio or streamable-HTTP, under a route-owned name; streamable-HTTP honouring observed on exact `1.18.18` without verified exact-model attribution | approved executable and environment; provider-supported host-owned OpenCode login without a credential lease; explicit working resource | `opencode.executable`; deprecated `1.18.18..=1.18.30`; maintained `1.18.31..=1.18.32`; later stable points may be visible `UnverifiedNewer` | `prepare_opencode_acp` → `prepare_session` → `open_session` | `OpenCodeAcpDriver`; `InteractiveSessionDriver` |
...
| `claude-code.headless` | `not-applicable` | `no` | `not-applicable` | `not-applicable` | `not-applicable` | `not-applicable` | qualified `2.1.220..=2.1.278` excluding unpublished `2.1.244`, `2.1.249`, `2.1.253` through `2.1.256`, `2.1.262`, and `2.1.264`; later stable releases may be visible `UnverifiedNewer`; every run disables session persistence | process and task cleanup only; no provider session is retained |
| `claude-code.response-only` | `not-applicable` | `no` | `not-applicable` | `not-applicable` | `not-applicable` | `not-applicable` | qualified `2.1.227..=2.1.278` excluding unpublished `2.1.244`, `2.1.249`, `2.1.253` through `2.1.256`, `2.1.262`, and `2.1.264`; later stable releases run provisionally unless explicitly denied; every run disables session persistence and exposes no reusable provider identity | process and task cleanup only; no provider session is retained |

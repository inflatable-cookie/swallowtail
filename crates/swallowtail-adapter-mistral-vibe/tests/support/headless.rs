...
use swallowtail_adapter_mistral_vibe::{
    MISTRAL_VIBE_RELEASE_VERSION, mistral_vibe_headless_descriptor,
    mistral_vibe_local_config_access_profile, mistral_vibe_release_binding,
};
...
pub fn stderr_chunk(bytes: Vec<u8>) -> ProcessOutputChunk {
    ProcessOutputChunk::new(ProcessOutputStream::Stderr, bytes)
}
...
pub fn selection(host: ExecutionHostId) -> FixtureSelection {
    ...
    let access_id =
        AccessProfileId::new("mistral-vibe.fixture.local-config").expect("valid access id");
    let instance_id =
        ConfiguredInstanceId::new("mistral-vibe.fixture.instance").expect("valid instance");
    ...
    let version_binding =
        mistral_vibe_release_binding(MISTRAL_VIBE_RELEASE_VERSION).expect("fixture version");
    let instance = ConfiguredInstance::new(
        instance_id,
        InstanceRevision::new("fixture-revision").expect("valid revision"),
        AdapterId::new("swallowtail.mistral-vibe.headless").expect("valid adapter"),
        host.clone(),
        InstanceTargetRef::new("mistral-vibe.fixture.executable").expect("valid target"),
        InstanceOwnership::HostOwnedEphemeral,
        access_id.clone(),
        SupportAuthority::ProviderSupported,
        ProtocolFacadeId::new("mistral-vibe-streaming-v1").expect("valid facade"),
        InstancePolicyId::new("mistral-vibe.fixture.isolated").expect("valid policy"),
        capabilities.clone(),
    )
    .with_interface_versions([version_binding.clone()])
    .with_harness_configuration_posture(HarnessConfigurationPosture::Ambient);
    let access = mistral_vibe_local_config_access_profile(access_id.clone());
    ...
    let plan =
        preflight(&context, &requirements).expect("Vibe headless fixture preflight succeeds");
    FixtureSelection {
        plan,
        resource: WorkingResourceRef::new("mistral-vibe.fixture.workspace")

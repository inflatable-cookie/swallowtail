...
fn validate_recovery(
    ...
    let requirement = plan
        ...
        .ok_or_else(|| {
            failure(
                "swallowtail.grok.acp.attachment_recovery_capability_mismatch",
                "Grok Build attachment recovery does not match its preflight plan",
            )
        })?;
    for constraint in [
            ...
            .any(|present| present == &constraint)
        {
            return Err(failure(
                "swallowtail.grok.acp.attachment_recovery_capability_mismatch",
                "Grok Build attachment recovery bounds do not match its preflight plan",
            ));
        }
    }
    ...
    {
        return Err(failure(
            "swallowtail.grok.acp.attachment_recovery_binding_mismatch",
            "Grok Build attachment recovery does not match its durable binding",
        ));
    }
    Ok(())
}

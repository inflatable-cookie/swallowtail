...
impl AcpConnection {
    ...
    pub(crate) async fn initialize(&self) -> Result<Value, RuntimeFailure> {
        ...
        if response.get("protocolVersion").and_then(Value::as_u64) != Some(ACP_PROTOCOL_VERSION) {
            return Err(failure(
                "swallowtail.cline.acp.version_mismatch",
                "Cline negotiated an incompatible ACP version",
            ));
        }
        Ok(response)
    }
    ...
    async fn begin_request_with_id(
        ...
        if self.closed.load(Ordering::SeqCst) {
            return Err(failure(
                "swallowtail.cline.acp.connection_closed",
                "Cline ACP connection is closed",
            ));
        }
        let (sender, response) = response_channel();
        {
            ...
            if pending.len() >= MAXIMUM_PENDING_REQUESTS || pending.insert(id, sender).is_some() {
                return Err(failure(
                    "swallowtail.cline.acp.correlation_limit",
                    "Cline ACP request correlation limit was exceeded",
                ));
            }
        }
    ...
    pub(crate) fn set_session_id(&self, session_id: String) -> Result<(), RuntimeFailure> {
        ...
        if current.is_some() {
            return Err(failure(
                "swallowtail.cline.acp.session_duplicate",
                "Cline returned more than one ACP session",
            ));
        }
    ...
    pub(crate) fn set_active_turn(&self, turn: Arc<ActiveTurn>) -> Result<(), RuntimeFailure> {
        ...
        if active.as_ref().is_some_and(|active| !active.is_finished()) {
            return Err(failure(
                "swallowtail.cline.acp.turn_active",
        ...
        self.cleanup
            ...
            .unwrap_or_else(|| {
                CleanupOutcome::Failed(SafeDiagnostic::new(
                    "swallowtail.cline.acp.cleanup_missing",
                    "Cline process cleanup did not complete",
                ))
            })
    }

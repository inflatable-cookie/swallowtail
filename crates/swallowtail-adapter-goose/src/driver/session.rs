...
impl InteractiveSessionHandle for GooseSessionHandle {
    ...
    fn start_turn<'a>(
        ...
        Box::pin(async move {
            ...
            {
                return Err(failure(
                    "swallowtail.goose.acp.turn_active",
                    "Goose session already has an active turn",
                ));
            }
            ...
            let scope = ScopeId::new(format!("goose-acp:turn:{}", request.turn_id().as_str()))
                .map_err(|_| malformed())?;
            ...
            let task_cleanup = match self.pump_task.take() {
                Some(task) => match task.join().await {
                    ...
                    Err(_) => CleanupOutcome::Failed(swallowtail_core::SafeDiagnostic::new(
                        "swallowtail.goose.acp.task_join_failed",
                        "Goose ACP protocol task did not join cleanly",
                    )),
                },
            ...
            let resource_cleanup = match self.resource.take() {
                Some(resource) => match self.services.working_resource() {
                    ...
                    None => CleanupOutcome::Failed(swallowtail_core::SafeDiagnostic::new(
                        "swallowtail.goose.acp.resource_release_failed",
                        "Goose ACP working-resource service disappeared during cleanup",
                    )),
                },
                None => CleanupOutcome::NotApplicable,
            };

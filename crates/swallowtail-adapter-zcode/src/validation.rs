...
pub(crate) fn validate(
    ...
    for (service, present, name) in [
            ...
            || !present
        {
            return Err(crate::failure::failure(
                "swallowtail.zcode.app_server.host_service_missing",
                format!("ZCode app-server requires the preflight-bound {name} service"),
            ));
        }
    }
    if plan.ownership() != InstanceOwnership::HostOwnedEphemeral
        ...
        || plan.endpoint_audience().as_str() != crate::ZCODE_CONFIG_AUDIENCE
...
fn plan_mismatch(dimension: &str) -> RuntimeFailure {
    crate::failure::failure(
        "swallowtail.zcode.app_server.request_plan_mismatch",
        format!("ZCode app-server request does not match its preflight-bound {dimension}"),
    )
}
...
mod tests {
    ...
    #[test]
    fn host_services_are_task_process_time_and_session_stop_is_not_a_role() {

use super::{events::ClineHeadlessEventParser, handle::ClineHeadlessCancellation};
...
const ROUTE: &str = "cline.headless";

pub(super) async fn pump(
    process: Arc<dyn ProcessHandle>,
    events: RuntimeEventSender,
    cancellation: Arc<ClineHeadlessCancellation>,
    deadline: BoxFuture<'static, DeadlineObservation>,
    operation_id: ActivityOperationId,
    services: HostServices,
) -> TerminalOutcome {
    let mut parser = ClineHeadlessEventParser::new(operation_id);
    ...
    match (parser.finish(), exit) {
        ...
        (_, Err(_)) => {
            let diagnostic = SafeDiagnostic::new(
                "swallowtail.cline.headless.process_wait_failed",
                "Cline process wait failed",
            );
...
async fn next_output(
    ...
    cancellation: &ClineHeadlessCancellation,
...
fn event_delivery_failed(cleanup: CleanupOutcome) -> TerminalOutcome {
    TerminalOutcome::new(
        TerminalStatus::RuntimeFailed(SafeDiagnostic::new(
            "swallowtail.cline.headless.event_delivery_failed",
            "Cline event delivery failed",
        )),
        cleanup,
    )
}
...
fn process_cleanup_failed() -> CleanupOutcome {
    CleanupOutcome::Failed(SafeDiagnostic::new(
        "swallowtail.cline.headless.process_cleanup_failed",
        "Cline process cleanup failed",
    ))
}

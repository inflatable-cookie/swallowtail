# g06.034 Copilot CLI ACP Consumer HTTP MCP Wiring
...
Wire the consumer-supplied streamable-HTTP MCP entry into `copilot-cli.acp`
production `session/new` at the route's current point (`1.0.80` in Research
351). Provider-free tests only.

## Why It Matters

Research 351 classified `copilot-cli.acp` as a `direct-http-candidate` from its shipped
...
- Emit `http` with a route-owned unique name. The provider rejects stdio client entries and skips names that collide with agent-configured servers, so a colliding name must be a typed refusal or a reserved-name guarantee, never a silent skip.
...
- No stdio client entry is offered on this route.
...
- **Owned mutable paths:** `crates/swallowtail-adapter-copilot-cli/**`; `docs/guides/copilot-cli-acp-prepared-integration.md`; the `copilot-cli.acp` route
  and feature matrix rows; the working public-API baseline for `swallowtail-adapter-copilot-cli` if
...
Research 351. Focused and affected-package validation for `swallowtail-adapter-copilot-cli`, route and
docs QA.

## Next Task

Chatterbox asks the operator whether to run a live gate for `copilot-cli.acp`.

...
fn require_service(
        ...
        .any(|required| required == service)
    {
        ...
        Err(crate::failure::failure(
            "swallowtail.qoder.headless.host_service_missing",
            format!("Qoder headless requires the preflight-bound {name} service"),
        ))
    } else {
        Ok(())
    }
...
fn plan_mismatch(dimension: &str) -> RuntimeFailure {
    crate::failure::failure(
        "swallowtail.qoder.headless.request_plan_mismatch",
        format!("Qoder headless request does not match its preflight-bound {dimension}"),
    )
}

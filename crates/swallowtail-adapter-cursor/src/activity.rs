...
use swallowtail_protocol_acp::{
    AcpContentBlock, AcpMessageChunk, AcpMessageRole, AcpPlanEntry, AcpPlanEntryPriority,
    AcpPlanEntryStatus, AcpSessionUpdate, AcpToolCall, AcpToolCallContent, AcpToolCallStatus,
    AcpToolCallUpdate,
};
use swallowtail_runtime::{
    ...
    RuntimeTurnId, TaskListItem, TaskListItemPriority, TaskListItemStatus, TaskListSnapshot,
    TerminalStatus,
};
...
impl AcpActivityProjection {
    ...
    fn unknown(&mut self, namespace: &str) -> Result<Vec<ActivityObservation>, RuntimeFailure> {
        ...
        let activity = OpenActivity {
            id: ActivityId::new(format!("cursor-agent-acp:unknown:{}", self.next_id))
                .map_err(|_| malformed())?,
            provider_ref: None,
            kind: ActivityKind::Unknown(
                ActivityNamespace::new(format!("cursor-agent.acp.{namespace}"))
                    .map_err(|_| malformed())?,
            ),
    ...
    fn open_or_insert(
        ...
        let id = ActivityId::new(format!("cursor-agent-acp:{key}"))
            .or_else(|_| {
                self.next_id = self.next_id.saturating_add(1);
                ActivityId::new(format!("cursor-agent-acp:minted:{}", self.next_id))
            })
            .map_err(|_| malformed())?;
...
include!("activity/tool.rs");
include!("activity/content.rs");

---
title: g04.085d Cline ACP Plan-mode evidence worker handoff
...
created: 2026-08-27
updated: 2026-08-27
handoff_path: /Users/tom/Dev/projects/swallowtail/docs/handoffs/20260827-202540-g04-085d-cline-acp-plan-mode-evidence.md
...
This lane owns Cline ACP Plan mode: card 241, Research 240, its reserved log,
and optional new Cline-local frozen evidence.
...
Cline headless has a proved Plan mapping, but exact source says root mode argv
is discarded by the ACP early-return. The ACP route needs its own session-level
selection and confirmation proof or an explicit evidence stop.
...
- **Planning artifacts:** g04.085, card 241, Research 240 reservation, reserved
  lane log, indexes, and sole Next Task
- **Worker branch:** `worker/g04-085d-cline-acp-plan-mode-evidence`
- **Worker worktree:** `/Users/tom/Dev/worktrees/swallowtail-g04-085d-cline-acp-plan-mode-evidence`
- **Worktree creation command:** `git worktree add -b
  worker/g04-085d-cline-acp-plan-mode-evidence
  /Users/tom/Dev/worktrees/swallowtail-g04-085d-cline-acp-plan-mode-evidence
  origin/main`
...
- **Active lane:** per-route feature completion, g04.085 lane D
- **Roadmap:** `docs/roadmaps/g04/085-third-parallel-per-route-feature-qualification.md`
- **Ready card:** `docs/roadmaps/g04/batch-cards/241-cline-acp-plan-mode-evidence.md`
- **Research:** `docs/research/240-cline-acp-plan-mode-evidence.md`
- **Lane log:** `docs/logs/2026-08-27-g04-085d-cline-acp-plan-mode-evidence.md`
...
- **Canonical refs:** `docs/architecture/system-architecture.md`; Contracts
  011, 020, 024, 029, 034, 037, 040, 041, 047, 052; Research 146 and 220;
...
- **Tool/runtime restrictions:** official docs and exact package source plus
...
- **Required validation:** `effigy validate:focused
  swallowtail-adapter-cline`, `effigy qa:northstar`, `git diff --check`
...
- **In scope:** card 241 exactly; `cline.acp` `3.0.55` Plan selection,
  application, confirmation, permission and lifecycle truth, omission,
  Research 240, and the assigned log.
- **Allowed changed files:** assigned card, Research 240, assigned log, and new
  Cline-local frozen evidence under a unique fixture/evidence path.
- **Out of scope:** production code/API, Cline headless, thinking, model
  selection, Act/Yolo/Zen, auto-approve, shared milestone/inventory/programme/
  triage/matrices/indexes/Next Task, currentness, live provider work, release,
  merge, rollover, or g04 closure.
- Do not promote the headless `--plan` row onto ACP. Prove an ACP-owned seam.
- Plan behavior does not prove read-only access, permission denial, sandboxing,
  filesystem containment, or process containment.
...
- The exact ACP route is qualified at package `3.0.55`, uses ACP v1 stdio,
  caller working resources, observational permissions, and no auto-approve.
- Research 220 proves root `--plan` is applied on headless and discarded before
  ACP setup. That is decisive sibling evidence, not the full ACP assessment.
- A non-empty row needs an ACP initialize/session/config/command seam, exact
  pre-prompt application, selected-value confirmation, and Contract 034 fit.
- An empty set is expected if mode exists only in ambient state or on discarded
  root argv.
- Report after Research 240 and card 241 are complete, or earlier on a stop.
- Report to the operator, who relays the PR to the orchestrator.

## Suggested Next Move

Run the startup preflight. Inspect exact ACP initialize, session, configuration,
and command surfaces first; stop with an honest empty set if none owns Plan
selection and confirmation before prompt effects.
...
5. Read `AGENTS.md`, g04.085, card 241, Research 240, the lane log, and named
   canonical refs. Then run the cheap repo orientation checks.

### While you work

- Execute only card 241 and edit only allowed lane files.
...
2. Complete card 241, Research 240, and the assigned log honestly.
3. Push the worker branch and open a PR against current pushed `main`.
4. Link g04.085, card 241, Research 240, changed evidence, validation, and
...
after any earlier evidence PR lands and before fast-forward-only merge. An

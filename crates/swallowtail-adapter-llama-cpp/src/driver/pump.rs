...
fn apply_event(
    ...
    match (event, &*state) {
        ...
        (Event::Finished(reason), StreamState::Streaming)
            if matches!(reason.as_str(), "stop" | "length") =>
        {
            *state = StreamState::Finished;
            Ok(Applied::None)
        }
        (Event::Usage(usage), StreamState::Finished) => {
            *state = StreamState::Usage;
            Ok(Applied::Usage(usage))
        }
        (Event::Done, StreamState::Usage) => {
            *state = StreamState::Complete;
            Ok(Applied::Complete)
        }
        (Event::ProviderFailed, _) => Err(failure(
            "swallowtail.llama_cpp.stream_failed",
            "llama.cpp reported a stream failure",
        )),
        _ => Err(failure(
            "swallowtail.llama_cpp.stream_order_invalid",
            "llama.cpp stream event order was invalid",
        )),
    }
}

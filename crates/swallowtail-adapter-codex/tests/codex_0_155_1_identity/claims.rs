...
#[test]
fn production_exec_claim_admits_0_155_1_as_the_maintained_ceiling() {
    let exec = codex_exec_claim();
    for point in [
        "0.150.1", "0.152.1", "0.153.0", "0.154.0", "0.155.0", "0.155.1",
    ] {
    ...
    let InterfaceCompatibilityAssessment::UnverifiedNewer(unverified) =
        ...
        panic!("0.155.2 must remain unverified newer after the claim card");
    };
...
#[test]
fn production_app_server_claim_admits_0_155_1_as_the_maintained_ceiling() {
...
#[test]
fn identity_decision_names_compatible_extension() {
    ...
    assert_eq!(decision["raise_latest_qualified_to"], "0.155.1");
    ...
    assert_eq!(decision["later_unverified_after_qualification"], "0.155.2");
    assert_eq!(
    ...
    for unmapped in [
        ...
        "map_thread_attachment",
        "map_managed_daemon",
        "map_daemon_thread_recovery",
        "map_turn_admission",
        "map_memory_status",

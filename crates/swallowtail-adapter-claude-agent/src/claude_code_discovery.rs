...
async fn probe_joined(
    ...
    if request.target().version_axis() != claim.axis() {
        return Err(failure(
            ...
            "Claude Code discovery target uses a different version axis",
        ));
    }
...
async fn probe_process(
    ...
    .map_err(|_| {
        failure(
            "swallowtail.claude_code.discovery_classification_failed",
            "Claude Code version observation could not be classified",
        )
    })?;
...
fn outcome(status: DiscoveryStatus) -> DiscoveryOutcome {
    DiscoveryOutcome::new(
        ...
        Some(SafeDiagnostic::new(
            status_code(status),
            "Claude Code installed discovery did not produce a compatible observation",
        )),
    )
}
...
const fn status_code(status: DiscoveryStatus) -> &'static str {
    match status {
        DiscoveryStatus::Absent => "swallowtail.claude_code.discovery_absent",
        DiscoveryStatus::Discovered => "swallowtail.claude_code.discovery_discovered",
        DiscoveryStatus::Incompatible => "swallowtail.claude_code.discovery_incompatible",
        DiscoveryStatus::Malformed => "swallowtail.claude_code.discovery_malformed",
        DiscoveryStatus::TimedOut => "swallowtail.claude_code.discovery_timed_out",
        DiscoveryStatus::Cancelled => "swallowtail.claude_code.discovery_cancelled",
        DiscoveryStatus::Failed => "swallowtail.claude_code.discovery_failed",
        DiscoveryStatus::CleanupFailed => "swallowtail.claude_code.discovery_cleanup_failed",
    }
}

...
| `claude-code.response-only` | `not-applicable` | `no` | `not-applicable` | `not-applicable` | `not-applicable` | `not-applicable` | exact qualified-only `2.1.227`; every run disables session persistence and exposes no reusable provider identity | process and task cleanup only; no provider session is retained |

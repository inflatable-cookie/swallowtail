...
pub(super) fn profile(
    ...
    let behavior = match observation.compatibility() {
        ...
        InstalledExecutableCompatibility::Incompatible => {
            return Err(failure(
                "swallowtail.opencode.acp.preparation.activity_version_incompatible",
                "OpenCode ACP activity requires the exact permitted release",
            ));
        }
    };
    ...
    .map_err(|_| {
        failure(
            "swallowtail.opencode.acp.preparation.activity_profile_invalid",
            "OpenCode ACP activity profile could not be derived",
        )
    })
}

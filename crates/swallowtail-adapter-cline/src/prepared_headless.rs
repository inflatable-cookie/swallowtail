//! Adapter-local prepared constructor for `cline.headless`.

#[path = "prepared_headless/activity.rs"]
mod activity;
#[path = "prepared_headless/run.rs"]
mod run;

pub use run::{ClineHeadlessPreparedRun, ClineHeadlessRunProfileInput};

use crate::ClineHeadlessDriver;
...
const PROTOCOL_FACADE_ID: &str = "cline-json-v1";
const POLICY_ID: &str = "cline-prepared-ambient-headless";

#[derive(Clone, Debug, Eq, PartialEq)]
/// Host, target, environment, and local-account inputs for Cline headless preparation.
pub struct ClineHeadlessPreparationInput {
...
impl ClineHeadlessPreparationInput {
    #[must_use]
...
#[derive(Clone, Debug)]
/// Bounded installed-executable probe used during Cline headless preparation.
pub struct ClineHeadlessPreparationProbe {
...
impl ClineHeadlessPreparationProbe {
    #[must_use]
...
#[derive(Clone, Debug, Eq, PartialEq)]
/// Qualified Cline headless integration ready to prepare one bounded print run.
pub struct ClineHeadlessPreparedIntegration {
    environment: EnvironmentRef,
    target: InstalledExecutableTarget,
...
impl ClineHeadlessPreparedIntegration {
    ...
    pub fn low_level_driver(&self) -> ClineHeadlessDriver {
        ClineHeadlessDriver::new(self.environment.clone())
    }
    ...
    pub fn validate_execution_binding(
        ...
        if execution_host_id != self.observation.execution_host_id() || target != &self.target {
            return Err(failure(
                ...
                "swallowtail.cline.headless.preparation.target_drift",
...
/// Discovers and prepares exactly one qualified Cline headless package.
pub async fn prepare_cline_headless(
    ...
    let driver = ClineHeadlessDriver::new(input.environment.clone());
    let outcome = driver
...
fn validate_input(input: &ClineHeadlessPreparationInput) -> Result<(), PreparationFailure> {
    if input.target.version_axis().as_str() != crate::CLINE_PACKAGE_AXIS {
        return Err(failure(
            PreparationStage::TargetSelection,
            "swallowtail.cline.headless.preparation.target_axis_mismatch",
            "Cline headless preparation target uses a different package axis",
        ));
    }
        ...
        || input.access_profile.support_authority() != SupportAuthority::ProviderSupported
    {
        return Err(failure(
            ...
            "Cline headless requires its provider-supported local-account profile",
        ));
    }
            ...
            != input.access_profile.support_authority()
    {
        return Err(failure(
            PreparationStage::AccessEvidence,
            "swallowtail.cline.headless.preparation.access_evidence_mismatch",
            "Cline headless access evidence does not match the selected profile",
        ));
    }
    Ok(())
}
...
fn promote(
    ...
    Ok(ClineHeadlessPreparedIntegration {
...
fn configured_instance(
    ...
    let target =
        InstanceTargetRef::new(input.target.executable().as_host_value()).map_err(|_| {
            failure(
                PreparationStage::TargetSelection,
                "swallowtail.cline.headless.preparation.target_invalid",
                "Cline headless target could not be bound to the configured instance",
            )
        })?;
    Ok(ConfiguredInstance::new(
        input.instance_id.clone(),
        input.instance_revision.clone(),
        crate::cline_headless_descriptor().identity().id().clone(),
        input.execution_host_id.clone(),
        target,
        InstanceOwnership::HostOwnedEphemeral,
        input.access_profile.id().clone(),
        SupportAuthority::ProviderSupported,
        ProtocolFacadeId::new(PROTOCOL_FACADE_ID).expect("static Cline headless facade is valid"),
        InstancePolicyId::new(POLICY_ID).expect("static Cline headless policy is valid"),

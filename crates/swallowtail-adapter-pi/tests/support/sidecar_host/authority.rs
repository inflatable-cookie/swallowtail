use super::{CleanupEvent, SidecarFixtureHost, fixture_failure};
...
impl CredentialService for SidecarFixtureHost {
    fn acquire(
        &self,
        scope: ScopeId,
...
impl WorkingResourceService for SidecarFixtureHost {
    fn resolve(
        &self,
        scope: ScopeId,
        ...
        Box::pin(async move {
            Ok(
                ResourceLease::consumer_owned(scope, reference, access, representation)
                    .with_filesystem(
                        MaterializedResourceRef::new("/fixture/pi-sidecar-workspace")
                            .expect("valid path"),
                    ),
            )
        })

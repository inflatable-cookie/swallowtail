...
pub(super) fn profile(
    ...
    let behavior = match observation.compatibility() {
        ...
        InstalledExecutableCompatibility::Incompatible => {
            return Err(failure(
                "swallowtail.copilot-cli.acp.preparation.activity_version_incompatible",
                "Copilot CLI ACP activity requires the exact permitted release",
            ));
        }
    };
    ...
    .map_err(|_| {
        failure(
            "swallowtail.copilot-cli.acp.preparation.activity_profile_invalid",
            "Copilot CLI ACP activity profile could not be derived",
        )
    })
}
...
pub(super) fn with_activity(
    ...
    requirements.push(
        activity
            ...
            .expect("prepared Copilot CLI activity is available"),
    );

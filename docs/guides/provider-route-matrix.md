...
This is the integration front door for Swallowtail's 51 production routes.
...
runtime posture, version posture, and qualified feature coverage across all 51
...
| `claude-agent.acp` | `swallowtail-adapter-claude-agent`; `swallowtail.claude-agent.acp` | ambient read-write one-prompt structured run and read-only interactive session, both with optional consumer-mediated one-shot permissions; reasoning and plan-mode selection plus inactive provider-session delete; ACP v1 stdio | approved executable and environment; maintainer-supported local Claude subscription auth by default, or explicit Anthropic public API-key pay-as-you-go access | `claude-agent.acp-adapter`; maintained `0.53.0..=0.79.0` excluding `0.58.0`, plus permitted unverified-newer stable points | `prepare_claude_agent` → run, session, or delete profile → `start_run`, `open_session`, or `execute` | `ClaudeAgentAcpDriver`; `StructuredRunDriver`, `InteractiveSessionDriver`, and `ProviderSessionManagementDriver` |
| `claude-agent.sdk` | `swallowtail-adapter-claude-agent`; `swallowtail.claude-agent.sdk` | Unix-only fresh interactive session with bounded open-time supported-model evidence from Query.supportedModels, confirmed effective model, an explicit admitted tool set that is read-only by default and binds a read-write working-resource lease when `Edit`, `Write`, `MultiEdit`, or explicit `Bash` is admitted, and a `default`/`plan`/`acceptEdits` permission mode selected at open and changeable mid-session through a confirmed `set_permission_mode`, a `set_model` route-local model change with confirmed-or-typed-unconfirmed outcome, optional open-time effort selection, and optional provider-owned persistence with replay-free resume and bounded session listing, optional consumer-declared stdio MCP servers with per-server open status and per-call `canUseTool` mediation for SDK-emitted permission requests (default-allowed native reads may complete without that callback; Research 303), caller-deadline-bounded open, turn, interrupt, and close, streamed output and identity-and-lifecycle activity, consumer-mediated tool admission with ordinary tool names and bounded, truncation-flagged Bash views plus intact sidecar input, capability-gated interrupt, and a root-only degraded close that reports Clean only on attested owned-tree emptiness; strict LF JSONL sidecar wire over stdio to the source-tagged Node sidecar driving the official Claude Agent SDK | approved host launch recipe (interpreted Node plus the sidecar entry point) and environment; maintainer-supported `claude/delegated-subscription-auth` profile with the credential held only by the official store | `claude-agent.sdk.package`; qualified exact `0.3.270` one-point claim plus exact native `2.1.270`, Node `22.23.2`, `swallowtail-claude-agent-sdk-jsonl-v1` wire, and sidecar source-tag axes; no unverified-newer posture | `prepare_claude_agent_sdk_session` → `open_session` → `start_turn`; an opt-in persistent profile adds `resume_session`, and the prepared session also exposes bounded `list_sessions`; open verifies wire, behavior, SDK, native binary, Node, the leased cwd and its granted access, the exact admitted tool set, the selected permission mode, and first-party subscription readiness before any provider work; `open_route_session` publishes supported-model evidence and optional effort, adds `set_permission_mode` and `set_model`, and may bind consumer-declared stdio MCP servers with per-server open status under `canUseTool` mediation for SDK-emitted permission requests | `ClaudeAgentSdkDriver`; `InteractiveSessionDriver` plus route-local session listing |
| `claude-code.headless` | `swallowtail-adapter-claude-agent`; `swallowtail.claude-code.headless` | read-only plan-mode one-prompt structured run with usage, reasoning selection, and an optional adapter-local maximum agentic-turn bound; Claude Code stream JSON over stdio | approved `claude` executable and environment; provider-supported local Claude subscription access; explicit ambient harness configuration with no session persistence | `claude-code.headless-stream-json`; `2.1.220..=2.1.281` excluding unpublished `2.1.244`, `2.1.249`, `2.1.253` through `2.1.256`, `2.1.262`, `2.1.264`, and `2.1.279`; plus permitted unverified-newer stable points | `prepare_claude_code_headless` → `prepare_run` → `start_run` | `ClaudeCodeHeadlessDriver`; `StructuredRunDriver` |
| `claude-code.response-only` | `swallowtail-adapter-claude-agent`; `swallowtail.claude-code.response-only` | one tool-free bounded assistant text response with usage and reasoning selection; strict Claude Code stream JSON over stdio | approved `claude` executable and explicit OAuth/keychain environment; provider-supported local Claude subscription access; provider-suppressed tools and MCP, ambient-host isolation, optional host-approved read-only working resource used solely as the native child's project location (v1 absence keeps the inherited working directory; v2 from `2.1.280` uses an adapter-owned empty temporary directory), no session persistence | `claude-code.response-only-stream-json`; qualified `2.1.227..=2.1.278` on `stream-json.v1` and `2.1.280..=2.1.281` on `stream-json.v2` excluding unpublished `2.1.244`, `2.1.249`, `2.1.253` through `2.1.256`, `2.1.262`, `2.1.264`, and `2.1.279`; later stable releases provisionally `UnverifiedNewer`; explicit known-bad deny-list | `prepare_claude_code_response_only` → `prepare_run` → `start_run` | `ClaudeCodeResponseOnlyDriver`; `StructuredRunDriver` |
| `cursor-agent.catalogue` | `swallowtail-adapter-cursor`; `swallowtail.cursor-agent.catalogue` | authenticated model catalogue; Cursor models stdio | approved executable and environment; provider-supported delegated Cursor subscription login | `cursor-agent.release-date`; exact `2026.07.01-41b2de7`, `2026.07.23-e383d2b`, `2026.08.04-aaa8809`, `2026.08.11-e8db854`, `2026.08.31-4057e58`, `2026.09.02-c22c1a3`, `2026.09.10-fd3934a`, `2026.09.15-d2fe57e`, and `2026.09.18-9a7762b`; no inferred gap; later dates permitted as visible unverified newer | `prepare_cursor(Catalogue)` → `prepare_catalogue` → `list_models` | `CursorCatalogueDriver`; `ModelCatalogDriver` |
| `cursor-agent.acp` | `swallowtail-adapter-cursor`; `swallowtail.cursor-agent.acp` | durable interactive session; ACP v1 stdio | approved executable and environment; provider-supported delegated Cursor subscription login; explicit ambient read-write workspace authority | `cursor-agent.release-date`; exact `2026.07.01-41b2de7`, `2026.07.23-e383d2b`, `2026.08.04-aaa8809`, `2026.08.11-e8db854`, `2026.08.31-4057e58`, `2026.09.02-c22c1a3`, `2026.09.10-fd3934a`, `2026.09.15-d2fe57e`, and `2026.09.18-9a7762b`; no inferred gap; later dates permitted as visible unverified newer | `prepare_cursor(Acp)` → `prepare_session` → `open_session` | `CursorAcpDriver`; `InteractiveSessionDriver` |
| `cursor-agent.headless` | `swallowtail-adapter-cursor`; `swallowtail.cursor-agent.headless` | explicit-model one-prompt structured run with optional typed bracket model parameters (`fast`, `context`, `effort`), qualified high-effort `ReasoningSelection` on named Opus tuples only, assistant, thinking, tool, result, and usage events; Cursor stream JSON over stdio | approved executable and environment; provider-supported delegated Cursor subscription login; explicit read-only plan mode or read-write authority; optional closed Cursor-local Ask read mode on read authority and exactly qualified releases only, at qualified dispatch without an enforced read-only boundary; optional sandbox not selected | `cursor-agent.release-date`; exact `2026.07.01-41b2de7`, `2026.07.23-e383d2b`, `2026.08.04-aaa8809`, `2026.08.11-e8db854`, `2026.08.31-4057e58`, `2026.09.02-c22c1a3`, `2026.09.10-fd3934a`, `2026.09.15-d2fe57e`, and `2026.09.18-9a7762b`; no inferred gap; later dates permitted as visible unverified newer | `prepare_cursor(Headless)` → `prepare_run` → `start_run` | `CursorHeadlessDriver`; `StructuredRunDriver` |
| `gemini-cli.acp` | `swallowtail-adapter-gemini`; `swallowtail.gemini.acp` | interactive session with negotiated model options, optional exact plan-mode selection on the read-only profile, bounded host text-write callbacks, and one consumer-supplied streamable-HTTP MCP entry emitted under a route-owned name (emission only; honouring gated on a live probe); ACP v1 stdio | approved executable; provider-supported Gemini Developer API-key profile | `gemini-cli.acp-agent`; maintained `0.51.0..=0.59.0` plus permitted unverified-newer stable points | `prepare_gemini_acp` → `prepare_session` → `open_session`; read options from the authorized handle | `GeminiAcpDriver`; `InteractiveSessionDriver` |
| `gemini-cli.headless` | `swallowtail-adapter-gemini`; `swallowtail.gemini.headless` | one-prompt structured run with usage; stream-json stdio | approved executable and environment; provider-supported Gemini Developer API-key profile; explicit ambient harness configuration and durable transcript retention | `gemini-cli.headless-stream-json`; maintained `0.51.0..=0.59.0` plus permitted unverified-newer stable points | `prepare_gemini_cli(Headless)` → `prepare_run` → `start_run` | `GeminiHeadlessDriver`; `StructuredRunDriver` |
| `grok-build.acp` | `swallowtail-adapter-grok`; `swallowtail.grok-build.acp` | operation-private structured run and interactive session with session-negotiated model options; optional exact consumer-mediated one-shot permissions on interactive sessions; ACP v1 stdio | approved executable and environment; provider-supported delegated Grok subscription OAuth activated only through the advertised `cached_token` method | `grok-build.executable`; deprecated `0.2.114..=0.2.117`, maintained `1.0.4..=1.0.41` (`grok-4.6`), then permitted unverified-newer stable points; mid-gap `0.2.118..=0.2.121` and unprobed `1.0.0..=1.0.3` remain incompatible | `prepare_grok_build` → run or session profile → `start_run` or `open_session`; opt into `with_consumer_mediated_permissions` on the session profile; read authorized model options from the session handle | `GrokAcpDriver`; `StructuredRunDriver` and `InteractiveSessionDriver` |
| `grok-build.catalogue` | `swallowtail-adapter-grok`; `swallowtail.grok-build.catalogue` | authenticated non-inference model catalogue with ordered exact ids and the source default; Grok `--no-auto-update models` stdio | approved executable and prepared ambient environment; provider-supported delegated Grok subscription OAuth; authentication refresh and bounded catalogue metadata traffic permitted | `grok-build.executable`; exact `1.0.30` under `QualifiedOnly`; every older or newer point fails closed | `prepare_grok_build` → `prepare_catalogue` → `list_models` | `GrokCatalogueDriver`; `ModelCatalogDriver` |
| `kimi-code.acp` | `swallowtail-adapter-kimi`; `swallowtail.kimi.acp` | resource-scoped provider-session catalogue/import plus persistent interactive session with negotiated model options; ACP v1 stdio | approved executable; delegated Kimi membership OAuth reference; explicit opaque state-root identity for catalogue/import | `kimi-code.executable`; exact `0.28.1` plus qualified `0.29.0..=0.38.0` under `QualifiedOnly`; exact `0.39.0` and `0.39.1` are excluded for an uncontained process-authority change that Research 325 proved byte-identical through `0.43.0`; every point above `0.38.0` fails closed, including the published `0.40.0..=0.43.0` gap; ACP does not admit unverified-newer | `prepare_kimi_code(Acp)` → `prepare_session_catalogue`/`prepare_session_import` or `prepare_session` → typed operation; imported bindings then use ordinary `load_session` or `resume_session` | `KimiAcpDriver`; `ProviderSessionCatalogueDriver`, `ProviderSessionImportDriver`, and `InteractiveSessionDriver` |
| `kimi-code.headless` | `swallowtail-adapter-kimi`; `swallowtail.kimi.headless` | one-prompt structured run; stream-json stdio | approved executable and ambient harness configuration; delegated Kimi membership OAuth reference; durable provider retention | `kimi-code.executable`; `0.29.0..=0.32.0` under `kimi.headless.stream-json.v1`; `0.33.0..=0.43.0` under `kimi.headless.stream-json.v2`; the split is the exact point where the default `-p` engine becomes agent-core-v2; synthetic later stables permitted `UnverifiedNewer` on the assessed revision | `prepare_kimi_code(Headless)` → `prepare_run` → `start_run` | `KimiHeadlessDriver`; `StructuredRunDriver` |
...
| `command-code.headless` | `swallowtail-adapter-command-code`; `swallowtail.command-code.headless` | explicit-model read-only structured run or Contract 043 interactive turns with thinking, text, tool, result, and usage events; Command Code AgentEvent NDJSON over stdio | approved executable and environment; provider-supported local Command Code account state without a credential lease; plan-mode read-only working resource; structured `--no-session` or interactive exact private `--resume` | `command-code.npm`; exact `1.65.0`; no unverified-newer execution | `prepare_command_code_headless` → `prepare_run` / `prepare_session` → `start_run` / `open_session`; prepared sessions expose fresh context-losing restoration | `CommandCodeHeadlessDriver`; `StructuredRunDriver` and `InteractiveSessionDriver` |
| `cline.acp` | `swallowtail-adapter-cline`; `swallowtail.cline.acp` | one bounded interactive session; ACP v1 stdio; optional portable `HarnessMode::Plan` via confirmed `session/set_config_option` `mode=plan` before the first prompt; no portable reasoning selection because exact `3.0.55` parses then discards `--thinking` and hard-codes thinking off | approved executable and environment; provider-supported local Cline account state without a credential lease; explicit working resource | `cline.package`; exact `3.0.55`; no unverified-newer execution | `prepare_cline_acp` → `prepare_session` → `open_session` | `ClineAcpDriver`; `InteractiveSessionDriver` |
| `cline.headless` | `swallowtail-adapter-cline`; `swallowtail.cline.headless` | one-prompt structured run with optional portable `HarnessMode::Plan` as canonical `--plan`; envelope NDJSON over stdio; no portable reasoning selection because exact `3.0.55` model-entitles and may normalize the value while this route selects no provider/model and gets no applied-tier acknowledgement | approved executable and environment; provider-supported local Cline account state without a credential lease; explicit working resource and host process deadline | `cline.package`; exact `3.0.55`; no unverified-newer execution | `prepare_cline_headless` → `prepare_run` → `start_run` | `ClineHeadlessDriver`; `StructuredRunDriver` |
| `goose.acp` | `swallowtail-adapter-goose`; `swallowtail.goose.acp` | one bounded interactive session; ACP v1 stdio; typed provider-authentication failures on `session/new` and `session/prompt`; one consumer streamable-HTTP MCP entry under a route-owned name; emission is not honouring | approved executable and environment; provider-supported local Goose config state without a credential lease; explicit working resource | `goose.release`; exact `1.50.1`; no unverified-newer execution | `prepare_goose_acp` → `prepare_session` → `open_session` | `GooseAcpDriver`; `InteractiveSessionDriver` |
| `kiro.acp` | `swallowtail-adapter-kiro`; `swallowtail.kiro.acp` | one bounded interactive session; ACP v1 stdio; one consumer-supplied streamable-HTTP MCP entry under a route-owned name; live honouring stays gated | approved executable and environment; provider-supported local Kiro account state without a credential lease; explicit working resource | `kiro-cli.release`; exact `2.21.4`; no unverified-newer execution | `prepare_kiro_acp` → `prepare_session` → `open_session` | `KiroAcpDriver`; `InteractiveSessionDriver` |
| `deepagents.acp` | `swallowtail-adapter-deepagents`; `swallowtail.deepagents.acp` | one bounded interactive session; ACP v1 stdio | approved executable and environment; provider-supported host-owned Anthropic or OpenAI API keys without a credential lease; explicit working resource | `deepagents-acp.package`; exact `0.1.30`; no unverified-newer execution | `prepare_deepagents_acp` → `prepare_session` → `open_session` | `DeepAgentsAcpDriver`; `InteractiveSessionDriver` |
...
| `opencode.acp` | `swallowtail-adapter-opencode`; `swallowtail.opencode.acp` | one bounded interactive session; ACP v1 stdio on `opencode acp --pure`; one consumer MCP entry, stdio or streamable-HTTP, under a route-owned name; streamable-HTTP honouring accepted on exact `1.18.18`; host default agent model unverified | approved executable and environment; provider-supported host-owned OpenCode login without a credential lease; explicit working resource | `opencode.executable`; deprecated `1.18.18..=1.18.30`; maintained `1.18.31..=1.18.32`; later stable points may be visible `UnverifiedNewer` | `prepare_opencode_acp` → `prepare_session` → `open_session` | `OpenCodeAcpDriver`; `InteractiveSessionDriver` |
| `mistral-vibe.headless` | `swallowtail-adapter-mistral-vibe`; `swallowtail.mistral-vibe.headless` | one-prompt structured run; streaming NDJSON over stdio; exact caller-decreasing maximum turns `1..=8` with omission fixed at `8` | approved executable and environment; provider-supported local Vibe config state without a credential lease; explicit working resource and host process deadline | `mistral-vibe.release`; exact `2.25.4`; no unverified-newer execution | `prepare_mistral_vibe_headless` → `prepare_run` → `start_run` | `MistralVibeHeadlessDriver`; `StructuredRunDriver` |
| `qoder.headless` | `swallowtail-adapter-qoder`; `swallowtail.qoder.headless` | one-prompt structured run; stream-json stdio; adapter-owned fixed `--max-turns 8` AgentLoop ceiling | approved executable and environment; provider-supported local Qoder config state without a credential lease; explicit working resource and host process deadline | `qoder.package`; exact `1.1.54`; no unverified-newer execution | `prepare_qoder_headless` → `prepare_run` → `start_run` | `QoderHeadlessDriver`; `StructuredRunDriver` |
...
| `oh-my-pi.rpc` | `swallowtail-adapter-oh-my-pi`; `swallowtail.oh-my-pi.rpc` | catalogue, one-prompt structured run, and interactive session with exact model/reasoning selection, typed questions, and optional bounded PNG input; negotiated RPC v2 chunked JSONL stdio | approved executable and environment; provider-supported local OMP authentication state; explicit ambient read authority and a provider-suppressed tool set | `oh-my-pi.package`; retained `17.2.9..=17.4.2` (deprecated) and maintained `18.0.0..=18.2.7` segments with separate private behavior revisions; explicit `18.0.2`/`18.1.7` exclusions; incompatible `17.4.3`/`17.4.4`; then permitted visible unverified-newer stable points | `prepare_oh_my_pi_rpc` → catalogue, run, or session profile → `list_models`, `start_run`, or `open_session`; prepared sessions expose fresh context-losing restoration | `OhMyPiRpcDriver`; `ModelCatalogDriver`, `StructuredRunDriver`, and `InteractiveSessionDriver` |
...
| `qwen.headless` | `swallowtail-adapter-qwen`; `swallowtail.qwen.headless` | catalogue, structured run, and turn-scoped interactive continuation; structured CLI stream JSON with one joined child per run or turn; optional portable `HarnessMode::Plan` as `--approval-mode plan` on exact `0.21.15`, `0.22.0`, `0.22.1`, `0.22.2`, and `0.22.3`; exact `0.21.15` reasoning control for `qwen3.8-max` and `qwen3.8-max-preview`; exact adapter-local caller-decreasing turn/tool budgets | approved executable; maintainer-supported `qwen-code/delegated-harness-auth` profile | `qwen-code.package`; deprecated `0.19.11..=0.20.1`; deprecated `0.21.0..=0.21.14`; exact maintained `0.21.15` reasoning-control revision with adapter-local budgets; same-revision maintained `0.22.0..=0.24.2` excluding unpublished `0.22.4` and `0.23.5`; unpublished `0.21.16` incompatible; then permitted unverified-newer stable points | `prepare_qwen_catalogue` → `list_models`, or `prepare_qwen_headless` → run or session profile → `start_run` or `open_session`; later turns privately use the exact observed provider session ID | `QwenHeadlessDriver`; `ModelCatalogDriver`, `StructuredRunDriver`, and `InteractiveSessionDriver` |
...
provider-free surface is requalified. Research 347 live acceptance is bound to
...
| discovery-only (1) | `claude-agent.sdk` | bounded `listSessions` exposes provider session metadata, but the route has no import binding or replay surface; resume requires an independently supplied exact `SessionResumeBinding` |
...
| not applicable (28) | `codex.exec`; `kimi-code.headless`; `claude-code.headless`; `claude-code.response-only`; `cursor-agent.catalogue`; `cursor-agent.headless`; `qwen.headless`; `antigravity.catalogue`; `antigravity.headless`; `gemini-cli.acp`; `grok-build.acp`; `grok-build.catalogue`; `muse-code.headless`; `command-code.headless`; `deepseek-harness.jsonrpc`; `oh-my-pi.rpc`; `anthropic.managed-agent`; `zcode.app-server`; `cline.acp`; `cline.headless`; `goose.acp`; `copilot-cli.acp`; `opencode.acp`; `mistral-vibe.headless`; `qoder.headless`; `kiro.acp`; `deepagents.acp`; `pi.sdk-sidecar` | selected route exposes no reusable external provider-session identity; `pi.sdk-sidecar` qualifies its own Contract 017 load and resume but exposes no foreign-session import surface by design |
...
| `openai.realtime` | `swallowtail-adapter-openai`; `swallowtail.openai.realtime` | realtime media session with exact output-token maximum and session-scoped reasoning effort; WebSocket | approved public Realtime endpoint; OpenAI public API key and pay-as-you-go billing | `openai.realtime-facade`; exact opaque revision `openai-realtime-reasoning-2026-08-27` | `prepare_openai_realtime` → `prepare_realtime_session` → `open_session` or fresh realtime restoration | `OpenAiRealtimeDriver`; `RealtimeMediaSessionDriver` |
...
| `llama-cpp.owned` | `swallowtail-adapter-llama-cpp`; `swallowtail.llama-cpp.owned-b10069-openai-chat` | owned ephemeral serving lifecycle with optional adapter-local `--ctx-size N` and `--reasoning off` dispatch; process plus HTTP/SSE | approved `llama-server` executable and exact GGUF artifact; local unauthenticated compute | `llama.cpp.owned-runtime`; exact opaque b10069/178a6c449 revision | `prepare_llama_cpp_owned` → optional `LlamaCppContextSize` (`1..=2147483647`) and optional `LlamaCppReasoningSelection::Disabled` → `prepare_serving_start` → `start` and returned-handle `stop`; omission preserves no context and no reasoning flag | `LlamaCppOwnedDriver`; `ServingInstanceDriver` |
...
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| `antigravity.catalogue` | `not-applicable` | `no` | `not-applicable` | `not-applicable` | `not-applicable` | `not-applicable` | maintained `1.1.9..=1.2.11`; later stable points remain visible `UnverifiedNewer`; catalogue has no provider session | process and task cleanup only |
| `antigravity.headless` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | maintained `1.1.9..=1.1.17`; `1.1.18..=1.2.11` stay unqualified at the `1.1.22` provider-managed-retry stop; structured runs and exact-id continuation preserve provider state without exposing public session identity | each run or turn joins its process and task; provider state is preserved |
...
| `claude-agent.acp` | `supported` | `yes` | `unsupported` | `unsupported` | `supported` | `ProviderDataDeleted` | `0.53.0..=0.79.0`, excluding unpublished `0.58.0`; later stable releases are visible `UnverifiedNewer` | durable structured runs preserve history; the opt-in temporary profile closes then deletes its operation-private session |
| `claude-agent.sdk` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | qualified exact `0.3.270` on `claude-agent.sdk.package` plus exact native `2.1.270`, qualified Node point `22.23.2`, wire, and sidecar source-tag points; a newer Node that passes the sidecar floor may open only as observed `UnverifiedNewer` with its version recorded; provider persistence is opt-in, and retained sessions are resumable without replay while listing remains metadata-only | one caller cleanup deadline bounds turn resolution, interruption, close, escalation, root join, and both lease releases; the sidecar joins its retained native handle to a declared 2000 ms bound and the host makes the declared descendant termination attempt; `OwnedTreeEmpty` alone reports `Clean`, confirmed root-only completion is the accepted `Degraded` posture, and an observed survivor or unconfirmed root exit is cleanup failure |
| `claude-code.headless` | `not-applicable` | `no` | `not-applicable` | `not-applicable` | `not-applicable` | `not-applicable` | qualified `2.1.220..=2.1.281` excluding unpublished `2.1.244`, `2.1.249`, `2.1.253` through `2.1.256`, `2.1.262`, `2.1.264`, and `2.1.279`; later stable releases may be visible `UnverifiedNewer`; every run disables session persistence | process and task cleanup only; no provider session is retained |
| `claude-code.response-only` | `not-applicable` | `no` | `not-applicable` | `not-applicable` | `not-applicable` | `not-applicable` | qualified `2.1.227..=2.1.278` on `stream-json.v1` and `2.1.280..=2.1.281` on `stream-json.v2` excluding unpublished `2.1.244`, `2.1.249`, `2.1.253` through `2.1.256`, `2.1.262`, `2.1.264`, and `2.1.279`; later stable releases run provisionally unless explicitly denied; every run disables session persistence and exposes no reusable provider identity | process and task cleanup only; no provider session is retained |
| `cursor-agent.catalogue` | `not-applicable` | `no` | `not-applicable` | `not-applicable` | `not-applicable` | `not-applicable` | exact qualified `2026.07.01-41b2de7`, `2026.07.23-e383d2b`, `2026.08.04-aaa8809`, `2026.08.11-e8db854`, `2026.08.31-4057e58`, `2026.09.02-c22c1a3`, `2026.09.10-fd3934a`, `2026.09.15-d2fe57e`, and `2026.09.18-9a7762b`; no inferred gap; later dates remain visible `UnverifiedNewer`; catalogue has no provider session | process cleanup only |
| `cursor-agent.acp` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | exact qualified `2026.07.01-41b2de7`, `2026.07.23-e383d2b`, `2026.08.04-aaa8809`, `2026.08.11-e8db854`, `2026.08.31-4057e58`, `2026.09.02-c22c1a3`, `2026.09.10-fd3934a`, `2026.09.15-d2fe57e`, and `2026.09.18-9a7762b`; no inferred gap; later dates remain visible `UnverifiedNewer`; load suppresses history-read and per-turn replay failures, so advertisement cannot qualify continuation recovery or lifecycle management | connection, resource, process, and task cleanup only; provider session state is preserved |
| `cursor-agent.headless` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | exact qualified `2026.07.01-41b2de7`, `2026.07.23-e383d2b`, `2026.08.04-aaa8809`, `2026.08.11-e8db854`, `2026.08.31-4057e58`, `2026.09.02-c22c1a3`, `2026.09.10-fd3934a`, `2026.09.15-d2fe57e`, and `2026.09.18-9a7762b`; no inferred gap; later dates remain visible `UnverifiedNewer`; durable provider state exposes no public session identity | process and task cleanup only; provider state is preserved |
| `gemini-cli.acp` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | maintained `0.51.0..=0.59.0`; later stable points may be visible `UnverifiedNewer`; selected ACP route advertises no close or delete | connection and process cleanup only; provider state is preserved |
| `gemini-cli.headless` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | maintained `0.51.0..=0.59.0`; delete exit and provider prose cannot confirm removal, while `--list-sessions` may invoke summary inference and mutate history | durable runs preserve the transcript; the opt-in owned cleanup profile issues one joined delete attempt and reports removal unconfirmed without listing |
| `kimi-code.acp` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | exact qualified `0.28.1` plus `0.29.0..=0.38.0` under `QualifiedOnly`; exact `0.39.0` and `0.39.1` are incompatible because the agent-core-v2 ACP terminal runner replaces fail-closed errors with an uncontained local host-process spawn under the `terminal: false` capabilities this route advertises, and Research 325 proved that surface byte-identical through `0.43.0`; every point above `0.38.0` fails closed; selected ACP route supports separate catalogue/import; initialize may advertise close/delete, which stay unmapped | catalogue/import joins its process and releases leases; interactive connection cleanup preserves provider state |
...
| `kimi-code.local-server` | `supported` | `yes` | `supported` | `supported` | `unsupported` | `unsupported` | exact qualified `0.28.1` plus `0.29.0..=0.39.1` under `QualifiedOnly`; profile and disabled-tool controls require `0.29.0`; `0.31.0` has a distinct subagent-status revision, `0.31.1` a refresh-stable revision, `0.32.0..=0.34.0` optional-meta-flags, and `0.35.0..=0.39.1` heartbeat-ping; every point above `0.39.1` fails closed, including the published `0.40.0..=0.43.0` gap where the Bash `cwd` workspace assertion is gone | joins WebSocket and task work, answers application ping with pong, releases the server-bearer lease, preserves provider state, and stops only an owned foreground child |
...
| `goose.acp` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | exact `1.50.1`; selected ACP route advertises no close, load, or delete | connection, process, and task cleanup only; provider-owned session state is preserved |
| `kiro.acp` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | exact `2.21.4`; selected ACP route advertises no close, load, or delete | connection, process, and task cleanup only; provider-owned session state is preserved |
| `deepagents.acp` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | exact `0.1.30`; selected ACP route advertises no close, load, or delete; in-process sessions are not durable across restart | connection, process, and task cleanup only |
...
| `opencode.acp` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | deprecated `1.18.18..=1.18.30`; maintained `1.18.31..=1.18.32`; later stable points may be visible `UnverifiedNewer`; cleanup sends `session/close` then joins; no public load, resume, or delete | connection, process, and task cleanup only; provider-owned session state is preserved |
| `mistral-vibe.headless` | `not-applicable` | `no` | `not-applicable` | `not-applicable` | `not-applicable` | `not-applicable` | exact `2.25.4`; one print run and no reusable provider-session identity | process and task cleanup only; local Vibe config state is preserved |
| `qoder.headless` | `not-applicable` | `no` | `not-applicable` | `not-applicable` | `not-applicable` | `not-applicable` | exact `1.1.54`; one print run with an adapter-owned eight-turn ceiling and no reusable provider-session identity | process and task cleanup only; local Qoder config state is preserved |
...
| `oh-my-pi.rpc` | `not-applicable` | `no` | `not-applicable` | `not-applicable` | `not-applicable` | `not-applicable` | qualified `17.2.9..=17.4.2` (retained, deprecated) and `18.0.0..=18.2.7` (maintained) with explicit `18.0.2`/`18.1.7` exclusions and incompatible `17.4.3`/`17.4.4`; later stable points may be visible `UnverifiedNewer` | operation process and materialized attachment cleanup only; local OMP auth/config state is preserved |
...
| `qwen.headless` | `not-applicable` | `no` | `not-applicable` | `not-applicable` | `not-applicable` | `not-applicable` | deprecated `0.19.11..=0.20.1` and `0.21.0..=0.21.14`; exact maintained `0.21.15` plus same-revision maintained `0.22.0..=0.24.2` excluding unpublished `0.22.4` and `0.23.5`; unpublished `0.21.16` incompatible; current structured-run operation has no provider session | run and process cleanup only |
| `opencode.http` | `supported` | `yes` | `unsupported` | `unsupported` | `supported` | `ProviderDataDeleted` | `1.14.48..=1.18.31` across exact published segments; later stable points may be visible `UnverifiedNewer` | structured runs delete their operation-private session; interactive attachment cleanup preserves the external server |
...
| `grok-build.catalogue` | `not-applicable` | `no` | `not-applicable` | `not-applicable` | `not-applicable` | `not-applicable` | exact `1.0.30` under `QualifiedOnly`; every older or newer point fails closed; catalogue has no provider session | process and time cleanup only |
| `grok-build.acp` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | deprecated `0.2.114..=0.2.117` plus maintained `1.0.4..=1.0.41`; mid-gap `0.2.118..=0.2.121` incompatible; stripped artifacts and no deterministic load transcript cannot prove complete client-visible replay, so no continuation-recovery, lifecycle, or management mapping is qualified | attachment, process, credential, and working-resource cleanup only; provider-owned session state is preserved |
...
- Installed harnesses:
  ...
  [OpenCode ACP](../../crates/swallowtail-adapter-opencode/examples/prepared_opencode_acp.rs),

use crate::managed::{API_VERSION, BETA_VERSION, Method};
...
fn configure(
    ...
    let credential = std::str::from_utf8(credential).map_err(|_| {
        failure(
            "swallowtail.anthropic.managed.credential_invalid",
            "Anthropic Managed Agents API-key credential was invalid",
        )
    })?;
    ...
    easy.progress_function(move |_, _, _, _| !worker_cancelled.load(Ordering::SeqCst))
        .map_err(curl_failure)?;
    let mut headers = List::new();
    headers
        .append(&format!("x-api-key: {credential}"))
        .map_err(curl_failure)?;
    headers
        .append(&format!("anthropic-version: {API_VERSION}"))
        .map_err(curl_failure)?;
    headers
        .append(&format!("anthropic-beta: {BETA_VERSION}"))
        .map_err(curl_failure)?;
    headers

...
pub(super) fn row_semantic_id(row: &ConsumerRouteProjectionRow) -> String {
    match row.identity() {
        ...
        ConsumerRouteRowIdentity::Control(c) => match c {
            ...
            ConsumerRouteControlId::ReasoningSelection => "control.reasoning-selection".to_string(),
...
pub(super) fn claimed_tuples(
    ...
    tranche
        .iter()
        .filter(|e| e.emitted_by.contains(&profile))
        .map(|e| (e.route_id, e.operation_shape, e.semantic_id.to_string()))
        .collect()
}

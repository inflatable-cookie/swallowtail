...
## Entries
- [2026-09-15 Kimi Code Local Server 0.43.0 Claim](./2026-09-15-kimi-code-local-server-0-43-0-claim.md) — extended `kimi.local-server.executable-window-2` to maintained `0.35.0..=0.39.1` on the unchanged heartbeat-ping revision and changed the posture to `QualifiedOnly`, so the uncontained `0.40.0` Bash `cwd` widening and every later point through `0.43.0` fail closed; ACP and headless untouched

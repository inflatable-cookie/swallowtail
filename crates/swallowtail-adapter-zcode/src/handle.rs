...
pub(crate) struct ZcodeCancellation {
    process: Arc<dyn ProcessHandle>,
    requested: AtomicBool,
}

impl ZcodeCancellation {
...
impl CancellationControl for ZcodeCancellation {
...
pub(crate) struct ZcodeRunHandle {
...
impl RunHandle for ZcodeRunHandle {
        ...
        Box::pin(async move {
            match self.task.join().await {
                ...
                Err(_) => CleanupOutcome::Failed(SafeDiagnostic::new(
                    "swallowtail.zcode.app_server.task_join_failed",
                    "ZCode app-server operation task could not be joined",
                )),
            }
        })
    }

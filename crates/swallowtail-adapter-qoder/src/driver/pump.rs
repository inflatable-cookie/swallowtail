use super::{events::QoderHeadlessEventParser, handle::QoderHeadlessCancellation};
...
const ROUTE: &str = "qoder.headless";
...
pub(super) async fn pump(
    ...
    let mut parser = QoderHeadlessEventParser::new(operation_id);
    ...
    match (parser.finish(), exit) {
        ...
        (_, Err(_)) => {
            let diagnostic = SafeDiagnostic::new(
                "swallowtail.qoder.headless.process_wait_failed",
                "Qoder process wait failed",
            );
...
async fn next_output(
    ...
    cancellation: &QoderHeadlessCancellation,
...
fn event_delivery_failed(cleanup: CleanupOutcome) -> TerminalOutcome {
    TerminalOutcome::new(
        TerminalStatus::RuntimeFailed(SafeDiagnostic::new(
            "swallowtail.qoder.headless.event_delivery_failed",
            "Qoder event delivery failed",
        )),
        cleanup,
    )
}
...
fn process_cleanup_failed() -> CleanupOutcome {
    CleanupOutcome::Failed(SafeDiagnostic::new(
        "swallowtail.qoder.headless.process_cleanup_failed",
        "Qoder process cleanup failed",
    ))
}

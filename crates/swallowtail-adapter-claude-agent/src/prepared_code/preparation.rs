use super::{ClaudeCodePreparationInput, ClaudeCodePreparedIntegration, instance};
...
pub(super) fn validate_input(input: &ClaudeCodePreparationInput) -> Result<(), PreparationFailure> {
    if input.target.version_axis().as_str() != crate::CLAUDE_CODE_HEADLESS_AXIS {
        return Err(preparation_failure(
            PreparationStage::TargetSelection,
            "swallowtail.claude_code.headless.preparation.target_axis_mismatch",
            "Claude Code preparation target uses a different version axis",
        ));
    }
        ...
        || input.access_profile.support_authority() != SupportAuthority::ProviderSupported
    {
        return Err(preparation_failure(
            ...
            "swallowtail.claude_code.headless.preparation.access_profile_rejected",
            "Claude Code headless requires provider-supported local subscription access",
        ));
    }
        ...
        || status.support_authority() != input.access_profile.support_authority()
    {
        return Err(preparation_failure(
            PreparationStage::AccessEvidence,
            "swallowtail.claude_code.headless.preparation.access_evidence_mismatch",
            "Claude Code access evidence does not match the selected local subscription profile",
        ));
    }
    Ok(())
}
...
pub(super) fn promote(
        ...
        || observation.version().axis() != input.target.version_axis()
    {
        return Err(preparation_failure(
            ...
            "swallowtail.claude_code.headless.preparation.observation_mismatch",
            "Claude Code discovery observation does not match the prepared target",
        ));
    }
    let configured = instance::configured_instance(&input, &observation)?;
    Ok(ClaudeCodePreparedIntegration {
...
pub(super) fn discovery_runtime_failure(
    ...
    let stage = match error.diagnostic().code() {
        "swallowtail.claude_code.discovery_axis_mismatch"
...
fn discovery_outcome_failure(outcome: &DiscoveryOutcome) -> PreparationFailure {
    ...
    let diagnostic = outcome.diagnostic().cloned().unwrap_or_else(|| {
        swallowtail_core::SafeDiagnostic::new(
            "swallowtail.claude_code.headless.preparation.discovery_rejected",
            "Claude Code executable discovery was not promotable",
        )
    });

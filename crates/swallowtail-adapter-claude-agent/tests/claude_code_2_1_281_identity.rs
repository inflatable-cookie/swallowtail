...
fn shipped_file_inventory_and_binary_identity_are_frozen() {
    ...
    let wrapper_files = inventory["packages"]["claude-code"]["2.1.281"]["files"]
        .as_object()
        .unwrap();
    assert_eq!(
        wrapper_files.keys().map(String::as_str).collect::<Vec<_>>(),
        [
            "LICENSE.md",
            "README.md",
            "bin/claude.exe",
    ...
    assert_eq!(
        inventory["file_delta"]["claude-code"]["2.1.278_to_2.1.280"]["changed"],
        json!(["package.json"])
    );
    assert_eq!(
        inventory["file_delta"]["claude-code"]["2.1.280_to_2.1.281"]["changed"],
        json!(["package.json", "sdk-tools.d.ts"])
    );
    assert_eq!(
...
fn builtin_hook_set_and_decisive_prompt_context_are_frozen() {
    ...
    for version in ["2.1.280", "2.1.281"] {
        ...
        assert!(
            rows.iter()
                .all(|row| row["registered_in_darwin_arm64"] == true
                    && row["registered_in_linux_x64"] == true)
        );
        for (row, expected_events) in rows.iter().zip(&events) {
            assert_eq!(&row["hook_events"], expected_events);
            assert_eq!(
                row["linux_embedded_module_sha256"].as_str().unwrap().len(),
                64
            );
        }
    }
    assert_eq!(
        hooks["versions"]["2.1.280"][1]["linux_embedded_module_sha256"],
        "fd7b0d390f79bcfb3c49e2594053a202cc38a4f510f8f9d31d40cfe2e53820d3"
    );
    assert_eq!(
        hooks["versions"]["2.1.281"][1]["linux_embedded_module_sha256"],
        "799d2032c65cf657de0076c93ad77cc79db9a08c3237ff1b6fd967f630a1621f"
    );
    let protocol = fixture(PROTOCOL);
    assert_eq!(
        protocol["agents_md_default_enabled"],
        json!({"2.1.280": false, "2.1.281": true})
    );
    assert_eq!(
        protocol["enabled_plugins_builtin_override_from_cli_settings_ignored"],
        true
    );
    assert_eq!(

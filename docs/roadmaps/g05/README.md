...
| Keep production route versions current one family at a time. | g05.016 card 041 froze Kimi Code `0.39.1` identity, proved the headless routing boundary is `0.33.0`, and traced the `0.39.0` ACP process-authority delta to no containment; card 042 has not yet applied the per-axis disposition | Contract 029 identity-before-claim; Research 267-270; current-main validation | `g05.016` cards 041-042 |

## Planned Next Roadmaps

- [g05.016 Kimi Code 0.39.1 Useful Newer](016-kimi-code-0-39-1-useful-newer.md) — ready; Research 270; card 041 froze identity and named a per-axis disposition; card 042 has not applied it
...
g05 has sixteen numbered roadmaps: 10 completed milestones including standing
currentness at 004, 005, 012, 013, 014, and 015 and contract promotion at 008
and 010, honest evidence stops at 002, 003, 007, and 011. Milestone 016 is the
ready standing-currentness milestone. Milestone 009 is planned and
...
Workflow and changed no claim. g05.016 card 041 froze that identity and
recorded a per-axis disposition: the headless agent-core-v2 routing boundary
is `0.33.0`, so `kimi.headless.stream-json.v1` should correct to
`0.29.0..=0.32.0` and `kimi.headless.stream-json.v2` should correct and
qualify `0.33.0..=0.39.1`, while `kimi-code.acp` should stop at `0.38.0` and
exclude exact `0.39.0` and `0.39.1` because the agent-core-v2 ACP terminal
runner spawns uncontained local host processes under the `terminal: false`
capabilities the route advertises. Card 042 owns that claim edit.
`kimi-code.local-server` stays a separate family.

## Milestones

- [016 Kimi Code 0.39.1 Useful Newer](./016-kimi-code-0-39-1-useful-newer.md) — ready; Research 270; identity frozen at card 041; card 042 owns the claim

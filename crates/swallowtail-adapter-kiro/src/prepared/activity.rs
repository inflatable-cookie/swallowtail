...
pub(super) fn profile(
    ...
    let behavior = match observation.compatibility() {
        ...
        InstalledExecutableCompatibility::Incompatible => {
            return Err(failure(
                "swallowtail.kiro.acp.preparation.activity_version_incompatible",
                "Kiro ACP activity requires the exact permitted release",
            ));
        }
    };
    ...
    .map_err(|_| {
        failure(
            "swallowtail.kiro.acp.preparation.activity_profile_invalid",
            "Kiro ACP activity profile could not be derived",
        )
    })
}
...
pub(super) fn with_activity(
    ...
    requirements.push(
        activity
            ...
            .expect("prepared Kiro activity is available"),
    );

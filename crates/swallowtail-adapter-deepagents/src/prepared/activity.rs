...
pub(super) fn profile(
    ...
    let behavior = match observation.compatibility() {
        ...
        InstalledExecutableCompatibility::Incompatible => {
            return Err(failure(
                "swallowtail.deepagents.acp.preparation.activity_version_incompatible",
                "Deep Agents ACP activity requires the exact permitted release",
            ));
        }
    };
    ...
    .map_err(|_| {
        failure(
            "swallowtail.deepagents.acp.preparation.activity_profile_invalid",
            "Deep Agents ACP activity profile could not be derived",
        )
    })
}
...
pub(super) fn with_activity(
    ...
    requirements.push(
        activity
            ...
            .expect("prepared Deep Agents activity is available"),
    );

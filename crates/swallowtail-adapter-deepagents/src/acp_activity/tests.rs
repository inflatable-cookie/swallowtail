...
#[test]
fn exact_deepagents_thought_classification_and_tools_remain_distinct() {
    let mut projection =
        AcpActivityProjection::new(RuntimeTurnId::new("deepagents-activity-turn").unwrap());
    let updates = [
        json!({"sessionUpdate":"agent_thought_chunk","content":{"type":"text","text":"Inspecting"}}),
        json!({"sessionUpdate":"agent_message_chunk","content":{"type":"text","text":"Deep Agents display text."}}),
        ...
        json!({"sessionUpdate":"future_deepagents_activity","private":"excluded"}),
        json!({"sessionUpdate":"usage_update","used":1,"size":1}),
    ];
...
fn noncanonical_tool_lifecycles_degrade_without_failure() {
    let mut projection =
        AcpActivityProjection::new(RuntimeTurnId::new("deepagents-tool-lifecycle-turn").unwrap());
    let updates = [

...
rustup run "$SWALLOWTAIL_GENERAL_MSRV" cargo check \
  --workspace --all-targets --locked \
  --exclude swallowtail-adapter-bedrock

rustup run "$SWALLOWTAIL_BEDROCK_MSRV" cargo check \
  --package swallowtail-adapter-bedrock --all-targets --locked

rustup run "$SWALLOWTAIL_CURRENT_STABLE" cargo check \
  --workspace --all-targets --locked

printf 'MSRV and current-stable checks passed\n'

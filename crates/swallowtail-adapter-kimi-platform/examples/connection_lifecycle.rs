...
use swallowtail_adapter_kimi_platform::{
    ...
    KIMI_PLATFORM_ENDPOINT_AUDIENCE, KIMI_PLATFORM_PROVIDER_ID, KimiPlatformPreparationInput,
...
fn admit_hosted_kimi_platform(
    ...
    admit_instance(
        ...
        .with_config_refs([(
            ConfigFieldId::new(KIMI_PLATFORM_CHAT_ENDPOINT_FIELD_ID).expect("config id is valid"),
...
fn collect_api_key(
    ...
    let mut session = start_sign_in(
        ...
        swallowtail_runtime::SignInStartRequest::new(
            ScopeId::new("kimi-platform.lifecycle.sign-in").expect("scope is valid"),
            admitted.id().clone(),
...
fn refresh_hosted_kimi_platform(
    store: &MemoryConnectionLifecycleStore,
    instance_id: ConfiguredInstanceId,

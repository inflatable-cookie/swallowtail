...
fn a_stalled_open_returns_on_the_deadline_against_the_real_local_task_host() {
    // The same adversarial open, but the guardian runs on the real
    // `LocalHostServices` task lifecycle rather than the fixture's cooperative
    // task seam. That host's handle owns its worker thread: joining it blocks,
    // and so does dropping it. If the route ever waits on a join instead of the
    // finished observation, this public operation stops returning inside its
    // deadline.

...
#[test]
fn exact_thought_classification_and_tools_remain_distinct() {
    let mut projection =
        AcpActivityProjection::new(RuntimeTurnId::new("opencode-acp-activity-turn").unwrap());
    let updates = [
        json!({"sessionUpdate":"agent_thought_chunk","content":{"type":"text","text":"Inspecting"}}),
        json!({"sessionUpdate":"agent_message_chunk","content":{"type":"text","text":"OpenCode display text."}}),
        ...
        json!({"sessionUpdate":"opencode_future_activity","private":"excluded"}),
...
fn noncanonical_tool_lifecycles_degrade_without_failure() {
    let mut projection =
        AcpActivityProjection::new(RuntimeTurnId::new("opencode-acp-tool-lifecycle-turn").unwrap());
    let updates = [

fn validate_open(
    ...
    for (present, code, message) in [
        (
            ...
            "swallowtail.goose.acp.task_service_missing",
            "Goose ACP requires a scoped task service",
        ),
        (
            services.process().is_some(),
            "swallowtail.goose.acp.process_service_missing",
            "Goose ACP requires a process service",
        ),
        (
            services.working_resource().is_some(),
            "swallowtail.goose.acp.resource_service_missing",
            "Goose ACP requires a working-resource service",
        ),
    ] {
        if !present {
    ...
    if request.access_policy() != &SessionAccessPolicy::ambient_harness(resource_access) {
        return Err(failure(
            "swallowtail.goose.acp.access_policy_rejected",
            "Goose ACP requires exact ambient working-resource access",
        ));
    }
...
fn validate_initialize(
    ...
    if info.get("name").and_then(Value::as_str) != Some("goose")
        || info.get("version").and_then(Value::as_str) != Some(selected_version.as_str())
    {
        return Err(failure(
            "swallowtail.goose.acp.agent_version_rejected",
            "Goose ACP identity does not match the preflight version",
        ));
    }
    Ok(())
}
...
fn session_resource_access(plan: &PreflightPlan) -> Result<ResourceAccess, RuntimeFailure> {
    plan.requirements()
        ...
        .ok_or_else(|| {
            failure(
                "swallowtail.goose.acp.resource_access_missing",
                "Goose ACP requires explicit working-resource access",
            )
        })
}

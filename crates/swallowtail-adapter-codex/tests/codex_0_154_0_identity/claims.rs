...
#[test]
fn production_exec_claim_still_ends_at_0_152_1_before_the_claim_card() {
    let exec = codex_exec_claim();
    let qualified = exec.assess(&version("0.152.1"));
    let InterfaceCompatibilityAssessment::Qualified(matched) = qualified else {
        panic!("0.152.1 must stay qualified before the claim card");
    };
    ...
    for hop in [
        ...
        let InterfaceCompatibilityAssessment::UnverifiedNewer(unverified) =
            exec.assess(&version(hop))
        else {
            panic!("{hop} must stay unverified newer before the claim card");
        };
    ...
    let InterfaceCompatibilityAssessment::UnverifiedNewer(unverified_152_2) =
        exec.assess(&version("0.152.2"))
    else {
        panic!("0.152.2 must stay unverified newer before the claim card");
    };
    assert_eq!(unverified_152_2.latest_qualified().as_str(), "0.152.1");
...
#[test]
fn production_app_server_claim_still_ends_at_0_152_1_before_the_claim_card() {
    let app_server = codex_app_server_claim();
    let qualified = app_server.assess(&version("0.152.1"));
    let InterfaceCompatibilityAssessment::Qualified(matched) = qualified else {
        panic!("0.152.1 must stay qualified before the claim card");
    };
    assert_eq!(

...
struct DeepAgentsSessionHandle {
...
impl InteractiveSessionHandle for DeepAgentsSessionHandle {
    ...
    fn start_turn<'a>(
        ...
        Box::pin(async move {
            ...
            let scope = ScopeId::new(format!("deepagents-acp:turn:{}", request.turn_id().as_str()))
                .map_err(|_| malformed())?;
            ...
            *self.active.lock().expect("ACP active-task lock poisoned") = Some(ActiveTask {
            ...
            Ok(Box::new(DeepAgentsTurnHandle {
            ...
            let task_cleanup = match self.pump_task.take() {
                Some(task) => match task.join().await {
                    ...
                    Err(_) => CleanupOutcome::Failed(swallowtail_core::SafeDiagnostic::new(
                        "swallowtail.deepagents.acp.task_join_failed",
                        "Deep Agents ACP protocol task did not join cleanly",
                    )),
                },
            ...
            let resource_cleanup = match self.resource.take() {
                Some(resource) => match self.services.working_resource() {
                    ...
                    None => CleanupOutcome::Failed(swallowtail_core::SafeDiagnostic::new(
                        "swallowtail.deepagents.acp.resource_release_failed",
                        "Deep Agents ACP working-resource service disappeared during cleanup",
                    )),
                },
                None => CleanupOutcome::NotApplicable,
            };

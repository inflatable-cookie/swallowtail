...
versions. Keep distribution membership, host configuration, model visibility,
process authority, activity projection, turn completion, feature lifecycle,
and compatibility evidence distinct.

g05 does not make Swallowtail a shell, daemon manager, arbitrary process
controller, ambient skill registry, or consumer UI. Prompt text is not
enforcement.
...
| Keep production route versions current one family at a time. | active campaign; eighteen families processed through Research 326 and g05.078 complete; Goose stopped at the selected-route provider-auth failure change and keeps exact `1.46.0` pending an operator ruling; Kiro advanced to exact `2.21.4`; Mistral Vibe advanced to exact `2.25.4` with an adapter-private `--legacy-harness` argv pin; Deep Agents advanced to exact `0.1.30` with the stdio wire unchanged; Antigravity advanced the catalogue claim to `1.2.2` and stopped headless at the `1.1.22` provider-retry stop under the release-notes authority ruling; Gemini advanced both the ACP and headless claims to maintained `0.51.0..=0.59.0` with the selected mapped sources byte-identical; Kimi installed headless advanced to maintained `0.33.0..=0.43.0` while ACP held the A2 `0.38.0` cap; Kimi local server extended the safe prefix to maintained `0.35.0..=0.39.1` and made uncontained `0.40.0..=0.43.0` fail closed under `QualifiedOnly`; Command Code's separate live requalification remains named in g05.069; Oh My Pi `18.1.22` is ready as the final campaign family | Contract 029; Research 217 and 308-326; Tom's 2026-09-14 and 2026-09-15 directions | g05.079 |
...
| Reconcile the missing Claude SDK consumer editing evidence without repeating provider work. | completed; PR 308 merged at `05f1a3a1` after independent exact-head review `5610583309` with no follow-ups; native mediation limitation recorded provider-free | stopped g05.029 audit; released `v0.4.4`; Desktop queue task `bc4acd98-91a3-4e14-86e0-38376b037da7`; 2026-09-10 operator disposition | g05.045 |
...
- [g05.079 Oh My Pi 18.1.22 Major-Line Qualification](./079-oh-my-pi-18-1-22-major-line-qualification.md) — ready; freeze every published stable after `17.4.0`, settle `17.4.1`/`17.4.2` independently, and compile any admitted 18.x range as a distinct private behavior segment
- [g05.078 Kimi Code 0.43.0 Local-Server Containment](./078-kimi-code-0-43-0-local-server-containment.md) — complete; Research 326 extended the safe local-server prefix through `0.39.1` and changed the posture to `QualifiedOnly`, so uncontained `0.40.0..=0.43.0` fails closed
- [g05.077 Kimi Code 0.43.0 Installed Useful Newer](./077-kimi-code-0-43-0-installed-useful-newer.md) — complete; Research 325 froze official npm/GitHub `0.39.1`, `0.40.0`, `0.40.1`, `0.41.0`, `0.42.0`, and `0.43.0` with one deterministic tagged-source inventory per point and a mutation-sensitive selected-file ledger; the `dispatchNativeEvent` switch, `prompt-render.ts`, and `options.ts` are unchanged across all six points, so headless advanced to maintained `0.33.0..=0.43.0` with the JSONL grammar, retry record, terminal shape, retention, cancellation, and cleanup unchanged, while ACP held the A2 `0.38.0` cap because `acpTerminalRunner.ts` and the bundled `AcpProcessService` are byte-identical at every point and `0.40.0..=0.43.0` stays the posture-rejected gap
- [g05.076 Gemini CLI 0.59.0 Useful Newer](./076-gemini-cli-0-59-0-useful-newer.md) — complete; Research 324 froze official npm/GitHub `0.56.0`, `0.57.0`, `0.58.0`, and `0.59.0` with one deterministic tagged-source inventory per point; no ACP source changed, every selected stream-json, terminal, exit-code, option, and retention source is unchanged, and only `geminiChat.ts` provider-request internals and the worktree git helper moved, so both claims advanced to maintained `0.51.0..=0.59.0` with workspace trust, policy checkers, sandbox, and ambient MCP unmapped
...
g05 has seventy-nine numbered tasks: 60 complete and one ready, including
...
g05.067 is complete: PR #329 merged at `3a519b09`; Research 316 reproduced the Research 314
platform and executable identity for `1.0.25..=1.0.30`, proved the catalogue
argv, preamble, bullet grammar, module path, and embedded default-model
document identical at every hop, and advanced the exact `QualifiedOnly`
catalogue point to `1.0.30` with the accepted prompt-free capsule; the Grok
ACP window and registered-tool courier stay independent;
...
g05.076 is complete: Research 324 froze official npm/GitHub identity for
`0.56.0`, `0.57.0`, `0.58.0`, and `0.59.0` with one deterministic
tagged-source inventory per point, found no changed path under
`packages/cli/src/acp/**` and no selected stream-json, terminal, exit-code,
option, or retention change, and advanced both separate claims to maintained
`0.51.0..=0.59.0` with workspace-trust fail-closing, the policy
safety-checker declaration, macOS Seatbelt sandboxing, and the MCP OAuth SSRF
repair unmapped;
g05.077 is complete: Research 325 froze official npm/GitHub identity for
`0.39.1`, `0.40.0`, `0.40.1`, `0.41.0`, `0.42.0`, and `0.43.0` with one
deterministic tagged-source inventory per point and a mutation-sensitive
selected-file ledger, advanced the Kimi Code installed headless claim to
maintained `0.33.0..=0.43.0` with the selected JSONL grammar, retry record,
terminal shape, retention, cancellation, and cleanup unchanged, and held
`kimi-code.acp` at the A2 `0.38.0` cap because `acpTerminalRunner.ts` and the
bundled `AcpProcessService` are byte-identical through `0.43.0` so
`0.40.0..=0.43.0` stays the posture-rejected published gap;
g05.078 is complete: Research 326 froze official npm/GitHub identity for
every published stable `0.38.0..=0.43.0` with a mutation-sensitive
thirteen-file local-server ledger, extended the separate local-server claim
to maintained `0.35.0..=0.39.1` with the Bash workspace assertion intact at
both `0.39.x` points, and changed the posture to `QualifiedOnly` because the
uncontained `0.40.0` `cwd` widening persists byte-identical through `0.43.0`
so `0.40.0..=0.43.0` is the posture-rejected published gap;
...
post-card-020 recommendation. Spec 012 promoted the reviewed 767-row census
into one composing-contract lane. g05.008 card 021 closed that lane: Contract
061 is active, Spec 012 is archived, and the contract front door, index, and
summaries agree. Contract 061 keeps the three projection views and per-turn
truth distinct, binds one immutable snapshot to exact instance revision, route,
model, operation shape, and source evidence identity, preserves source
availability dimensions plus bounded safe reasons without an exhaustive
taxonomy, and names four fail-closed points. Contracts 037, 047, and 057 are
unchanged.

The post-Contract-061 reassessment first classified g05 as `strict-paused`.
Qoder skill visibility still lacks positive evidence, the watcher lanes remain
evidence stops, and bounded consumer-facing watcher observation still depends
on a successful route proof. Structural closeout is not automatic at eight or
nine numbered roadmaps. The operator selected a planning-only Contract 061
...
`session.updated` acknowledgement on the existing generic open path. The
operator approved one additive adapter-owned typed prepared-open result while
...
Candidate A passed the first full readiness rubric. Card 023 completed the
remaining 35 `codex.exec` and 24 `openai.background` rows through PR 133 at
`58be7122`. Its exact-head review restored full tuple identity, cross-access
counterexamples, and descriptor-only prepared activity state. The post-card-023
reassessment corrected the Codex output-token row as descriptor-only negative
coverage and promoted candidate H alone. Card 024 completed the exact 9-row
Deep Agents, 9-row Kiro, 8-row Qoder, and 12-row ZCode package proof through
PR 138 at `8b295e6b`. Candidates B-G and I-L initially remained without
implementation authority. Planning-only card 030 then audited acknowledgement candidates D, F,
and G against current `main` and promoted none. All three exact active-session
acknowledgement routes — `claude-agent.acp`, `kimi-code.acp`, and `cline.acp` —
validate their provider confirmation and discard it, so rubric item 2 has no
active-observation facade to name and the census `rejected` state has no exact
retained value. `openai.realtime` differs only because card 022 added its
acknowledgement retention and additive open seam under the operator decision
the Batch 9.1 gate recorded for that route alone. Candidate D is the narrowest
unblock — one adapter package, one acknowledgement row, no second post-open
observation family — so its gate is scoped to `claude-agent.acp` alone:
adapter-local exact effective and rejected retention plus one additive
adapter-owned open-with-projection outcome and failure preserving the existing
`open_session`. The operator accepted both points. The gate fixes the exact
...
Research 308 family through official Qwen Code `0.23.3` with Research 310;
selected headless surfaces remain compatible, the exact Plan set remains
bounded at `0.22.3`, and exact reasoning/budgets remain `0.21.15`. g05.061
then closed the third Research 308 family through official Codex `0.154.0`
...
feature-specific sets stay on their probed points. g05.063 then closed the
fifth Research 308 family at official Ollama `0.34.0` with Research 313;
the attached-runtime claim qualifies `0.33.0` through `0.33.2` while
`0.33.3` and `0.34.0` stay visibly unverified on the named strict-decoder
cached-count stop with a decoder-tolerance follow-up, and historical
exclusions, claim identity, and behavior revision stay. g05.064 then closed
the sixth Research 308 family at official Grok Build ACP `1.0.30` with
Research 314; the ACP window extends `1.0.4..=1.0.30` as a compatible
extension with a byte-identical mapped literal presence map, one unread
model-document metadata delta, and a persistent mapped ACP module core, while
the exact `1.0.25` catalogue claim and the `1.0.4`/`1.0.5` registered-tool
courier stay independently bounded and alpha `1.0.31` plus unpublished
`1.0.32` stay unverified. g05.067 then closed the eighth Research 308 family
at the separate Grok Build catalogue exact point with Research 316; the
catalogue argv grammar, authentication preamble, shipped bullet grammar,
`xai-grok-pager/src/models.rs` module path, and embedded default-model
document are identical at every published stable from `1.0.25` through
`1.0.30`, so the exact `QualifiedOnly` claim
`grok-build.catalogue.executable-1-0-30` keeps the
`grok-build.catalogue.models-text-v1` behavior revision and rejects every
other point, with the prompt-free `1.0.30` observation capsule frozen. The
Grok ACP window and the `1.0.4`/`1.0.5` registered-tool courier stay
independent.
...
reserved MCP family. Card 029 then rejected every exact `2.1.251` isolation
shape: none preserves authentication plus private watcher composition while
excluding ambient authority. PR 135 merged the evidence stop without a
production behavior change. A separate instruction-delivery mechanism now
needs planning before any new live turn. g05.009 completed card 024 and
prioritizes acknowledgement and per-turn candidates before breadth-only
packages; card 030 closed the D/F/G reassessment as an evidence stop. The persistent Contract 029
lane selected Codex `0.151.0` first.
...
Card 025 promoted Contract 062 without Rust or a production claim. Cards 026
and 029 closed without a production behavior change. Card 024 merged through
PR 138 at `8b295e6b`. Card 030 closed without promoting a candidate or changing
...
Upgrade Workflow and changed no claim. Operator restart after official
latest moved to `0.73.0`. Research 272 and cards 044-045 then qualify
...
canonical `main`. Card 051's coordinated candidate is complete: PR 201 exact
...
rather than joining it. The route is restored and merged through PR 196 at
`493f8194`. ACP remains an independently
useful portable route rather than an approximation hidden behind the SDK
identity.

## Tasks

- [079 Oh My Pi 18.1.22 Major-Line Qualification](./079-oh-my-pi-18-1-22-major-line-qualification.md) — ready; nineteenth and final Research 308 family; classify the two later 17.x points and the complete 18.x published ledger, with a distinct private behavior segment required across the major boundary
- [078 Kimi Code 0.43.0 Local-Server Containment](./078-kimi-code-0-43-0-local-server-containment.md) — completed; eighteenth Research 308 family; Research 326 froze the `0.38.0..=0.43.0` local-server ledger, the maintained heartbeat-ping segment extended to `0.35.0..=0.39.1`, and the `QualifiedOnly` posture makes every point above `0.39.1` fail closed
- [077 Kimi Code 0.43.0 Installed Useful Newer](./077-kimi-code-0-43-0-installed-useful-newer.md) — completed; seventeenth Research 308 family; official identity and selected-source ledgers advanced headless to `0.43.0` while ACP retained the A2 `0.38.0` cap
- [076 Gemini CLI 0.59.0 Useful Newer](./076-gemini-cli-0-59-0-useful-newer.md) — completed; sixteenth Research 308 family; the exact npm/GitHub `0.56.0..=0.59.0` identity and deterministic tagged-source ledger advanced both the ACP and headless claims to maintained `0.51.0..=0.59.0` with no selected mapped-file change
...
- [066 Claude Agent SDK 0.3.270 Registered-Tool Live Requalification](./066-claude-agent-sdk-0-3-270-registered-tool-live-requalification.md) — planned; separately authorized live gate on the rebound tuple; awaiting Chatterbox compilation and queue dispatch
...
- [063 Ollama 0.34.0 Useful Newer](./063-ollama-0-34-0-useful-newer.md) — completed; PR #326 merged at `c0c14864`; lifecycle closeout `eeac1a61`; Research 313; qualified official `0.33.0` through `0.33.2` with named `0.33.3` decoder stop
- [062 Cursor Agent 2026.09.10 Useful Newer](./062-cursor-agent-2026-09-10-useful-newer.md) — completed; PR #325 merged at `de177ce0`; Research 312; three routes qualified through exact `2026.09.10-fd3934a`; lifecycle closeout `f6cf946b`
...
- [058 Claude Code 2.1.270 Useful Newer](./058-claude-code-2-1-270-useful-newer.md) — completed; PR #321 merged at `b1d648bf`; both existing stream-JSON axes qualified through `2.1.270`; lifecycle closeout `a8d12488`
- [057 Lifecycle Currentness Repair](./057-repair-lifecycle-currentness.md) — completed; lifecycle-owned state repair closed
- [056 Effigy-Hosted Lifecycle Hook](./056-adopt-effigy-hosted-lifecycle-hook.md) — completed; required Queue lifecycle hooks adopted
...
- [045 Claude SDK Consumer Multi-Turn Editing Acceptance](./045-claude-sdk-consumer-multi-turn-editing-acceptance.md) — completed; provider-limitation reconciliation; PR #308 merged at `05f1a3a1` after independent exact-head review `5610583309` with no follow-ups
- [044 Pi RPC 0.85.1 Useful Newer](./044-pi-rpc-0-85-1-useful-newer.md) — complete; PR #304 merged at `eeb44e86`; qualified `pi.package` through official `0.85.1`; sidecar stays exact `0.84.2`
...
<!-- northstar:lifecycle:begin schema=northstar.lifecycle.projection.v2 digest=sha256:cc5742cb964db90c3e57a58f00aab3cbb36a443e4b42e8187f4d6c8f1fd3cb0e -->

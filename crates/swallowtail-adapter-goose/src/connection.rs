...
impl AcpConnection {
    ...
    pub(crate) async fn initialize(&self) -> Result<Value, RuntimeFailure> {
        ...
        if response.get("protocolVersion").and_then(Value::as_u64) != Some(ACP_PROTOCOL_VERSION) {
            return Err(failure(
                "swallowtail.goose.acp.version_mismatch",
                "Goose negotiated an incompatible ACP version",
            ));
        }
        Ok(response)
    }
    ...
    async fn begin_request_with_id(
        ...
        if self.closed.load(Ordering::SeqCst) {
            return Err(failure(
                "swallowtail.goose.acp.connection_closed",
        ...
        let (sender, response) = response_channel();
        {
            ...
            if pending.len() >= MAXIMUM_PENDING_REQUESTS || pending.insert(id, sender).is_some() {
                return Err(failure(
                    "swallowtail.goose.acp.correlation_limit",
                    "Goose ACP request correlation limit was exceeded",
                ));
            }
        }
    ...
    pub(crate) fn set_session_id(&self, session_id: String) -> Result<(), RuntimeFailure> {
        ...
        if current.is_some() {
            return Err(failure(
                "swallowtail.goose.acp.session_duplicate",
                "Goose returned more than one ACP session",
            ));
        }
        ...
        self.cleanup
            ...
            .unwrap_or_else(|| {
                CleanupOutcome::Failed(SafeDiagnostic::new(
                    "swallowtail.goose.acp.cleanup_missing",
                    "Goose process cleanup did not complete",
                ))
            })
    }

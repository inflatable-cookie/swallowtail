...
`1.18.20`. Captured through 2026-08-21 from maintained GitHub release tags and each
...
Sixty-one stable releases produce 19 closed surfaces and 21 contiguous
...
- `delete-02`: `1.15.6` through `1.18.20`
...
guard or background-job cancellation. `1.14.51..=1.18.20` adds background-job
...
is present at both `1.14.48` and `1.18.20` boundaries. Cost remains separate
from token usage.
...
requests across all 61 qualified releases. Four exact closed-surface revisions
...
`session.list` and `session.status` operations. All 61 qualified releases map
...
tool and reasoning part replacement do not. The 61-release
...
Exact `1.18.11` through `1.18.20` share one OpenAPI digest. The six selected
...
adds `reasoning_text` plus a free string, so execution stays on private
`surface-19`. Swallowtail does not map that field. Published `1.18.19` and
`1.18.20` are byte-identical to `1.18.18`.

Sources:

- <https://github.com/anomalyco/opencode/releases>
- <https://github.com/anomalyco/opencode/tree/v1.14.48>
- <https://github.com/anomalyco/opencode/tree/v1.18.20>

...
fn descriptor_is_a_distinct_exact_pi_rpc_driver() {
    ...
    assert!(
        descriptor.supports_interface_version(&InterfaceVersionBinding::new(
            axis.clone(),
            InterfaceVersion::new("0.83.0").expect("valid version"),
        ))
    );
    assert!(
        descriptor.supports_interface_version(&InterfaceVersionBinding::new(
    ...
    assert!(
        descriptor.supports_interface_version(&InterfaceVersionBinding::new(
            axis.clone(),
            InterfaceVersion::new("0.84.3").expect("valid version"),
        ))
    );
    assert!(
        descriptor.supports_interface_version(&InterfaceVersionBinding::new(
            axis.clone(),
            InterfaceVersion::new("0.84.4").expect("valid version"),
        ))
    );
    assert!(
        descriptor.supports_interface_version(&InterfaceVersionBinding::new(
            axis.clone(),
            InterfaceVersion::new("0.85.1").expect("valid version"),
        ))
    );
    assert!(
        !descriptor.supports_interface_version(&InterfaceVersionBinding::new(
            axis.clone(),
            InterfaceVersion::new("0.83.1").expect("valid version"),
        ))
    );
    assert!(
        !descriptor.supports_interface_version(&InterfaceVersionBinding::new(

//! Live local-server claim identity versus frozen historical reservations.
//!
...
#[test]
fn frozen_historical_local_server_claim_ids_stay_reserved() {
    let range = json(FROZEN_0_30_0_0_31_0_RANGE);
    let release = json(FROZEN_0_31_1_RELEASE);
    let from_fixtures = [
        range["local_server"]["claim"]
            .as_str()
            .expect("0.30.0-0.31.0 local-server claim is text"),
        release["local_server"]["claim"]
            .as_str()
            .expect("0.31.1 local-server claim is text"),
    ];
    assert_eq!(
        from_fixtures, RESERVED_HISTORICAL_LOCAL_SERVER_CLAIM_IDS,
        "frozen corpora and the reserved set must name the same historical ids"
    );

    let live_claim = kimi_local_server_claim();
    let live = live_claim.id().as_str();
    assert_eq!(live, LIVE_LOCAL_SERVER_CLAIM_ID);
    assert_ne!(

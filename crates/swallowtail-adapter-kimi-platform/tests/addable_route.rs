//! Contract 057 addable-route descriptor tests for hosted Kimi Platform Chat.
...
fn host_id() -> ExecutionHostId {
    ExecutionHostId::new("kimi-platform.addable.host").expect("host id is valid")
}
...
fn descriptor_is_hosted_and_matches_the_direct_driver() {
    let descriptor = kimi_platform_chat_addable_route_descriptor(&services_with_credential());

    assert_eq!(
        descriptor.id().as_str(),
        KIMI_PLATFORM_CHAT_ADDABLE_ROUTE_ID
    );
    assert_eq!(descriptor.id().as_str(), "kimi-platform.chat");
...
fn credential_field_is_secret_and_carries_no_secret_bytes() {
    ...
    assert!(!debug.contains("MOONSHOT_API_KEY"));
    assert!(!debug.contains("KIMI_PLATFORM_API_KEY"));
}
...
fn endpoint_config_is_an_opaque_field_not_a_url() {
    ...
    let field = descriptor
        .config_field(
            &swallowtail_core::ConfigFieldId::new(KIMI_PLATFORM_CHAT_ENDPOINT_FIELD_ID)
    ...
    assert!(!debug.contains("api.moonshot.ai"));
}

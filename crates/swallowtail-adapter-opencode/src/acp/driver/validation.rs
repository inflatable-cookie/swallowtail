fn validate_open(
    ...
    for (present, code, message) in [
        (
            ...
            "swallowtail.opencode.acp.task_service_missing",
            "OpenCode ACP requires a scoped task service",
        ),
        (
            services.process().is_some(),
            "swallowtail.opencode.acp.process_service_missing",
            "OpenCode ACP requires a process service",
        ),
        (
            services.working_resource().is_some(),
            "swallowtail.opencode.acp.resource_service_missing",
            "OpenCode ACP requires a working-resource service",
        ),
    ] {
        if !present {
    ...
    if request.access_policy() != &SessionAccessPolicy::ambient_harness(resource_access) {
        return Err(failure(
            "swallowtail.opencode.acp.access_policy_rejected",
            "OpenCode ACP requires exact ambient working-resource access",
        ));
    }
...
fn validate_initialize(
    ...
    // Do not infer that the requested protocolVersion was accepted.
    // OpenCode answers 1 regardless. Fail closed if a present agent version drifts.
    if let Some(info) = response.get("agentInfo") {
        ...
        {
            return Err(failure(
                "swallowtail.opencode.acp.agent_version_rejected",
                "OpenCode ACP identity does not match the preflight version",
            ));
        }
    }
    Ok(())
...
fn session_resource_access(plan: &PreflightPlan) -> Result<ResourceAccess, RuntimeFailure> {
    plan.requirements()
        ...
        .ok_or_else(|| {
            failure(
                "swallowtail.opencode.acp.resource_access_missing",
                "OpenCode ACP requires explicit working-resource access",
            )
        })
}

...
pub(super) struct ClineHeadlessCancellation {
    process: Arc<dyn ProcessHandle>,
    requested: AtomicBool,
}

impl ClineHeadlessCancellation {
...
impl CancellationControl for ClineHeadlessCancellation {
...
pub(super) struct ClineHeadlessRunHandle {
...
impl RunHandle for ClineHeadlessRunHandle {
        ...
        Box::pin(async move {
            match self.task.join().await {
                ...
                Err(_) => CleanupOutcome::Failed(SafeDiagnostic::new(
                    "swallowtail.cline.headless.task_join_failed",
                    "Cline headless operation task could not be joined",
                )),
            }
        })
    }

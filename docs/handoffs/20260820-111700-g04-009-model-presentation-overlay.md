---
title: g04.009 model presentation overlay worker handoff
...
created: 2026-08-20
updated: 2026-08-20
handoff_path: /Users/tom/Dev/projects/swallowtail/docs/handoffs/20260820-111700-g04-009-model-presentation-overlay.md
...
Refresh, subject observation, and 029/032 update observation are on `main`.
...
Without overlay projection, the T3 hide/reorder/default/favourite surface
stays a stored marker with no catalogue view. Overlay must key to exact
instance, provider, and model identities and must not become a second
catalogue or a way to make `NotReady` selectable.
...
- **Planning base commit:** `8f4e3884c66c2c0cd732f4b916c24af8ad5336e4`
...
- **Planning artifacts included at the base:** Contract 057; realized kernel,
  catalog, admission, sign-in, refresh, subject, and update observation;
  `OverlayMarker` store port; milestone g04.009; ready cards 025-026
- **Worker branch:** `g04-009-model-presentation-overlay`
- **Worker worktree:** launcher-provided dedicated worktree first. Manual
  fallback path is
  `$AGENTS_WORKTREE_CONTAINER_DIR/swallowtail-g04-009-model-presentation-overlay`
- **Worktree creation command:** only if the current context is unusable and
  ...
  `git worktree add -b g04-009-model-presentation-overlay "$AGENTS_WORKTREE_CONTAINER_DIR/swallowtail-g04-009-model-presentation-overlay" origin/main`
...
- **Roadmap milestone:** `docs/roadmaps/g04/009-model-presentation-overlay.md`
- **Ready cards, in order:**
  `docs/roadmaps/g04/batch-cards/025-apply-model-presentation-overlay.md`,
  then `docs/roadmaps/g04/batch-cards/026-overlay-refusals.md`
- **Allowed runway:** g04.009 cards 025 → 026. Stop after overlay refusals.
  Do not start first-proof work.
- **Remaining card budget:** two cards, one PR
- **Dispatch topology:** serial; one worker, one worktree, one PR
- **Parallel safety check:** refusals share the overlay projection from card
  025. No parallel lane.
- **Canonical refs:** Contract 057 Model-Presentation Overlay; Contracts 020
  and 047; `OverlayMarker`; `ConnectionLifecycleStore`;
  `ConfiguredProviderInstanceRecord`;
  `ConfiguredProviderInstanceSelectionReadiness`;
  `ConfiguredProviderModelCatalogue`; `ModelCatalogEntry`
- **Model capability profile:** capable coding model, medium reasoning
- **Tool/runtime restrictions:** no live provider catalogue calls, install,
  login, or billing work. No first-proof adapters. No 047 snapshot field
  additions. No GitHub Release, crates.io, or tag mutation. Do not rewrite
  `release-baselines/public-api-0.3.3/`.
- **Required validation:** card 025:
  `effigy validate:focused swallowtail-runtime swallowtail-testkit`,
  `git diff --check`. Card 026:
  `effigy validate:focused swallowtail-runtime swallowtail-testkit`,
...
- **PR base/head:** `main` / selected worker branch
  (`g04-009-model-presentation-overlay` unless the launcher supplied a
...
- **In scope:** project hide, ordinal, consumer-default, and favourite onto
  one bound catalogue result; key to exact configured-instance, provider,
  and model ids; refuse invented models, cross-instance copies, and making
  `NotReady` selectable.
- **Out of scope:** first-proof Anthropic, Codex, or Ollama wiring; adding
  overlay metadata to 047 types; accent color and other chrome; gateway
  flattening; live provider catalogue calls; rewriting `public-api-0.3.3`;
  GitHub Release; crates.io; tag mutation.
- Do not invent architecture or change Contract 057.
- Overlay markers cannot change 047 `Ready` / `NotReady`. Provider catalogue
  defaults (`ModelCatalogEntry` / `ModelMetadata::is_default`) stay distinct
  from the consumer-default marker. Mixed gateway rows remain consumer
  assembly of several catalogues.
- Apply is a projection over a bound 047 catalogue result and stored
  `OverlayMarker` values. Do not mutate 047 types in place. Do not merge
  several instance catalogues into one overlay result.
...
- **Planning lineage:** g04 tagged `v0.3.3`, merged kernel (PR 4), catalog
  (PR 5), sign-in (PR 6), and refresh/subject/updates (PR 7). Overlay is the
  last connection-lifecycle facade piece before first-proof routes.
- **Why these cards are ready:** Contract 057 already names overlay
  projection and refusals. `OverlayMarker` records persist through the store.
  047 already projects a bound model catalogue and `Ready` / `NotReady`.
  Refresh exists so overlay is not covering for stale access status.
- **Decisions and preferences:** overlay cannot invent a model, copy across
  instances, or make `NotReady` selectable. Optional 047 presentation
  metadata stays later and must not change selection readiness.
- **Open tensions:** `start_sign_in` still requires immediately-ready host
  ...
  overwrites; do not change that here. Update observation is not
  instance-keyed; do not change that here. There is no `.agents.local.env`
  on the planning machine; if the launcher does not supply a worktree, ask
  the operator for `AGENTS_WORKTREE_CONTAINER_DIR`.
- **Report after:** card 025 overlay projection with `Ready` / `NotReady`
  unchanged; card 026 refusals and the PR
...
Once that checks out, take card 025 first. Project stored overlay markers
onto one bound catalogue result and leave 047 `Ready` / `NotReady` alone.
When 025 is green, continue into 026. When overlay refusals are green, open
the PR and stop.
...
4. From the selected worktree, confirm `git rev-parse HEAD` equals
   ...
   `git merge-base --is-ancestor 8f4e3884c66c2c0cd732f4b916c24af8ad5336e4 HEAD`
...
1. Run the required final validation for card 026, plus `effigy package:api`
...
4. Open a reviewable PR against the current pushed `main` tip. The handoff's
   planning base `8f4e3884c66c2c0cd732f4b916c24af8ad5336e4` is the planning
   commit before the handoff was created, not a self-referential hash for the
   commit that contains this file.
5. In the PR body, link the milestone, cards 025-026, Contract 057, changed
...
- **Closeout refs:** cards 025-026, g04.009, `docs/roadmaps/README.md`,
  `docs/logs/README.md`
...
orchestrator will return to the operator for merge, then compile first-proof
routes.

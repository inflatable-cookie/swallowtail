//! Adapter-local prepared constructor for `qoder.headless`.
...
pub use run::{QoderHeadlessPreparedRun, QoderHeadlessRunProfileInput};

use crate::QoderHeadlessDriver;
...
const PROTOCOL_FACADE_ID: &str = "qoder-stream-json-v1";
const POLICY_ID: &str = "qoder-prepared-ambient-headless";

#[derive(Clone, Debug, Eq, PartialEq)]
/// Host, target, environment, and local-config inputs for Qoder headless preparation.
pub struct QoderHeadlessPreparationInput {
...
impl QoderHeadlessPreparationInput {
    #[must_use]
    #[allow(clippy::too_many_arguments)]
    /// Creates explicit preparation input for one exact Qoder package.
    pub const fn new(
        instance_id: ConfiguredInstanceId,
        instance_revision: InstanceRevision,
        execution_host_id: ExecutionHostId,
...
#[derive(Clone, Debug)]
/// Bounded installed-executable probe used during Qoder headless preparation.
pub struct QoderHeadlessPreparationProbe {
...
impl QoderHeadlessPreparationProbe {
    #[must_use]
...
#[derive(Clone, Debug, Eq, PartialEq)]
/// Qualified Qoder headless integration ready to prepare one bounded print run.
pub struct QoderHeadlessPreparedIntegration {
    environment: EnvironmentRef,
    target: InstalledExecutableTarget,
...
impl QoderHeadlessPreparedIntegration {
    #[must_use]
    ...
    pub fn low_level_driver(&self) -> QoderHeadlessDriver {
        QoderHeadlessDriver::new(self.environment.clone())
    }
    ...
    pub fn validate_execution_binding(
        ...
        if execution_host_id != self.observation.execution_host_id() || target != &self.target {
            return Err(failure(
                ...
                "swallowtail.qoder.headless.preparation.target_drift",
                "Prepared Qoder host or executable target no longer matches",
            ));
        }
        Ok(())
    }
...
/// Discovers and prepares exactly one qualified Qoder headless package.
pub async fn prepare_qoder_headless(
    ...
    let driver = QoderHeadlessDriver::new(input.environment.clone());
    let outcome = driver
...
fn validate_input(input: &QoderHeadlessPreparationInput) -> Result<(), PreparationFailure> {
    if input.target.version_axis().as_str() != crate::QODER_PACKAGE_AXIS {
        return Err(failure(
            PreparationStage::TargetSelection,
            "swallowtail.qoder.headless.preparation.target_axis_mismatch",
            "Qoder headless preparation target uses a different package axis",
        ));
    }
    if input.access_profile.credential_mechanism() != &CredentialMechanism::LocalUnauthenticated
        ...
        || input.access_profile.endpoint_audience().as_str() != crate::QODER_LOCAL_ACCOUNT_AUDIENCE
        || input.access_profile.support_authority() != SupportAuthority::ProviderSupported
    {
        return Err(failure(
            ...
            "Qoder headless requires its provider-supported local-config profile",
        ));
    }
            ...
            != input.access_profile.support_authority()
    {
        return Err(failure(
            PreparationStage::AccessEvidence,
            "swallowtail.qoder.headless.preparation.access_evidence_mismatch",
            "Qoder access evidence does not match the selected profile",
        ));
    }
    Ok(())
}
...
fn promote(
    ...
    if observation.execution_host_id() != &input.execution_host_id
        ...
        || observation.version().version().as_str() != crate::QODER_PACKAGE_VERSION
    {
        return Err(failure(
            PreparationStage::CompatibilityClassification,
            "swallowtail.qoder.headless.preparation.observation_mismatch",
            "Qoder discovery observation does not match the prepared host and package",
        ));
    }
    let instance = configured_instance(&input, &observation)?;
    Ok(QoderHeadlessPreparedIntegration {
...
fn configured_instance(
    ...
    let target =
        InstanceTargetRef::new(input.target.executable().as_host_value()).map_err(|_| {
            failure(
                PreparationStage::TargetSelection,
                "swallowtail.qoder.headless.preparation.target_invalid",
                "Qoder headless target could not be bound to the configured instance",
            )
        })?;
    Ok(ConfiguredInstance::new(
        input.instance_id.clone(),
        input.instance_revision.clone(),
        crate::qoder_headless_descriptor().identity().id().clone(),
        input.execution_host_id.clone(),
        target,
        InstanceOwnership::HostOwnedEphemeral,
        input.access_profile.id().clone(),
        SupportAuthority::ProviderSupported,
        ProtocolFacadeId::new(PROTOCOL_FACADE_ID).expect("static Qoder headless facade is valid"),
        InstancePolicyId::new(POLICY_ID).expect("static Qoder headless policy is valid"),
...
fn discovery_outcome_failure(outcome: &DiscoveryOutcome) -> PreparationFailure {
    ...
    let diagnostic = outcome.diagnostic().cloned().unwrap_or_else(|| {
        swallowtail_core::SafeDiagnostic::new(
            "swallowtail.qoder.headless.preparation.discovery_rejected",
            "Qoder executable discovery was not promotable",
        )
    });

...
fn success_turn_uses_acp_only_and_joins_cleanup() {
    ...
    let mut session = block_on(driver.open_session(
        ...
        OpenSessionRequest::new(
            RequestId::new("cline-open").expect("valid request"),
    ...
    let mut turn = block_on(session.start_turn(
        TurnRequest::new(
            RuntimeTurnId::new("cline-turn").expect("valid turn"),
    ...
    assert!(
        !observed
            .arguments
            .iter()
            .any(|argument| argument == "--auto-approve")
    );
...
#[test]
fn read_write_plan_still_launches_acp_without_auto_approve() {
    ...
    let mut session = block_on(driver.open_session(
        selected.plan,
        OpenSessionRequest::new(
            RequestId::new("cline-write-open").expect("valid request"),
            selected.resource,
            None,
            SessionPlanAgreement::explicit(
...
#[test]
fn missing_host_credentials_fail_closed_as_auth_required() {
    ...
    let error = match block_on(driver.open_session(
        selected.plan,
        OpenSessionRequest::new(
            RequestId::new("cline-auth").expect("valid request"),
            selected.resource,
            None,
            SessionPlanAgreement::explicit(
                swallowtail_core::SessionAccessPolicy::ambient_harness(ResourceAccess::Read),
...
fn malformed_and_version_mismatch_fail_before_prompt() {
    for (scenario, suffix) in [
        ...
        let error = match block_on(driver.open_session(
            selected.plan,
            OpenSessionRequest::new(
                RequestId::new(format!("cline-{suffix}")).expect("valid request"),
                selected.resource,
                None,
                SessionPlanAgreement::explicit(
                    swallowtail_core::SessionAccessPolicy::ambient_harness(ResourceAccess::Read),
...
fn session_deadline_is_rejected_before_spawn() {
    ...
    let error = match block_on(driver.open_session(
        selected.plan,
        OpenSessionRequest::new(
            RequestId::new("cline-deadline").expect("valid request"),
            selected.resource,

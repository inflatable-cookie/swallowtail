...
async fn probe_joined(
    ...
    if request.target().version_axis() != qwen_headless_claim().axis() {
        return Err(failure(
            "swallowtail.qwen.discovery_axis_mismatch",
            "Qwen discovery target uses a different version axis",
        ));
    }
...
async fn probe_process(
    ...
    .map_err(|_| {
        failure(
            "swallowtail.qwen.discovery_classification_failed",
            "Qwen version observation could not be classified",
        )
    })?;
...
fn outcome(status: DiscoveryStatus) -> DiscoveryOutcome {
    DiscoveryOutcome::new(
        ...
        Some(SafeDiagnostic::new(
            status_code(status),
            "Qwen installed discovery did not produce a compatible observation",
        )),
    )
}
...
const fn status_code(status: DiscoveryStatus) -> &'static str {
    match status {
        DiscoveryStatus::Absent => "swallowtail.qwen.discovery_absent",
        DiscoveryStatus::Discovered => "swallowtail.qwen.discovery_discovered",
        DiscoveryStatus::Incompatible => "swallowtail.qwen.discovery_incompatible",
        DiscoveryStatus::Malformed => "swallowtail.qwen.discovery_malformed",
        DiscoveryStatus::TimedOut => "swallowtail.qwen.discovery_timed_out",
        DiscoveryStatus::Cancelled => "swallowtail.qwen.discovery_cancelled",
        DiscoveryStatus::Failed => "swallowtail.qwen.discovery_failed",
        DiscoveryStatus::CleanupFailed => "swallowtail.qwen.discovery_cleanup_failed",
    }
}

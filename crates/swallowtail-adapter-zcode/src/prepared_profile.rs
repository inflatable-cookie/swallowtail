...
use crate::prepared::ZcodePreparedIntegration;
...
use swallowtail_runtime::{
    BoxFuture, Deadline, ExecutableRef, HostServices, OperationContent, OperationPolicy,
    PreparationFailure, PreparationStage, PreparedOperationEvidence, ProviderRetentionPolicy,
    RequestId, RunHandle, RuntimeFailure, StructuredRunDriver, StructuredRunRequest,
    WorkingResourceRef,
};

#[derive(Clone, Debug, Eq, PartialEq)]
/// Explicit provider, model, and route selection for one harness run.
pub struct ZcodeModelSelection {
...
#[derive(Clone, Debug, Eq, PartialEq)]
/// Prompt, provider, model, mode, cwd, and deadline for one run.
pub struct ZcodeRunProfileInput {
...
#[derive(Clone, Debug, Eq, PartialEq)]
/// Immutable evidence for one prepared ZCode app-server operation.
pub struct ZcodePreparedEvidence {
...
impl ZcodePreparedEvidence {
    fn from_prepared(
        prepared: &ZcodePreparedIntegration,
        mode: ZcodeAppServerMode,
        plan: PreflightPlan,
        ...
        Ok(Self {
            ...
            interpreter: prepared.interpreter().clone(),
    ...
    #[must_use]
    /// Returns the host-supplied session mode.
    ...
    fn low_level_driver(&self) -> crate::ZcodeAppServerDriver {
        crate::ZcodeAppServerDriver::new(self.environment.clone(), self.mode.clone())
    }
}

#[derive(Clone, Debug, Eq, PartialEq)]
/// Prepared one-shot ZCode app-server structured run.
pub struct ZcodePreparedRun {
    evidence: ZcodePreparedEvidence,
...
impl ZcodePreparedRun {
    ...
    pub const fn evidence(&self) -> &ZcodePreparedEvidence {
        &self.evidence
    }
    ...
    #[must_use]
    /// Returns the host-supplied session mode bound into this run.
    pub const fn mode(&self) -> &ZcodeAppServerMode {
        self.evidence.mode()
    }
    ...
    pub fn low_level_driver(&self) -> crate::ZcodeAppServerDriver {
    ...
    pub fn into_parts(self) -> (ZcodePreparedEvidence, PreflightPlan, StructuredRunRequest) {
...
impl ZcodePreparedIntegration {
    /// Validates and prepares one exact ZCode app-server structured run.
    pub fn prepare_run(
        &self,
        ...
        .require_model_route();
        let descriptor = crate::zcode_app_server_descriptor();
        ...
        Ok(ZcodePreparedRun {
            evidence: ZcodePreparedEvidence::from_prepared(self, input.mode, plan, activity)?,
            request,
        })
    }
}
...
fn activity_profile(
    ...
    let behavior = match observation.compatibility() {
        ...
        _ => {
            return Err(failure(
                "swallowtail.zcode.app_server.preparation.activity_version_incompatible",
                "ZCode activity requires the exact permitted runtime payload",
            ));
        }
    };
    ...
    .map_err(|_| {
        failure(
            "swallowtail.zcode.app_server.preparation.activity_profile_invalid",
            "ZCode activity profile could not be derived",
        )
    })
}
...
fn with_activity(
    ...
    requirements.push(
        activity
            ...
            .expect("prepared ZCode activity is available"),
    );
...
fn access_requirement(integration: &ZcodePreparedIntegration) -> AccessRequirement {

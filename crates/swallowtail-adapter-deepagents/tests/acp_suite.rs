...
use swallowtail_runtime::{
    CleanupOutcome, Deadline, DiscoveryCancellation, DiscoveryDriver, ExecutableRef,
    InstalledExecutableDiscoveryRequest, InstalledExecutableTarget, InteractiveSessionDriver,
    MonotonicInstant, OpenSessionRequest, OperationContent, RequestId, RuntimeEventKind,
    RuntimeTurnId, ScopeId, SessionPlanAgreement, TerminalStatus, TurnRequest,
};
...
fn success_turn_uses_acp_only_and_joins_cleanup() {
    ...
    let mut session = block_on(driver.open_session(
        ...
        OpenSessionRequest::new(
            RequestId::new("deepagents-open").expect("valid request"),
    ...
    let mut turn = block_on(session.start_turn(
        TurnRequest::new(
            RuntimeTurnId::new("deepagents-turn").expect("valid turn"),
...
#[test]
fn read_write_plan_still_launches_acp_without_workspace_or_model() {
    ...
    let mut session = block_on(driver.open_session(
        selected.plan,
        OpenSessionRequest::new(
            RequestId::new("deepagents-write-open").expect("valid request"),
            selected.resource,
            None,
            SessionPlanAgreement::explicit(
    ...
    assert_eq!(outcome.status(), &TerminalStatus::Completed);
    assert!(host.observed_process().arguments.is_empty());
    assert!(host.writes().iter().any(|message| {
    ...
    assert!(host.writes().iter().any(|message| {
        message.get("id").and_then(serde_json::Value::as_u64) == Some(900)
            ...
            && message["result"]["outcome"].get("optionId")
                != Some(&serde_json::json!("allow-always"))
    }));
    assert_eq!(
...
fn missing_host_auth_fails_closed_without_login() {
    ...
    let error = match block_on(driver.open_session(
        selected.plan,
        OpenSessionRequest::new(
            RequestId::new("deepagents-auth").expect("valid request"),
            selected.resource,
            None,
            SessionPlanAgreement::explicit(
                swallowtail_core::SessionAccessPolicy::ambient_harness(ResourceAccess::Read),
...
fn malformed_and_version_mismatch_fail_before_prompt() {
    for (scenario, suffix) in [
        ...
        let error = match block_on(driver.open_session(
            selected.plan,
            OpenSessionRequest::new(
                RequestId::new(format!("deepagents-{suffix}")).expect("valid request"),
                selected.resource,
                None,
                SessionPlanAgreement::explicit(
                    swallowtail_core::SessionAccessPolicy::ambient_harness(ResourceAccess::Read),
...
#[test]
fn constructor_default_agent_info_version_is_not_the_package_axis() {
    let host_id = ExecutionHostId::new("fixture.host.agent-version").expect("valid host id");
    let selected = selection(host_id.clone());
    let host = FixtureHost::with_version(Scenario::Success, "0.0.1");
    let services = host.services(host_id);
    let driver = DeepAgentsAcpDriver::new(
        swallowtail_runtime::EnvironmentRef::new("deepagents.fixture.isolated")
            .expect("valid environment"),
    ...
    let session = block_on(driver.open_session(
        selected.plan,
        OpenSessionRequest::new(
            RequestId::new("deepagents-agent-version").expect("valid request"),
            selected.resource,
            None,
            SessionPlanAgreement::explicit(
                swallowtail_core::SessionAccessPolicy::ambient_harness(ResourceAccess::Read),
...
#[test]
fn drifted_agent_info_name_fails_closed() {
    let host_id = ExecutionHostId::new("fixture.host.agent-name").expect("valid host id");
    let selected = selection(host_id.clone());
    let host = FixtureHost::with_agent_info(Scenario::Success, "not-deepagents-acp", "0.0.1");
    let services = host.services(host_id);
    let driver = DeepAgentsAcpDriver::new(
        swallowtail_runtime::EnvironmentRef::new("deepagents.fixture.isolated")
            .expect("valid environment"),
    ...
    let error = match block_on(driver.open_session(
        selected.plan,
        OpenSessionRequest::new(
            RequestId::new("deepagents-agent-name").expect("valid request"),
            selected.resource,
            None,
            SessionPlanAgreement::explicit(
                swallowtail_core::SessionAccessPolicy::ambient_harness(ResourceAccess::Read),
        ...
        Err(error) => error,
        Ok(_) => panic!("agentInfo.name drift must fail closed"),
    };
    assert_eq!(
        error.diagnostic().code(),
...
fn session_deadline_is_rejected_before_spawn() {
    ...
    let error = match block_on(driver.open_session(
        selected.plan,
        OpenSessionRequest::new(
            RequestId::new("deepagents-deadline").expect("valid request"),
            selected.resource,

---
title: g04.016 hosted API-key DeepSeek continuation worker handoff
...
created: 2026-08-20
updated: 2026-08-20
handoff_path: /Users/tom/Dev/projects/swallowtail/docs/handoffs/20260820-171558-g04-016-hosted-api-key-deepseek.md
...
The first-proof shapes and Contract 052 consumer path are on `main`.
Research 170 mapped a second hosted API-key proof: DeepSeek continuation
already has a prepared facade. A consumer still cannot list or admit that
route through Contract 057. Hosted interactive OAuth stays a remaining
gate.
...
Without an adapter-local hosted descriptor, the second-proof programme
cannot start. DeepSeek is the hosted API-key analog of Anthropic Messages.
Tokens must not enter portable records. Do not invent an environment name.
...
- **Planning base commit:** `1ba74dd5336a5e17523695a64e64cf44cd053161`
...
- **Planning artifacts included at the base:** Contract 057; Research 170;
  realized Anthropic first-proof; completed g04.015; milestone g04.016;
  ready cards 045-047
- **Worker branch:** `g04-016-hosted-api-key-deepseek`
- **Worker worktree:** launcher-provided dedicated worktree first. Manual
  fallback path is
  `$AGENTS_WORKTREE_CONTAINER_DIR/swallowtail-g04-016-hosted-api-key-deepseek`
- **Worktree creation command:** only if the current context is unusable and
  ...
  `git worktree add -b g04-016-hosted-api-key-deepseek "$AGENTS_WORKTREE_CONTAINER_DIR/swallowtail-g04-016-hosted-api-key-deepseek" origin/main`
...
- **Roadmap milestone:** `docs/roadmaps/g04/016-hosted-api-key-deepseek-continuation.md`
- **Ready cards, in order:**
  `docs/roadmaps/g04/batch-cards/045-deepseek-continuation-addable-descriptor.md`,
  then `docs/roadmaps/g04/batch-cards/046-deepseek-continuation-admission-and-api-key.md`,
  then `docs/roadmaps/g04/batch-cards/047-deepseek-continuation-refresh-subject-and-047.md`
- **Allowed runway:** g04.016 cards 045 → 046 → 047. Stop after refresh,
  subject, and overlay. Do not start Claude Agent, llama.cpp, or hosted
  OAuth.
...
- **Parallel safety check:** admission and refresh write the same admitted
  DeepSeek record. No parallel lane.
- **Canonical refs:** Contract 057; Contracts 011, 014, 020, 037, 047;
  Research 170; `deepseek_direct_descriptor`; `deepseek_facade_claim`;
  `prepare_deepseek_direct`; `AddableRouteDescriptor`; `admit_instance`;
...
- **Tool/runtime restrictions:** no live provider, install, login, or
  billing work. No hosted OAuth. No Claude Agent or llama.cpp descriptor
  edits. No OpenHands production route. Do not invent an environment
  name. Do not invent a catalogue `provider_id`. No 047 snapshot field
  additions. No GitHub Release, crates.io, or tag mutation. Do not rewrite
  `release-baselines/public-api-0.3.3/`.
- **Required validation:** card 045:
  `effigy validate:focused swallowtail-adapter-deepseek swallowtail-runtime`,
  `git diff --check`. Card 046:
  `effigy validate:focused swallowtail-adapter-deepseek swallowtail-runtime swallowtail-host-local`,
  `git diff --check`. Card 047:
...
- **In scope:** adapter-local hosted addable descriptor for
  `deepseek.continuation`; secret API-key field with no invented env name;
  opaque API-endpoint config field; 057 admission and API-key collection;
  reuse of `prepare_deepseek_direct`; host-supplied refresh; subject
  Absent; overlay keys `deepseek` catalogue rows.
- **Out of scope:** hosted interactive OAuth; Claude Agent; llama.cpp;
  OpenHands production wiring; inventing `DEEPSEEK_API_KEY` or any other
  env name; adding overlay metadata to 047; rewriting `public-api-0.3.3`;
...
- **Planning lineage:** g04 tagged `v0.3.3`, facade through overlay, first
  proofs, Contract 052 consumer path (PR 12), hosted OAuth gate held,
  Research 170. DeepSeek is the next hosted API-key shape.
- **Why these cards are ready:** Research 170 mapped the prepared facade
  and overlay keys onto 057. Hosted OAuth stays gated.
- **Decisions and preferences:** do not invent an environment name. The
  DeepSeek guide forbids reading env values into portable records.
- **Open tensions:** `start_sign_in` still requires immediately-ready host
  futures; do not change that here. `prepare_deepseek_direct` still takes a
  host `InstanceTargetRef`, not stored `ConfigFieldRef` values. There is no
...
- **Report after:** card 045 descriptor; card 046 admission and API-key;
  card 047 refresh/subject/overlay and the PR
...
Once that checks out, take card 045 first. Ship the hosted DeepSeek
continuation addable descriptor. When 045 is green, continue into 046, then
047. When refresh, subject, and overlay are green, open the PR and stop.
...
4. From the selected worktree, confirm `git rev-parse HEAD` equals
   ...
   `git merge-base --is-ancestor 1ba74dd5336a5e17523695a64e64cf44cd053161 HEAD`
...
- Do not start Claude Agent, llama.cpp, or hosted OAuth work.

### When the assigned runway is complete

1. Run the required final validation for card 047, plus `effigy package:api`
...
4. Open a reviewable PR against the current pushed `main` tip. The handoff's
   planning base `1ba74dd5336a5e17523695a64e64cf44cd053161` is the planning
   commit before the handoff was created, not a self-referential hash for the
   commit that contains this file.
5. In the PR body, link the milestone, cards 045-047, Contract 057, Research
...
- **Closeout refs:** cards 045-047, g04.016, `docs/roadmaps/README.md`,
  `docs/logs/README.md`
...
remaining gate. Claude Agent ACP and llama.cpp attached stay planned.

...
#[test]
fn the_coverage_ledger_dispositions_exactly_the_eight_qoder_rows() {
    let mut tuples = BTreeSet::new();
    let mut semantics = BTreeSet::new();
    for entry in &QODER_HEADLESS_TRANCHE {
        assert_eq!(
            entry.route_id, QODER_ROUTE,
            "{} does not belong to the exact census route",
            entry.semantic_id
        );
        assert!(
            QODER_OPERATION_SHAPES.contains(&entry.operation_shape),
            "{} names an operation shape outside the qoder.headless census",
            entry.semantic_id
        );
        assert!(
        ...
        for profile in entry.emitted_by {
            assert!(
                QODER_PROFILES.contains(profile),
                "{} names an unknown prepared Qoder profile",
                entry.semantic_id
            );
        }
    }
    assert_eq!(QODER_HEADLESS_TRANCHE.len(), 8);
    assert_eq!(tuples.len(), 8);
    assert_eq!(semantics.len(), 8);
}

#[test]
fn every_prepared_qoder_profile_emits_exactly_its_ledger_identities() {
    let observed = observed_dispositions();
    assert_eq!(observed.len(), QODER_PROFILES.len());
    for profile in QODER_PROFILES {
...
#[test]
fn withheld_qoder_rows_are_emitted_by_no_prepared_profile() {
    let observed = observed_dispositions();
    let emitted = observed
        ...
        .cloned()
        .collect::<BTreeSet<_>>();
    let ledger = QODER_HEADLESS_TRANCHE
        .iter()
        .map(identity)
        .collect::<BTreeSet<_>>();
    ...
    for off_route in WITHHELD_OFF_ROUTE {
        assert!(
            ...
            "{off_route} has no qoder.headless census row and must never be constructed"
        );
        assert!(
            !QODER_HEADLESS_TRANCHE
                .iter()
                .any(|entry| entry.semantic_id == off_route),
            "{off_route} must not appear in the eight-row ledger"
        );
    }
    assert_eq!(
        emitted.len(),

...
pub fn selection_with_access(
    ...
    let access_id =
        AccessProfileId::new("opencode.acp.fixture.host-account").expect("valid access id");
    let instance_id =
        ConfiguredInstanceId::new("opencode.acp.fixture.instance").expect("valid instance");
    ...
    let version_binding = opencode_acp_binding("1.18.32").expect("fixture version is valid");
    let instance = ConfiguredInstance::new(
        instance_id.clone(),
        InstanceRevision::new("fixture-revision").expect("valid revision"),
        AdapterId::new("swallowtail.opencode.acp").expect("valid adapter"),
        host.clone(),
        InstanceTargetRef::new("opencode.acp.fixture.executable").expect("valid target"),
        InstanceOwnership::HostOwnedEphemeral,
        access_id.clone(),
        SupportAuthority::ProviderSupported,
        ProtocolFacadeId::new("acp-v1").expect("valid facade"),
        InstancePolicyId::new("opencode.acp.fixture.isolated").expect("valid policy"),
        capabilities.clone(),
    )
    .with_interface_versions([version_binding.clone()])
    .with_harness_configuration_posture(HarnessConfigurationPosture::Ambient);
    let access = opencode_acp_host_account_access_profile(access_id.clone());
    ...
    let plan = preflight(&context, &requirements).expect("OpenCode fixture preflight succeeds");
    FixtureSelection {
        plan,
        resource: WorkingResourceRef::new("opencode.acp.fixture.workspace").expect("valid resource"),
    }
}

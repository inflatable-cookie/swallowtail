...
| `goose.acp` | `swallowtail-adapter-goose`; `swallowtail.goose.acp` | one bounded interactive session; ACP v1 stdio; typed provider-authentication failures on `session/new` and `session/prompt` | approved executable and environment; provider-supported local Goose config state without a credential lease; explicit working resource | `goose.release`; exact `1.50.1`; no unverified-newer execution | `prepare_goose_acp` → `prepare_session` → `open_session` | `GooseAcpDriver`; `InteractiveSessionDriver` |
| `kiro.acp` | `swallowtail-adapter-kiro`; `swallowtail.kiro.acp` | one bounded interactive session; ACP v1 stdio | approved executable and environment; provider-supported local Kiro account state without a credential lease; explicit working resource | `kiro-cli.release`; exact `2.21.4`; no unverified-newer execution | `prepare_kiro_acp` → `prepare_session` → `open_session` | `KiroAcpDriver`; `InteractiveSessionDriver` |

...
pub(crate) fn parse_catalogue(
    ...
    if !response.success {
        return Err(failure(
            ...
            "Pi RPC rejected model catalogue discovery",
        ));
    }
    let models = response
        .data
...
fn source() -> IntegrationFamilyId {
    IntegrationFamilyId::new("pi").expect("static Pi family is valid")
}

fn protocol_failure() -> RuntimeFailure {
    failure(
        "swallowtail.pi.rpc.catalogue_invalid",
        "Pi RPC returned an invalid bounded model catalogue",
    )
}

...
Research: 075, 077, 087, 135; 183 to be produced by card 095
...
- [ ] freeze exact qualified CLI and official-documentation evidence for
      `fast`, `context`, and `effort`
- [ ] classify each exact model, parameter, and value tuple as deliver-now or
      evidence-gated
- [ ] add adapter-local typed inputs for only the deliver-now tuples
- [ ] preserve the existing plain-model path and its exact argv
- [ ] reject raw parameter grammar and unqualified model/value combinations
      before provider work
- [ ] bind Cursor `effort` to portable reasoning selection only where the
      exact tuple is qualified
- [ ] publish qualified-dispatch truth without claiming provider acceptance or
...
Card 095 freezes the exact model/parameter/value allowlist. Fast and context
...
- [ ] Execute card 095.
- [ ] freeze official help and documentation specimens without provider work
- [ ] promote Research 183 with the exact allowlist and evidence-gated rows

### Batch 35.2 — Typed Binding

- [ ] Execute card 096 after card 095.
- [ ] add typed adapter-local inputs and canonical rendering
- [ ] bind selected parameters into immutable plan, request, and evidence
- [ ] fail closed on raw grammar, unknown values, and unsupported combinations

### Batch 35.3 — Dispatch And Acceptance

- [ ] Execute card 097 after card 096.
- [ ] prove exact single-argument dispatch and unchanged plain-model behavior
- [ ] update realized architecture, guides, matrices, programme, and changelog

## Acceptance Criteria

- [ ] consumers never construct Cursor bracket grammar themselves
- [ ] only exact evidence-backed model/parameter/value tuples prepare
- [ ] typed parameters render in one canonical order into one `--model` argv
- [ ] the immutable plan carries the exact rendered model id
- [ ] effort request, capability constraint, model id, and argv agree
- [ ] fast and context remain route-local and do not become portable aliases
- [ ] plain-model preparation and dispatch remain unchanged
- [ ] documentation separates dispatched, accepted, and effective states
- [ ] deterministic QA performs no provider call or account inspection

## Lane Runway

- previous: g04.034 Gemini CLI 0.56.0 useful-newer qualification
- this milestone: Cursor headless model parameters
...
- [095-cursor-headless-model-parameter-evidence.md](batch-cards/095-cursor-headless-model-parameter-evidence.md) — ready
- [096-cursor-headless-model-parameter-binding.md](batch-cards/096-cursor-headless-model-parameter-binding.md) — ready after 095
- [097-cursor-headless-model-parameter-acceptance.md](batch-cards/097-cursor-headless-model-parameter-acceptance.md) — ready after 096

...
Planning state: cards 050-052 completed
Research: 170
...
- [x] expose an adapter-local installed addable descriptor for
      `claude-agent.acp`
- [x] admit through the 057 store on the local subscription profile
      without extracting keychain bytes
- [x] reuse `prepare_claude_agent` after admission
...
- [x] Execute card 050.
- [x] ship `AddableRouteDescriptor` from
      `swallowtail-adapter-claude-agent`
- [x] topology installed; config fields binary path and opaque env; no
      credential field

### Batch 18.2 — Admission And Prepare

- [x] Execute card 051 after card 050.
- [x] admit through the 057 store
- [x] no URL-open, loopback, or device-code ports; no keychain extraction
- [x] `prepare_claude_agent` still prepares after admission with
      `LocalUnauthenticated` + `SubscriptionAllowance`

### Batch 18.3 — Refresh, Update, And Subject

- [x] Execute card 052 after card 051.
- [x] refresh host-supplied `AccessStatus`; subject stays Absent
- [x] `observe_instance_update` reuses `claude_agent_acp_claim` and
      optional 032 observation
- [x] unmarked catalogue rows stay unmarked; do not invent a provider id

## Acceptance Criteria

- [x] a consumer can assemble a catalog that includes Claude Agent ACP by
      linking the adapter
- [x] subscription admission writes no secret bytes and no credential
      refs
- [x] `prepare_claude_agent` still runs after admission

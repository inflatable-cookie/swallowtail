...
pub(crate) struct LlamaCppActivityProjection {
    operation_id: ActivityOperationId,
    started: bool,
}

impl LlamaCppActivityProjection {
    ...
    fn observation(
        ...
        let mut observation = ActivityObservation::new(
            ActivityId::new("llama-cpp:assistant").map_err(|_| activity_drift())?,
...
fn activity_drift() -> RuntimeFailure {
    failure(
        "swallowtail.llama_cpp.activity_invalid",
        "llama.cpp activity did not match the qualified text stream",
    )
}

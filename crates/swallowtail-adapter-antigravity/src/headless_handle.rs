...
pub(crate) struct AntigravityHeadlessCancellation {
    process: Arc<dyn ProcessHandle>,
    requested: AtomicBool,
    scope: CancellationScope,
}

impl AntigravityHeadlessCancellation {
...
impl CancellationControl for AntigravityHeadlessCancellation {
...
pub(crate) struct AntigravityHeadlessRunHandle {
...
impl AntigravityHeadlessRunHandle {
    pub(crate) fn new(
        request_id: RequestId,
        run_id: RuntimeRunId,
...
impl RunHandle for AntigravityHeadlessRunHandle {
        ...
        Box::pin(async move {
            match self.task.join().await {
                ...
                Err(_) => CleanupOutcome::Failed(SafeDiagnostic::new(
                    "swallowtail.antigravity.headless.task_join_failed",
                    "Antigravity headless operation task could not be joined",
                )),
            }
        })
    }

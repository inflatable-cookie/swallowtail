use super::support::{
    AGENT_SESSION_BLOB, ARGS_BLOB, FROZEN_ARGS_JS_0_85_1, FROZEN_ARGS_JS_0_86_1, FROZEN_RPC_MODE_JS,
    FROZEN_RPC_TYPES_JS, GITHUB_TAG_COMMIT, IDENTITY, INTERMEDIATE_GITHUB_TAG_COMMIT, JSON_EVENT_BLOB,
    JSONL_BLOB, PRIOR_ARGS_BLOB, PRIOR_IDENTITY, PRIOR_RPC_MODE_JS, RPC_DOCS_BLOB, RPC_MODE_BLOB,
    RPC_TYPES_BLOB, SESSION_CWD_BLOB, TARBALL_SHA256, assert_blob, assert_sha256, json, strings,
};
use swallowtail_adapter_pi::PI_PACKAGE_AXIS;

#[test]
fn official_package_release_and_artifact_identity_is_exact() {
    let prior = json(PRIOR_IDENTITY);
    let identity = json(IDENTITY);
    assert_eq!(prior["official"]["version"], "0.85.1");
    assert_eq!(identity["axis"], PI_PACKAGE_AXIS);
    assert_eq!(identity["npm_package"], "@earendil-works/pi-coding-agent");
    assert_eq!(identity["npm_latest"], true);
    assert_eq!(identity["not_oh_my_pi"], true);
    assert_eq!(identity["oh_my_pi_latest_at_observation"], "18.2.7");
    assert_eq!(identity["dist_inventory"], "dist-inventory.json");
    assert_eq!(identity["official"]["version"], "0.86.1");
    assert_eq!(
        identity["official"]["published_at"],
        "2026-09-20T11:16:39.121Z"
    );
    assert_eq!(identity["official"]["github_tag"], "v0.86.1");
    assert_eq!(
        identity["official"]["github_release_published_at"],
        "2026-09-20T11:19:20Z"
    );
    assert_eq!(
        identity["official"]["npm_integrity"],
        "sha512-vZBuNfJnruxZyemZ3O05V0S/Ylze08ahFTIQ1Mik++gVdOevPl89gt/Uv0U97BPAJaj9cj6Vf9rcIgKtUrd0BA=="
    );
    assert_eq!(
        identity["official"]["npm_shasum"],
        "b8e38a10c51d28e946ac2740f13c01358f886a52"
    );
    assert_eq!(identity["official"]["npm_git_head"], GITHUB_TAG_COMMIT);
    assert_eq!(identity["official"]["github_tag_commit"], GITHUB_TAG_COMMIT);
    assert_eq!(identity["npm_git_head_matches_github_tag"], true);
    assert_sha256(&identity["official"]["tarball_sha256"], TARBALL_SHA256);
    assert_sha256(
        &identity["official"]["package_json_sha256"],
        "8e8e5cf99a033e9aae1db15e8455f819e1921b7c8351b1044acdab9525c14fd7",
    );
    assert_sha256(
        &identity["official"]["extracted_cli_sha256"],
        "8189b66abc4f9f431dbb70941dcba690d76d040de1fbfff212886be35a53639d",
    );
    assert_eq!(
        identity["official"]["extracted_cli_size"].as_u64(),
        Some(169)
    );
    assert_sha256(
        &identity["official"]["extracted_setup_js_sha256"],
        "4a2a7a0dbf82e2e5d18cec90896b36cbedce78b3d09e78d4040ac94fa3fbeba8",
    );
    assert_sha256(
        &identity["official"]["extracted_bundle_cli_sha256"],
        "e79626f2dd6f94aa45d30f3fa63cd84319a6eefcd150b353cfaf274366926774",
    );
    assert_eq!(
        identity["official"]["extracted_bundle_cli_size"].as_u64(),
        Some(160)
    );
    assert_sha256(
        &identity["official"]["extracted_bundle_cli_runtime_sha256"],
        "9b30cedde1105d7c54b9470632f8eba5e0d5fd65ba6f3a6ce4b6b0fae71d03bc",
    );
    assert_eq!(
        identity["official"]["extracted_bundle_cli_runtime_size"].as_u64(),
        Some(660)
    );
    assert_sha256(
        &identity["official"]["extracted_rpc_types_js_sha256"],
        FROZEN_RPC_TYPES_JS,
    );
    assert_sha256(
        &identity["official"]["extracted_rpc_mode_js_sha256"],
        FROZEN_RPC_MODE_JS,
    );
    assert_sha256(
        &identity["official"]["extracted_args_js_sha256"],
        FROZEN_ARGS_JS_0_86_1,
    );
    assert_sha256(
        &prior["official"]["extracted_rpc_types_js_sha256"],
        FROZEN_RPC_TYPES_JS,
    );
    assert_sha256(
        &prior["official"]["extracted_rpc_mode_js_sha256"],
        PRIOR_RPC_MODE_JS,
    );
    assert_sha256(
        &prior["official"]["extracted_args_js_sha256"],
        FROZEN_ARGS_JS_0_85_1,
    );
}

#[test]
fn intermediate_0_86_0_identity_is_exact() {
    let identity = json(IDENTITY);
    let hop = &identity["published_intermediate_0_86_0"];
    assert_eq!(hop["version"], "0.86.0");
    assert_eq!(
        hop["role"],
        "intermediate_supporting_evidence_not_standalone_ceiling"
    );
    assert_eq!(hop["published_at"], "2026-09-19T23:14:16.198Z");
    assert_eq!(hop["github_tag"], "v0.86.0");
    assert_eq!(hop["npm_git_head"], INTERMEDIATE_GITHUB_TAG_COMMIT);
    assert_eq!(hop["github_tag_commit"], INTERMEDIATE_GITHUB_TAG_COMMIT);
    assert_sha256(
        &hop["tarball_sha256"],
        "3f0f502f497c44888ddbd68a11897651b739eaabdf4b235e3a7ed9735353b0cd",
    );
    assert_sha256(&hop["extracted_args_js_sha256"], FROZEN_ARGS_JS_0_85_1);
    assert_sha256(&hop["extracted_rpc_mode_js_sha256"], FROZEN_RPC_MODE_JS);
    assert_eq!(
        hop["extracted_rpc_mode_js_sha256"],
        identity["official"]["extracted_rpc_mode_js_sha256"]
    );
    assert_ne!(
        hop["extracted_args_js_sha256"],
        identity["official"]["extracted_args_js_sha256"]
    );
    assert_eq!(
        hop["extracted_agent_session_js_sha256"],
        identity["official"]["extracted_agent_session_js_sha256"]
    );
}

#[test]
fn host_was_not_installed_and_artifacts_were_not_executed() {
    let identity = json(IDENTITY);
    assert_eq!(identity["host"]["not_installed"], true);
    assert_eq!(identity["identity_decision"]["host_install_changed"], false);
    assert_eq!(
        identity["identity_decision"]["official_binary_executed"],
        false
    );
}

#[test]
fn selected_source_blobs_are_exact() {
    let identity = json(IDENTITY);
    let blobs = &identity["selected_blobs_at_0_86_1"];
    assert_blob(&blobs["rpc_docs"], RPC_DOCS_BLOB);
    assert_blob(&blobs["rpc_types"], RPC_TYPES_BLOB);
    assert_blob(&blobs["rpc_mode"], RPC_MODE_BLOB);
    assert_blob(&blobs["jsonl"], JSONL_BLOB);
    assert_blob(&blobs["session_cwd"], SESSION_CWD_BLOB);
    assert_blob(&blobs["json_event"], JSON_EVENT_BLOB);
    assert_blob(&blobs["args"], ARGS_BLOB);
    assert_blob(&blobs["agent_session"], AGENT_SESSION_BLOB);
    assert_eq!(
        strings(&identity["selected_blobs_identical_to_0_85_1"]),
        [
            "rpc_docs",
            "rpc_types",
            "jsonl",
            "session_cwd",
            "json_event"
        ]
    );
    assert_eq!(
        strings(&identity["selected_blobs_identical_0_86_0_and_0_86_1"]),
        [
            "rpc_docs",
            "rpc_types",
            "rpc_mode",
            "jsonl",
            "session_cwd",
            "json_event",
            "agent_session"
        ]
    );
    let prior = json(PRIOR_IDENTITY);
    let prior_blobs = &prior["selected_blobs_at_0_85_1"];
    assert_eq!(prior_blobs["rpc_docs"], blobs["rpc_docs"]);
    assert_eq!(prior_blobs["rpc_types"], blobs["rpc_types"]);
    assert_eq!(prior_blobs["jsonl"], blobs["jsonl"]);
    assert_eq!(prior_blobs["session_cwd"], blobs["session_cwd"]);
    assert_eq!(prior_blobs["json_event"], blobs["json_event"]);
    assert_ne!(prior_blobs["rpc_mode"], blobs["rpc_mode"]);
    assert_ne!(prior_blobs["args"], blobs["args"]);
    let hop = &identity["selected_blobs_at_0_86_0"];
    assert_eq!(hop["rpc_docs"], blobs["rpc_docs"]);
    assert_eq!(hop["rpc_mode"], blobs["rpc_mode"]);
    assert_eq!(hop["agent_session"], blobs["agent_session"]);
    assert_blob(&hop["args"], PRIOR_ARGS_BLOB);
    assert_ne!(hop["args"], blobs["args"]);
}

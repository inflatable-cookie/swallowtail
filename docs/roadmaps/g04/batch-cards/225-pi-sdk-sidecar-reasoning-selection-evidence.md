...
1. [x] Reuse and verify the exact route, driver, package, Node, wire,
   source-tag, access, resource, model, persistence, replay, and cleanup
   boundaries from Research 181 and g04.033.
2. [x] Freeze exact `0.84.2` public types and tagged source for
...
3. [x] Enumerate the exact Pi vocabulary and distinguish accepted input,
...
4. [x] Build a closed provider/model/value table for only models already
   ...
   or provider work.
5. [x] Classify new, load, resume, session replacement, and fresh
...
6. [x] Freeze state confirmation and event ordering. Determine whether
...
7. [x] Audit the current private wire. It accepts `thinkingLevel` and fixtures
...
8. [x] Audit preparation, `SessionOptions`, capability/profile construction,
   ...
   baseline.
9. [x] Prove omission retains exact prior bytes and behavior. Empty options
   must not claim Pi's default or stored mode as caller-selected.
10. [x] Keep mode changing, cycling, model switching, raw settings, `pi.rpc`,
    newer SDK currentness, and provider execution out.
11. [x] Promote Research 228 with frozen sources, exact tables, and a non-empty
    deliver-now set or explicit empty set. Update milestone/card state and
    close out honestly.

## Acceptance Criteria

- [x] exact vocabulary, model membership, clamp, persistence, replacement,
      and state-confirmation semantics are frozen
- [x] every lifecycle has an explicit deliver-now or withheld disposition
- [x] the pre-effect rejection boundary for unsupported rows is exact
- [x] current Rust, sidecar, fixture, docs, and public seams are audited
- [x] Research 228 contains a non-empty exact table or honest empty set
- [x] no production code, public API, currentness, release, merge, rollover,

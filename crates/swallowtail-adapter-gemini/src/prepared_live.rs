    ...
    pub fn validate_execution_binding(
        ...
        if host != self.instance.execution_host_id() || target != self.instance.target_reference() {
            return Err(failure(
                ...
                "swallowtail.gemini.live_preparation.target_drift",
                "Prepared Gemini Live host or endpoint no longer matches",
            ));
        }
        Ok(())
    }
...
fn validate_input(
    ...
    if services.execution_host_id() != &input.execution_host_id {
        return Err(failure(
            PreparationStage::TargetSelection,
            "swallowtail.gemini.live_preparation.host_mismatch",
            "Gemini Live preparation services belong to a different execution host",
        ));
    }
    if input.access_profile.id().as_str() != crate::GEMINI_LIVE_ACCESS_PROFILE_ID
        ...
        || input.access_profile.endpoint_audience().as_str() != crate::GEMINI_LIVE_ENDPOINT_AUDIENCE
    {
        return Err(failure(
            PreparationStage::AccessEvidence,
            "swallowtail.gemini.live_preparation.access_profile_rejected",
            "Gemini Live preparation requires the project authorization API-key profile",
        ));
    }
        ...
        || status.support_authority() != input.access_profile.support_authority()
    {
        return Err(failure(
            PreparationStage::AccessEvidence,
            "swallowtail.gemini.live_preparation.access_evidence_mismatch",
            "Gemini Live access evidence does not match the selected profile",
        ));
    }
    Ok(())
}

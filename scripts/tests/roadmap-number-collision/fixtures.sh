...
collision_write_task \
  "$collision_updater" \
  docs/roadmaps/g04/077-kimi-platform-chat-admission-and-prepare.md \
  '# 077 Kimi Platform'
collision_write_task \
  "$collision_updater" \
  docs/roadmaps/g04/078-kimi-platform-chat-refresh-catalogue-and-047.md \
  '# 078 Kimi Platform'
collision_git "$collision_updater" add docs
collision_git "$collision_updater" commit -q -m 'g04.024 compiles tasks 076-078'

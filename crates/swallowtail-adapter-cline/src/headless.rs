//! Cline headless `--json` structured-run driver.
...
use handle::{ClineHeadlessCancellation, ClineHeadlessRunHandle};
use std::sync::Arc;
...
pub(crate) const DRIVER_ID: &str = "swallowtail.cline.headless";
const EVENT_CAPACITY: usize = 4098;

/// Low-level structured-run driver for installed `cline --json`.
pub struct ClineHeadlessDriver {
    environment: EnvironmentRef,
}

impl ClineHeadlessDriver {
...
/// Describes the installed Cline headless discovery and structured-run roles.
#[must_use]
pub fn cline_headless_descriptor() -> DriverDescriptor {
    DriverDescriptor::new(
        ...
        IntegrationFamilyId::new("cline").expect("static family id is valid"),
        TransportFamilyId::new("cline-json-ndjson-stdio").expect("static transport id is valid"),
    )
    ...
    .with_interface_compatibility(crate::cline_headless_claim())
}

impl StructuredRunDriver for ClineHeadlessDriver {
    fn start_run(
        &self,
        plan: PreflightPlan,
...
impl ClineHeadlessDriver {
    async fn start(
        ...
        let cwd = match resource.filesystem() {
            ...
            None => {
                let _ = resource_service.release(resource).await;
                return Err(failure(
                    "swallowtail.cline.headless.working_resource_rejected",
                    "Cline headless requires a materialized filesystem working resource",
                ));
            }
        };
        ...
        let cancellation = Arc::new(ClineHeadlessCancellation::new(Arc::clone(&process)));
        ...
        Ok(Box::new(ClineHeadlessRunHandle::new(
    ...
    fn validate_plan(&self, plan: &PreflightPlan) -> Result<(), RuntimeFailure> {
        if plan.driver_identity().id().as_str() != DRIVER_ID {
            return Err(failure(
                "swallowtail.cline.headless.plan_driver_mismatch",
        ...
        {
            return Err(failure(
                "swallowtail.cline.headless.access_profile_rejected",
                "Cline headless requires its host-owned local-account profile",
            ));
        }
        if plan.harness_configuration_posture() != Some(HarnessConfigurationPosture::Ambient) {
            return Err(failure(
                "swallowtail.cline.headless.configuration_posture_rejected",
                "Cline headless requires explicit ambient configuration inside its selected environment",
            ));
        }
        if plan.requirements().harness_isolation() != Some(HarnessIsolation::AmbientHost) {
            return Err(failure(
                "swallowtail.cline.headless.isolation_rejected",
                "Cline headless requires explicit ambient-host isolation posture",
            ));
        }
        crate::selection::select_cline_headless_plan(plan).map(|_| ())
    }
}

fn runtime_run_id(request_id: &str) -> Result<RuntimeRunId, RuntimeFailure> {
    RuntimeRunId::new(format!("cline-headless:{request_id}")).map_err(|_| invalid_request())
}

fn runtime_scope(request_id: &str) -> Result<ScopeId, RuntimeFailure> {
    ScopeId::new(format!("cline-headless:{request_id}")).map_err(|_| invalid_request())
}

fn invalid_request() -> RuntimeFailure {
    failure(
        "swallowtail.cline.headless.invalid_request",
        "Cline headless request identity is invalid",
    )
}

#[cfg(test)]
mod tests {
    use super::cline_headless_descriptor;
    ...
    fn descriptor_is_structured_run_only_and_not_acp() {
        ...
        assert_eq!(descriptor.integration_family().as_str(), "cline");
        assert_eq!(
            descriptor.transport_family().as_str(),
            "cline-json-ndjson-stdio"
        );

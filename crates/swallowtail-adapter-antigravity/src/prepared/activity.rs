...
pub(super) fn profile(
    ...
    let behavior = match observation.compatibility() {
        ...
        InstalledExecutableCompatibility::Incompatible => {
            return Err(failure(
                "swallowtail.antigravity.preparation.activity_version_incompatible",
                "Antigravity activity requires a permitted executable version",
            ));
        }
    };
    ...
    .map_err(|_| {
        failure(
            "swallowtail.antigravity.preparation.activity_profile_invalid",
            "Antigravity activity profile could not be derived",
        )
    })
}
...
pub(super) fn with_activity(
    ...
    requirements.push(
        activity
            ...
            .expect("prepared Antigravity activity is available"),
    );

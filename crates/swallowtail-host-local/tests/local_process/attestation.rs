...
//!
//! These tests are the falsification half of that question. Each one takes a
//! candidate positive mechanism and drives one of the three counterexample
//! classes named by the review oracle through it, natively, on the host that
//! runs them:
//!
...
//!
//! Together they show that no single mechanism available under the authorized
//! boundary defeats all three on macOS, which lacks a PID namespace, a cgroup
//! `populated` view, and a child-subreaper. The sound mechanism is a
//! kernel-enforced owned-tree container, and it does not exist on this host, so
//! the local host stays root-only rather than publishing a best-effort claim.

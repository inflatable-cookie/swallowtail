...
| `claude-agent.sdk` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | qualified exact `0.3.258` on `claude-agent.sdk.package` plus exact native, Node, wire, and sidecar source-tag points; session persistence is disabled, so no provider session is retained, loaded, or resumed | sidecar close joins its retained native handle to a declared 2000 ms bound and the host terminates the tree it owns; an observed root exit is reported degraded because the host does not attest tree-emptiness, and an unobserved exit is cleanup failure |

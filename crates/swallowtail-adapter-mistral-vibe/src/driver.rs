//! Mistral Vibe `--prompt --output streaming` structured-run driver.
...
use swallowtail_runtime::{
    ...
    terminal_outcome_channel, validate_session_resource_lease,
};

pub(crate) const DRIVER_ID: &str = "swallowtail.mistral-vibe.headless";
...
/// Describes the installed Vibe headless discovery and structured-run roles.
#[must_use]
pub fn mistral_vibe_headless_descriptor() -> DriverDescriptor {
    DriverDescriptor::new(
        ...
        IntegrationFamilyId::new("mistral-vibe").expect("static family id is valid"),
        TransportFamilyId::new("mistral-vibe-streaming-ndjson-stdio")
    ...
    .with_interface_compatibility(crate::mistral_vibe_headless_claim())
}

impl StructuredRunDriver for MistralVibeHeadlessDriver {
    fn start_run(
        &self,
        plan: PreflightPlan,
...
impl MistralVibeHeadlessDriver {
    async fn start(
        ...
        let cwd = match resource.filesystem() {
            ...
            None => {
                let _ = resource_service.release(resource).await;
                return Err(failure(
                    "swallowtail.mistral-vibe.headless.working_resource_rejected",
                    "Mistral Vibe headless requires a materialized filesystem working resource",
                ));
            }
        };
        ...
        let cancellation = Arc::new(VibeHeadlessCancellation::new(Arc::clone(&process)));
        ...
        Ok(Box::new(VibeHeadlessRunHandle::new(
    ...
    fn validate_plan(&self, plan: &PreflightPlan) -> Result<(), RuntimeFailure> {
        if plan.driver_identity().id().as_str() != DRIVER_ID {
            return Err(failure(
                "swallowtail.mistral-vibe.headless.plan_driver_mismatch",
        ...
        if plan.credential_mechanism() != &CredentialMechanism::LocalUnauthenticated
            ...
            || plan.endpoint_audience().as_str() != crate::MISTRAL_VIBE_LOCAL_ACCOUNT_AUDIENCE
        {
            return Err(failure(
                "swallowtail.mistral-vibe.headless.access_profile_rejected",
                "Mistral Vibe headless requires its host-owned local-config profile",
            ));
        }
        if plan.harness_configuration_posture() != Some(HarnessConfigurationPosture::Ambient) {
            return Err(failure(
                "swallowtail.mistral-vibe.headless.configuration_posture_rejected",
                "Mistral Vibe headless requires explicit ambient configuration inside its selected environment",
            ));
        }
        if plan.requirements().harness_isolation() != Some(HarnessIsolation::AmbientHost) {
            return Err(failure(
                "swallowtail.mistral-vibe.headless.isolation_rejected",
                "Mistral Vibe headless requires explicit ambient-host isolation posture",
            ));
        }
        crate::selection::select_mistral_vibe_headless_plan(plan).map(|_| ())
    }
}

fn runtime_run_id(request_id: &str) -> Result<RuntimeRunId, RuntimeFailure> {
    RuntimeRunId::new(format!("mistral-vibe-headless:{request_id}")).map_err(|_| invalid_request())
}

fn runtime_scope(request_id: &str) -> Result<ScopeId, RuntimeFailure> {
    ScopeId::new(format!("mistral-vibe-headless:{request_id}")).map_err(|_| invalid_request())
}

fn invalid_request() -> RuntimeFailure {
    failure(
        "swallowtail.mistral-vibe.headless.invalid_request",
        "Mistral Vibe headless request identity is invalid",
    )
}

#[cfg(test)]
mod tests {
    use super::mistral_vibe_headless_descriptor;
    use swallowtail_core::{DriverRole, ExecutionLayer, OperationShape};

    #[test]
    fn descriptor_is_structured_run_only_and_not_acp() {
        let descriptor = mistral_vibe_headless_descriptor();
        assert_eq!(descriptor.integration_family().as_str(), "mistral-vibe");
        assert_eq!(
            descriptor.transport_family().as_str(),
            "mistral-vibe-streaming-ndjson-stdio"
        );

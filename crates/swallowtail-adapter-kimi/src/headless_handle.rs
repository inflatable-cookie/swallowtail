...
pub(crate) struct KimiHeadlessCancellation {
    process: Arc<dyn ProcessHandle>,
    requested: AtomicBool,
}

impl KimiHeadlessCancellation {
...
impl CancellationControl for KimiHeadlessCancellation {
...
pub(crate) struct KimiHeadlessRunHandle {
...
impl RunHandle for KimiHeadlessRunHandle {
        ...
        Box::pin(async move {
            match self.task.join().await {
                ...
                Err(_) => CleanupOutcome::Failed(SafeDiagnostic::new(
                    "swallowtail.kimi.headless.task_join_failed",
                    "Kimi headless operation task could not be joined",
                )),
            }
        })
    }

...
def main() -> None:
    ...
    if len(rows) != 67:
        raise SystemExit("provider activity matrix must contain exactly 67 rows")
    counts = Counter(row["activity_profile"] for row in rows)
    if counts != Counter({"available": 41, "not-applicable": 26}):
        raise SystemExit(
    ...
    print(
        ...
        "67 operations, 41 available, 26 not-applicable, "
        "34 production routes, 4 auxiliary catalogues, "

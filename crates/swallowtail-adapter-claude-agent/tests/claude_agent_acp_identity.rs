...
const PROTOCOL: &str = include_str!("fixtures/claude-agent-acp-0.69.0/protocol.json");

#[test]
fn identity_and_claim_qualify_0_69_0_as_compatible_extension() {
    let identity: Value =
        serde_json::from_str(IDENTITY).expect("Claude Agent 0.69.0 identity corpus is valid JSON");
    let protocol: Value =
        serde_json::from_str(PROTOCOL).expect("Claude Agent 0.69.0 protocol corpus is valid JSON");
    ...
    assert_eq!(identity["official"]["version"], "0.69.0");
    assert_eq!(
    ...
    assert_eq!(
        ...
        "59a7e9367b3931a50178de4783cf6074b20060cd"
    );
    assert!(is_sha256(
    ...
    assert_eq!(
        identity["unpublished_patch_in_0_64_1_through_0_69_0"],
        false
    );
    ...
    assert_eq!(decision["extend_v6"], "0.64.0..=0.65.0");
    assert_eq!(decision["add_private_v7"], "0.66.0..=0.69.0");
    assert_eq!(
        decision["v7_behavior"],
        "claude-agent.acp.initialize-meta-extensions-v7"
    );
    assert_eq!(decision["raise_latest_qualified_to"], "0.69.0");
    ...
    assert_eq!(
        protocol["elicitation_js_identical_0_64_0_through_0_69_0"],
        true
    );
    assert_eq!(protocol["protocol_version"], 1);
    assert_eq!(protocol["allow_always_still_unmapped"], true);
    assert_eq!(protocol["unmapped_initialize_meta"]["goal_from"], "0.66.0");

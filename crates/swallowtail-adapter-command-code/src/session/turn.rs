use super::CommandCodeSessionHandle;
use super::lifecycle::{ActiveTurn, join_turn, reap_finished};
use crate::command::{interactive_arguments, resumed_arguments};
...
impl CommandCodeSessionHandle {
    pub(super) async fn start_turn_inner(
        ...
        if self
            .active
            .lock()
            .expect("Command Code active turn lock poisoned")
            .is_some()
        {
            return Err(failure(
                "swallowtail.command_code.headless.turn_active",
                "Command Code interactive session already has an active turn",
            ));
        }
        ...
        let scope = ScopeId::new(format!(
            "command-code-headless:turn:{}",
            request.turn_id().as_str()
        ))
        .map_err(|_| {
            failure(
                "swallowtail.command_code.headless.scope_invalid",
                "Command Code turn scope was invalid",
            )
        })?;
        let process_request =
            ProcessRequest::new(ExecutableRef::from_instance_target(&self.target))
                .with_arguments(match expected_session.as_deref() {
                    ...
                    None => interactive_arguments(&self.model),
                })
        ...
        let cancellation = Arc::new(CommandCodeCancellation::with_scope(
            Arc::clone(&process),
        ...
        *self
            .active
            .lock()
            .expect("Command Code active turn lock poisoned") = Some(ActiveTurn {
            turn_id: turn_id.clone(),
            task: Some(task),
            cancellation: Arc::clone(&cancellation),
            terminal: Arc::clone(&terminal_flag),
        });
        Ok(Box::new(CommandCodeTurnHandle {
            turn_id,
...
impl TurnHandle for CommandCodeTurnHandle {
...
fn validate_turn(
    session: &CommandCodeSessionHandle,
    request: &TurnRequest,
    ...
    if !state.usable {
        return Err(failure(
            "swallowtail.command_code.headless.session_unusable",
            "Command Code interactive session can no longer accept turns",
        ));
    }
    if state.completed_turns >= 24 {
        return Err(failure(
            "swallowtail.command_code.headless.turn_limit",
            "Command Code interactive session reached its bounded turn limit",
        ));
    }
    drop(state);
        ...
        >= deadline.instant()
    {
        return Err(failure(
            ...
            "Command Code turn deadline elapsed before provider work",
        ));
    }
    Ok(())
}

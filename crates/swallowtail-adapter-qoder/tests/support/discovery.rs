...
impl DiscoveryHost {
    ...
    pub fn observed_process(&self) -> Option<ObservedProcess> {
        self.process
            .lock()
...
impl ProcessService for DiscoveryHost {
    fn start(
        &self,
        _scope: ScopeId,
        ...
        *self.process.lock().expect("fixture process lock poisoned") = Some(ObservedProcess {
        ...
        let output = self
            .output
            .lock()
            .expect("fixture output lock poisoned")
            .take()
            .expect("fixture process starts once");
        Box::pin(async move {
            Ok(Box::new(VersionProcess(Mutex::new(output))) as Box<dyn ProcessHandle>)
        })
    }
}

struct VersionProcess(Mutex<VecDeque<ProcessOutputChunk>>);

impl ProcessHandle for VersionProcess {

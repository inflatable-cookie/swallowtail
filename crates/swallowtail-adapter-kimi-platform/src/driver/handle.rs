...
impl RunHandle for KimiPlatformRunHandle {
    ...
    fn cancellation(&self) -> &dyn CancellationControl { self.cancellation.as_ref() }
        ...
        Box::pin(async move {
            ...
            match self.task.join().await {
                ...
                Err(_) => CleanupOutcome::Failed(SafeDiagnostic::new(
                    "swallowtail.kimi_platform.task_join_failed",
                    "Kimi Platform operation task could not be joined",
                )),
            }
        })
    }

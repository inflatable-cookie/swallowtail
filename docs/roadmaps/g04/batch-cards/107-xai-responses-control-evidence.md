...
- current official evidence is frozen without secrets or provider output
- models, aliases, values, controls, profiles, and continuation shapes have
  explicit dispositions
- WebSocket-versus-Responses-body equivalence is explicit for each control
- multi-agent effort is excluded from portable reasoning depth
- current facade and contract decisions are explicit
- Research 187 is promoted and indexed
- production code, claims, matrices, architecture, and changelog are unchanged
- `effigy validate:focused swallowtail-adapter-xai` passes

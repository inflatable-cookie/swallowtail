# 271 All-Route Version Currentness Checkpoint
...
After Kimi Code installed-harness `0.39.1` and the operator A2 ACP cap, what
are the current official stable points and safe local observations for every
production family, and which one family should enter the next Contract 029
Upgrade Workflow run?

This checkpoint is research only. It does not change a compatibility claim,
feature-matrix row, fixture, or route. It does not reopen `kimi-code.acp`.
...
`kimi-code.acp` is `QualifiedOnly` at `0.38.0` with exact `0.39.0` and
`0.39.1` excluded. This record does not reopen it. Headless `0.39.1` is
on-ceiling. `kimi-code.local-server` stays its own family.
...
Partition: 12 unchanged, 7 visible unverified-newer, 20 record-only, 1 material
candidate.
...
| 4 | Claude Agent ACP (`claude-agent.acp`) | `claude-agent-acp` resolves; `--version` `0.63.0` | npm [`@agentclientprotocol/claude-agent-acp` `0.72.0`](https://registry.npmjs.org/@agentclientprotocol%2Fclaude-agent-acp/latest), published 2026-09-01; GitHub [`v0.72.0`](https://github.com/agentclientprotocol/claude-agent-acp/releases/tag/v0.72.0) | `0.53.0..=0.70.0` excluding `0.58.0`; AllowUnverified | material candidate |
| 5 | Claude Code headless and response-only (`claude-code.headless`; `claude-code.response-only`) | `claude` resolves; `--version` `2.1.251 (Claude Code)` | npm [`@anthropic-ai/claude-code` `2.1.257`](https://registry.npmjs.org/@anthropic-ai%2Fclaude-code/latest), published 2026-09-01; GitHub [`v2.1.257`](https://github.com/anthropics/claude-code/releases/tag/v2.1.257) | headless `2.1.220..=2.1.252`; response-only `2.1.227..=2.1.252`; gaps `2.1.244` and `2.1.249`; AllowUnverified; watcher stays exact `2.1.251` | visible unverified-newer |
...
| 31 | Oh My Pi RPC (`oh-my-pi.rpc`) | `omp` resolves; `--version` `omp/18.0.11` | npm [`@oh-my-pi/pi-coding-agent` `18.1.1`](https://registry.npmjs.org/@oh-my-pi%2Fpi-coding-agent/latest), published 2026-09-01; major line reset from 17.x | maintained `17.2.9..=17.4.0`; later stable unverified only under ordered semantics; major reset requires identity evidence | record only; future range work deferred |
...
| 38 | Grok Build ACP (`grok-build.acp`) | `grok` resolves; `--version` `grok 1.0.13 (5e9a58528b76) [stable]` | npm [`@xai-official/grok` latest `1.0.13`](https://registry.npmjs.org/@xai-official/grok/latest), published 2026-08-28; `1.0.14`, `1.0.15`, and `1.0.16` exist off `latest`; `alpha` is `1.0.17` and ignored; ACP registry `1.0.17` is discovery metadata | deprecated `0.2.114..=0.2.117`; maintained `1.0.4..=1.0.5`; later stable unverified; do not flatten registry/alpha identity into npm `latest` | visible unverified-newer |
...
| ACP agent registry | n/a | registry [`version` `1.0.0`](https://cdn.agentclientprotocol.com/registry/v1/latest/registry.json), 39 agents; selected entries include Claude `0.72.0`, Cursor `2026.08.31`, Gemini `0.57.0`, and Qwen `0.22.3` | discovery metadata only; it cannot move package, executable, schema, or facade claims | record only; future range work deferred |

## Changed Observations Since Research 269

- Kimi Code installed harness is no longer the next-family candidate. Headless
  official `0.39.1` now matches the qualified v2 ceiling after g05.016.
  `kimi-code.acp` is `QualifiedOnly` at `0.38.0` after g05.017 / operator A2,
  with exact `0.39.0` and `0.39.1` excluded. Classification moves from material
  candidate to record-only. Do not reopen ACP. Do not flatten onto
  `kimi-code.local-server`.
- Claude Agent ACP npm and GitHub moved `0.70.0` → `0.72.0`. Published stables
  after the current `0.70.0` ceiling are exactly `0.71.0` and `0.72.0`. Host
  `0.63.0` remains inside the qualified window. Classification moves from
  unchanged to material candidate.
- Claude Code npm and GitHub moved `2.1.252` → `2.1.257`. The only published
  stable after `2.1.252` is `2.1.257`; `2.1.253` through `2.1.256` are absent.
  Existing unpublished gaps `2.1.244` and `2.1.249` remain. Watcher stays exact
  `2.1.251`. Classification moves from unchanged to visible unverified-newer.
- Cursor ACP registry moved `2026.08.11` → `2026.08.31`, with download identity
  `2026.08.31-4057e58`. Host `2026.08.04-aaa8809` remains a qualified exact
  build. Classification moves from unchanged to visible unverified-newer.
- Oh My Pi npm `latest` moved `18.1.0` → `18.1.1`. The 18.x major reset stays
  record-only.
- Copilot prerelease moved `1.0.83-0` → `1.0.83-1`. Official `latest` remains
  `1.0.82`. Exact `1.0.80` stays record-only.
- Grok npm `1.0.16` is now a published version off `latest`. Official `latest`
  remains `1.0.13`. Alpha and ACP registry `1.0.17` stay discovery-only.

All other official points and host observations match Research 269.

## Rank After This Record

1. **Claude Agent ACP `0.72.0`** — the host is already on a qualified bound at
   `0.63.0`, the official npm and GitHub stable point is `0.72.0`, and the
   current claim already permits an exact forward attempt. Published stable
   points after the current `0.70.0` ceiling are exactly `0.71.0` and
   `0.72.0`. The next ordered stable `0.73.0` is not published. Exclusion
   `0.58.0` remains. Do not flatten this family onto Claude Code.

No second family is selected in this record. Claude Code `2.1.257`, Cursor
`2026.08.31-4057e58`, OpenCode, Ollama, Antigravity, Grok, and
`kimi-code.local-server` remain visible ordered newer candidates for later
one-family runs. `kimi-code.local-server` needs its own identity run before any
claim. Gemini remains deferred by operator decision. `kimi-code.acp` remains
`QualifiedOnly` at `0.38.0` and is not reopened. Oh My Pi's 18.x reset,
exact/opaque families, the exact llama.cpp build axes, the DeepSeek RC axis,
and the Bedrock SDK pins remain record-only until their named identity or
...
Compile Claude Agent ACP `0.72.0` next through the Contract 029 Upgrade
Workflow, one family only. This checkpoint itself changes no claim, selection,
matrix, fixture, or route. Do not bulk-bump from registry `latest`; do not
contact a provider; do not lift Gemini's deferral; do not reopen
`kimi-code.acp`. Do not compile the identity or claim cards in this record.
...
- [Research 127](./127-all-route-version-currentness-checkpoint.md),
  ...
  [Research 269](./269-all-route-version-currentness-checkpoint.md), and
...
- frozen currentness records [127](./127-all-route-version-currentness-checkpoint.md),
  ...
  [269](./269-all-route-version-currentness-checkpoint.md), and
  [270](./270-kimi-code-0-39-1-identity.md)

...
pub(super) fn profile(
    ...
    let behavior = match observation.compatibility() {
        ...
        InstalledExecutableCompatibility::Incompatible => {
            return Err(failure(
                "swallowtail.goose.acp.preparation.activity_version_incompatible",
                "Goose ACP activity requires the exact permitted release",
            ));
        }
    };
    ...
    .map_err(|_| {
        failure(
            "swallowtail.goose.acp.preparation.activity_profile_invalid",
            "Goose ACP activity profile could not be derived",
        )
    })
}
...
pub(super) fn with_activity(
    ...
    requirements.push(
        activity
            ...
            .expect("prepared Goose activity is available"),
    );

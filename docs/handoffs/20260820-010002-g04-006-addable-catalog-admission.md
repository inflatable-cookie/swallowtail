---
title: g04.006 addable catalog and admission worker handoff
...
created: 2026-08-20
updated: 2026-08-20
handoff_path: /Users/tom/Dev/projects/swallowtail/docs/handoffs/20260820-010002-g04-006-addable-catalog-admission.md
...
The connection-lifecycle kernel is on `main`. Apps still cannot assemble a
list of addable routes or admit a configured instance into the store.
Discovery still probes one already selected driver. This thread is the
catalog, admission, and config-field wiring in front of that store.
...
Without a catalog and admission API, the store is a bag of records consumers
cannot fill from addable-route descriptors. This is still a library. It is
not a registry of every production route, and it is not sign-in.
...
- **Planning base commit:** `c64bd64d31fa0bcfd34ea68a2c42aaeb9f0ba320`
...
- **Planning artifacts included at the base:** Contract 057, realized kernel
  records and `ConnectionLifecycleStore`, milestone g04.006, ready cards
  016-018, planned g04.007
- **Worker branch:** `g04-006-addable-catalog-admission`
- **Worker worktree:** launcher-provided dedicated worktree first. Manual
  fallback path is
  `$AGENTS_WORKTREE_CONTAINER_DIR/swallowtail-g04-006-addable-catalog-admission`
- **Worktree creation command:** only if the current context is unusable and
  ...
  `git worktree add -b g04-006-addable-catalog-admission "$AGENTS_WORKTREE_CONTAINER_DIR/swallowtail-g04-006-addable-catalog-admission" origin/main`
...
- **Roadmap milestone:** `docs/roadmaps/g04/006-addable-catalog-admission-and-config-fields.md`
- **Ready cards, in order:**
  `docs/roadmaps/g04/batch-cards/016-addable-route-catalog.md`,
  then `docs/roadmaps/g04/batch-cards/017-instance-admission.md`,
  then `docs/roadmaps/g04/batch-cards/018-config-field-descriptors.md`
- **Allowed runway:** g04.006 cards 016 → 017 → 018. Stop after config-field
  wiring. Do not start cards 019-021.
- **Remaining card budget:** three cards, one PR
- **Dispatch topology:** serial; one worker, one worktree, one PR
- **Parallel safety check:** catalog assembly, admission, and config-field
  wiring share one type graph and the store. Sign-in depends on this
  admission. No parallel lane.
- **Canonical refs:** Contract 057;
  `docs/architecture/system-architecture.md` connection-lifecycle section;
  Contracts 006, 008, 014, 037, 047; `docs/contracts/001-working-rules.md`;
  kernel types in `swallowtail-core::connection_lifecycle` and
  `swallowtail-runtime::ConnectionLifecycleStore`
...
- **PR base/head:** `main` / selected worker branch
  (`g04-006-addable-catalog-admission` unless the launcher supplied a
...
- **In scope:** consumer-assembled addable-route catalog from adapter-local
  descriptors; testkit fixture descriptors only; admission of a configured
  instance through `ConnectionLifecycleStore` as `AdmittedInstanceRecord`;
  several instances of one family as distinct ids; config-field descriptors
  attached as opaque `ConfigFieldRef` values.
- **Out of scope:** production adapter descriptors; an umbrella registry
  crate; Contract 008 discovery as the catalog; Contract 037 preparation;
  sign-in loop and new host ports; readiness refresh; overlay projection;
  first-proof Anthropic, Codex, or Ollama wiring; cards 019-021; rewriting
  `public-api-0.3.3`; GitHub Release; crates.io; tag mutation; consumer
  repository edits.
- Do not invent architecture or change Contract 057.
- Topology grouping is hosted / installed / local-runtime. It is not
  `ExecutionLayer`.
- A discovered candidate is not an addable-route row and cannot be
  admitted as if it were. Absence of a descriptor means the consumer did
  not link that adapter.
- Admission writes through the existing store. It does not prepare, select
  a model, or change 047 readiness. Card 017's `ConfiguredInstance` means
  an admitted configured-instance id in `AdmittedInstanceRecord`, not a
  prepared facade.
- Config field values stay host-private. Public records and diagnostics
  expose no paths, URLs, or env bodies. The kernel already has
  `ConfigFieldDescriptor` and `ConfigFieldRef`; do not invent a second
  field system.
...
- **Planning lineage:** g04 promoted Contract 057, tagged `v0.3.3` at
  `51d18620`, and merged the kernel in PR 4. Catalog and admission are the
  next generation-runway step. Sign-in is g04.007.
- **Why these cards are ready:** the kernel types and store exist. 016-018
  do not need a new product choice. Testkit fixtures stand in for
  adapter-local descriptors until first-proof routes.
- **Decisions and preferences:** consumers assemble catalogs the same way
  they assemble prepared facades: they link adapter crates. There is no
  runtime inventory of every production route.
- **Open tensions:** `SubjectDisclosure::Absent` is unused in the kernel;
  do not expand subject observation here. Overlay marker records already
  exist in the store; do not project them. There is no
...
- **Report after:** card 016 catalog assembly with fixture descriptors;
  card 017 admission of two instances of one family; card 018 host-private
  config refs and the PR
...
Once that checks out, take card 016 first. Build catalog assembly over
`AddableRouteDescriptor` with testkit fixtures. Keep discovery candidates
out of the catalog. When 016 is green, continue into 017, then 018. When
admission and host-private config refs are green, open the PR and stop.
...
4. From the selected worktree, confirm `git rev-parse HEAD` equals
   ...
   `git merge-base --is-ancestor c64bd64d31fa0bcfd34ea68a2c42aaeb9f0ba320 HEAD`
...
- Do not start cards 019-021.

### When the assigned runway is complete

1. Run the required final validation for card 018, plus `effigy package:api`
...
4. Open a reviewable PR against the current pushed `main` tip. The handoff's
   planning base `c64bd64d31fa0bcfd34ea68a2c42aaeb9f0ba320` is the planning
   commit before the handoff was created, not a self-referential hash for the
   commit that contains this file.
5. In the PR body, link the milestone, cards 016-018, Contract 057, changed
...
- **Closeout refs:** cards 016-018, g04.006, `docs/roadmaps/README.md`,
  `docs/logs/README.md`

...
impl ActiveTurn {
            ...
            match stop_reason {
                ...
                "max_tokens" => {
                    TerminalStatus::ProviderFailed(swallowtail_core::SafeDiagnostic::new(
                        "swallowtail.deepagents.acp.prompt_stopped",
                        "Deep Agents stopped before completing the turn",
                    ))
                }
                _ => TerminalStatus::RuntimeFailed(
                    failure(
                        "swallowtail.deepagents.acp.stop_reason_unsupported",
                        "Deep Agents returned an unsupported ACP stop reason",
                    )
                    .diagnostic()
                    .clone(),
                ),
    ...
    fn append_output(&self, text: &str) -> Result<(), RuntimeFailure> {
        ...
        if output.len().saturating_add(text.len()) > MAXIMUM_OUTPUT_BYTES {
            return Err(failure(
                "swallowtail.deepagents.acp.output_limit_exceeded",
                "Deep Agents output exceeded the adapter limit",
            ));
        }
    ...
    fn verify_session(&self, session_id: &str) -> Result<(), RuntimeFailure> {
        if session_id == self.session_id {
            ...
            Err(failure(
                ...
                "Deep Agents update does not match the active session",
            ))
        }
    }
}

...
1. [ ] Keep route `claude-code.headless`, exact published points in qualified
...
2. [ ] Freeze official `--max-budget-usd` documentation plus exact package
...
3. [ ] Build a closed version/access/value/unit table. Separate subscription
...
4. [ ] Determine whether the selected subscription-only prepared route can
...
5. [ ] Prove exact argv/environment/settings precedence, repeated-turn or
       one-run scope, limit-reached result, exit, cleanup, and omission bytes.
6. [ ] Separate requested, parsed, applied, accrued, provider-billed, enforced,
       returned, and observed truth.
7. [ ] Audit prepared input/evidence, plan/request agreement, command builder,
       decoder, fixtures, guide, matrices, and API baseline without changing
       production surfaces.
8. [ ] Promote Research 241 and complete the reserved lane log. Do not edit
       shared milestone, inventory, programme, triage, matrices, or indexes.

## Acceptance Criteria

- [ ] exact version/access/value table or honest empty set is recorded
- [ ] a non-empty row proves units, billing source, precedence, enforcement,
      terminal mapping, and route-compatible access
- [ ] local subscription allowance is not presented as API-key USD spend
- [ ] omission retains current command and authority exactly

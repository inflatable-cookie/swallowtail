...
- [2026-09-02 g05.023 Card 059 Unix Owned-Tree Attestation](./2026-09-02-g05-023-card-059-unix-owned-tree-attestation.md) — evidence stop; the operator-authorized `unsafe`/dependency boundary was driven against the setsid, descriptor-close, and identity-reuse counterexamples natively; no permitted mechanism attests owned-tree emptiness on macOS, which has no PID namespace, cgroup `populated` view, or child-subreaper; the host stays root-only, adds no unsafe, and the macOS SDK tree gate is an open operator decision

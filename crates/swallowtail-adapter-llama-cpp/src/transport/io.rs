...
fn perform_sse(
    url: Url,
    request: Request,
    mut sender: mpsc::Sender<SseFrame>,
    cancelled: Arc<AtomicBool>,
) -> Result<(), RuntimeFailure> {
    let mut easy = Easy::new();
    configure(&mut easy, &url, &request, &cancelled)?;
...
fn endpoint_failure() -> RuntimeFailure {
    failure(
        "swallowtail.llama_cpp.endpoint_invalid",
        "llama.cpp endpoint grant was invalid",
    )
}

fn curl_failure(_: curl::Error) -> RuntimeFailure {
    failure(
        "swallowtail.llama_cpp.transport_failed",
        "llama.cpp HTTP transport failed",
    )
}

fn response_limit() -> RuntimeFailure {
    failure(
        "swallowtail.llama_cpp.response_limit",
        "llama.cpp HTTP response exceeded its limit",
    )
}

fn cancelled_failure() -> RuntimeFailure {
    failure(
        "swallowtail.llama_cpp.cancelled",
        "llama.cpp HTTP work was cancelled",
    )
}

fn backpressure() -> RuntimeFailure {
    failure(
        "swallowtail.llama_cpp.stream_backpressure",
        "llama.cpp stream exceeded its bounded consumer capacity",
    )
}

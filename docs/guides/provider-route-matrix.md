...
| `cursor-agent.acp` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | exact qualified `2026.07.01-41b2de7` and `2026.07.23-e383d2b`; no inferred gap; later dates remain visible `UnverifiedNewer`; load and list advertisement does not qualify lifecycle management | connection, resource, process, and task cleanup only; provider session state is preserved |
...
| `kimi-code.acp` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | exact qualified `0.28.1` plus `0.29.0..=0.31.1`; later stable points may be visible `UnverifiedNewer`; selected ACP route advertises no close or delete | connection and process cleanup only; provider state is preserved |
...
| `grok-build.acp` | `unsupported` | `no` | `unsupported` | `unsupported` | `unsupported` | `unsupported` | maintained `0.2.114..=0.2.117`; later stable points may be visible `UnverifiedNewer`; no lifecycle method or public management binding is qualified | attachment, process, credential, and working-resource cleanup only; provider-owned session state is preserved |
